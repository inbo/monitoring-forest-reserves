...
t <- data_vegetation_all_CA %>% 
  anti_join(data_vegetation_CA) %>% 
  group_by(plot_id, period) %>% 
  summarise() %>% 
  ungroup()

# 11000: aan Peter gevraagd, zelf al gecorrigeerd
```
...
```{r check_year_veg}
t <- data_vegetation %>% 
  select(plot_id, period, contains("year"), date_vegetation) %>% 
  mutate(test_yr = (year == year_main_survey)) %>% 
  filter(test_yr == FALSE)

t %>% filter(plot_id < 10000)

# ? NA's - NEE
summary(t) 
# t %>% filter_at(vars(year_main_survey, year),any_vars(is.na(.))) %>% nrow()
# t %>% filter_at(vars(year_main_survey, year),all_vars(is.na(.))) %>% nrow()
t %>% filter_all(any_vars(is.na(.))) %>% nrow()

write_csv2(t, here::here("Data_controle/nalv_package_review_2022-03-08/verschillende_datum_veg.csv"))
# aan peter verstuurd 10/3/2022
```
...
# t %>% filter_at(vars(year_main_survey, year),any_vars(is.na(.))) %>% nrow()
# t %>% filter_at(vars(year_main_survey, year),all_vars(is.na(.))) %>% nrow()
data_vegetation %>% filter_all(any_vars(is.na(.))) %>% nrow()

t <- data_vegetation %>%  filter((is.na(year_main_survey) & !is.na(year))
                                 | (!is.na(year_main_survey) & is.na(year))
                                 )

write_csv2(t, here::here("Data_controle/nalv_package_review_2022-03-08/geen_opnamejaar.csv"))
# aan peter verstuurd 10/3/2022
```
...
# t %>% filter_at(vars(year_main_survey, year),any_vars(is.na(.))) %>% nrow()
# t %>% filter_at(vars(year_main_survey, year),all_vars(is.na(.))) %>% nrow()
data_herblayer %>% filter_all(any_vars(is.na(.))) %>% nrow()
...
Resultaten kunnen weggeschreven worden naar git, csv en access ("BR_resultaten.accdb" in folder "C:\03_BR\1_Dataverwerking\Output").
