...
Bauwensplots: de data 1954-2000 zit al in forestRePlot, en de extra opname in 2014 is 'te kort' na de opname 2000. Daarom de dataset 1954-2014 nu niet extra aanbieden, want dat wordt verwarrend. Mochten we in 2028 een extra opname doen, dan kunnen we de deeldataset 2000-2028 nog indienen.
...
(Bauwensplots na terugkoppeling met Pieter De Frenne toch niet, zie ook verder)

```{r}
# library(datapasta)
# df_paste()

list_reserves <- data.frame(stringsAsFactors = FALSE,
                                    dataset_ID = c("EU_86","EU_87",
                                          ...
                                          name = c("De Heirnisse",
                                                   ...
                                                   "Rodebos", "Rodebos",
                                                   "Sonian forest", "Sonian forest",
                                                   "Wijnendalebos",
                                                   "Withoefse heide"),
                                 name_fieldmap = c("Heirnisse",
                                                   "Everzwijnbad","Hannecart",
                                                   "Jansheideberg",
                                                   "Rodebos KV1", "Rodebos KV2",
                                                   "Zoniën KERN_1983", "Zoniën UITBR_1995",
                                                   "Wijnendalebos",
                                                   "Withoefse heide"),
                                  number_plots = c("166","146","10/50",
                                                   "131","20/108/148","20/108/148",
                                                   NA,"53","221","29"),
                                 plot_size_min = c(100L,100L,20L,100L,20L,
                                                   20L,100L,256L,100L,256L),
                                 plot_size_max = c(256L,256L,100L,256L,100L,
                                                   100L,256L,256L,256L,256L),
                                      survey_1 = c(2003L,2002L,1991L,2004L,
                                                   1991L,1991L,2000L,2000L,2002L,
                                                   2003L)
                            )

# Bauwensplots <- data.frame(stringsAsFactors = FALSE,
#                            dataset_ID = c("EU_85"),
#                            name = c("Meerdaalwoud"),
#                            name_fieldmap = c("Bauwensplots"),
#                            number_plots = c("21"),
#                            plot_size_min = c(NA),
#                            plot_size_max = c(NA),
#                            survey_1 = c(1954L)
...
Sonian forest is enkel "Zoniën KERN_1983" en "Zoniën UITBR_1995" (voorheen Kersselaerspleyn), 
...
```{r}
list_reserves %>% select(dataset_ID, name, name_fieldmap, survey_1
                         , plot_size_min
                         , number_plots) %>% DT::datatable()
```

<!-- > OPGEPAST -->
<!-- *Withoefse heide* nog niet verwerkt => strengere datacontrole -->
<!-- *Jansheideberg en Heirnisse* 2024 zitten ook nog niet in moederdb -->
<!-- 7/7/2025: in tussentijd wel verwerkt en in moederdb -->


# Templates

De templates zijn de xlsx-files door ForestREplot aangeleverd om in te vullen door ons.
Reeds deels ingevuld door Arno en verder aangevuld door mij

Zie "G:\Gedeelde drives\Team_Boseco_BR\PRJ_BR_AanvraagGegevens\UGENT_ForestREplot\templates_per_forest_reserve"

```{r}
path_to_templates <- paste0(path_to_project_teamdrive, "/templates_per_forest_reserve/")
```
...
Grazing trend vervangen door -1, 0 of 1 en in afzonderlijke tabel beschrijven over welke dieren het gaat.
...
plotinfo <- load_plotinfo(database = path_to_fieldmap_db, processed = TRUE) %>% 
  filter(plottype %in% c("CP", "CA")) %>%   # "OTHER": Bauwensplots
  filter(survey_veg == TRUE) %>% 
  rename(name_fieldmap = forest_reserve)
names(plotinfo)


# check van non-processed reeds gebeurd in script 
```

<!-- Withoefse heide nog niet verwerkt => strengere datacontrole -->
<!-- Jansheideberg en Heirnisse 2024 zitten ook nog niet in moederdb -->

Bauwensplots moeten niet mee aangeleverd worden.
Mocht dat toch op termijn toch nodig zijn, dan moeten we hier vertrekken van de originele shapefile aangemaakt door Marc Esprit, 
en kopie geplaatst op mijn c-schijf.

```{r load_strata_shapefile_NIET, results='hide', eval = FALSE}
...
Gemaild naar Peter (8/7/2025): "volgens mij kleine typfout: plot 2006, veg_3eSET:
subplot noemt daar 2001 ipv 1, waardoor link met de opnames ervoor niet klopt (scriptmatig).
Kan je dat aanpassen in de moederdatabank?"

Hierna scriptmatig opvangen, want niet zo eenvoudig in db aan te passen

```{r corr_plot_2006}
data_vegetation_ %>% filter(subplot_id == 2001)

data_vegetation_ <- data_vegetation_ %>% 
  mutate(subplot_id = ifelse(subplot_id == 2001, 1, subplot_id))
```
...
- Zonien KERN_1983: de 11 CP's maar 2x gemeten: 2011 en 2021
...
<!-- Bauwensplots wijken af van de rest:     -->

<!-- - period_1 = year_resurvey_R1 -->
<!-- - period_2 = year_resurvey_R2 (maar 14 jaar tussen!)    -->
<!-- - Baseseline survey = 1954, maar welke plots werden daar bemonsterd?  -->
<!-- Volgens Luc allemaal, maar toch best aan Arno en Yannick vragen, vooral om link te hebben, -->
<!-- ook tss 2000 en 2014. Mogelijks fieldmap immers niet 100% correct, zeker te checken. -->
<!-- Arno zou ook data van 1954 hebben volgens Luc. -->
<!-- ==> Mailings van 29/10/2024 => we laten Bauwensplots (for now) vallen -->
...
!! **opnames 1991 KV Rodebos en Hannecart**: leveren we niet aan
Immers kleinere subplots (volgens mij 2x2), geen cte plotgrootte
Zitten ook nog niet in fieldmap

!! **opnames KV Rodebos en Hannecart**: in periode 2 meer subplots opgemeten
Hier behouden we enkel de subplots die in periode 1 opgemeten werden (10 plots), dat is het 
initiele transect. De overige 40 plots werden pas in 2012 en 2022 opgemeten.
...
- **year_afforestation**: If no ancient forests: The year on which afforestation took place  
=> dat mag een range zijn voor Markus
(initieel werd gemiddeld tijdstip van twee kaartlagen genomen (voor en na bebossing): 
dat ook nog behouden)


```{r hist}
names(strata_hist)
hist <- strata_hist %>% 
  # strata_hist bevat geen "name_fieldmap"
...
#            hist_uitgebreid %in% c("0000000011", "0000001011") ~ paste(NGI1_t, BSKT_t, sep="-"),
#            hist_uitgebreid == "0000000111" ~ paste(MGI_t, NGI1_t, sep="-"),
#            hist_uitgebreid == "0000001111" ~ paste(DG3_t, MGI_t, sep="-"),
#            hist_uitgebreid == "0000011111" ~ paste(DG2_t, DG3_t, sep="-"),
#            hist_uitgebreid == "0000111111"~ paste(DG1_t, DG2_t, sep="-"))
#          , year_mid = case_when(
...
hist2_with <- hist %>% 
  ...
  mutate(former_landuse = "heathland"
         , year_afforestation = case_when(
           hist_uitgebreid %in% c("11", "1011") ~ paste(NGI1_t, BSKT_t, sep="-"),
           hist_uitgebreid == "111" ~ paste(MGI_t, NGI1_t, sep="-"),
           hist_uitgebreid == "1111" ~ paste(DG3_t, MGI_t, sep="-"),
           hist_uitgebreid == "11111" ~ paste(DG2_t, DG3_t, sep="-"),
           hist_uitgebreid == "111111"~ paste(DG1_t, DG2_t, sep="-"))
         , year_mid = case_when(
           ...
           hist_uitgebreid == "111111"~ (DG1_t + DG2_t)/2)
         )
...
hist4_wijn <- hist %>% 
  ...
  mutate(former_landuse = "arable"
         , year_afforestation = case_when(
           hist_uitgebreid == "1011111111" ~ paste(VDM_t, Gkad_t, sep="-"),
           hist_uitgebreid == "1000111111" ~ paste(DG1_t, DG2_t, sep="-"),
           hist_uitgebreid == "1110011111" ~ paste(DG2_t, DG3_t, sep="-"))
         , year_mid = case_when(
...
# Heirnisse: plot 757 volgens topokaart 2009 op een weg (zie mail van 29/10/2024 van Marc Esprit)
# hier toch als bos beschouwen, aangezien er in 2003 en 2014 een gewone kruidlaagbedekking is
...
hist5_heirn <- hist %>% 
  ...
  mutate(former_landuse = case_when(
         ...
         , year_afforestation = case_when(
           hist_uitgebreid == "1111" ~ paste(DG3_t, MGI_t, sep="-"),
           #110011110: we veronderstellen dat dit 110011111 moet zijn
           hist_uitgebreid %in% c("11111", "1110011111", "111011111", "110011111", "110011110", "110011111", "111011111") ~ paste(DG2_t, DG3_t, sep="-"),
           hist_uitgebreid %in% c("111111", "110111111") ~ paste(DG1_t, DG2_t, sep="-"),
           hist_uitgebreid == "111111111" ~ paste(Ferr_t, VDM_t, sep="-"))
         , year_mid = case_when(
           hist_uitgebreid == "1111" ~ (MGI_t + DG3_t)/2,
           #110011110: we veronderstellen dat dit 110011111 moet zijn
           hist_uitgebreid %in% c("11111", "1110011111", "111011111", "110011111", "110011110", "110011111", "111011111") ~ (DG3_t + DG2_t)/2,
...
#   mutate(former_landuse = "arable"
#          , year_afforestation = case_when(
#            hist_uitgebreid %in% c("1110011111") ~ paste(DG2_t, DG3_t, sep="-"),
#            hist_uitgebreid %in% c("1110111111") ~ paste(DG1_t, DG2_t, sep="-"))
#          , year_mid = case_when(
...
hist_total <- hist_total %>% 
  mutate(year_mid = round(year_mid, 0)) %>% 
  select(dataset_ID, name, name_fieldmap
         , plot_id
         , hist_uitgebreid
         , former_landuse, year_afforestation, year_mid) 
...
Z gemeenschappelijk uit shapefile van Marc op plotniveau
...
coord_centre <- info_XY %>% 
  # info_XY bevat geen "name_fieldmap"
...
names(coord_all)  
names(info_survey) # ook plotsize_veg én periode
veg_plot_size <- info_survey %>% 
  select(plot_id, plotsize_veg) %>% unique
names(veg_plot_size)
...
plotinfo_def <- hist_total %>%  
  ...
  left_join(coord_all) %>% 
  left_join(veg_plot_size) %>% 
   # plotsize
  mutate(plot_size = ifelse(str_detect(plotsize_veg, "16 x 16 m")
                           , 256
                           , ifelse(str_detect(plotsize_veg, "10 x 10 m")
                                    , 100
                                    , ifelse(str_detect(plotsize_veg, "5 x 5 m")   # Bauwensplots
                                             , 25, NA)
                           )
  )) %>% 
...
table(plotinfo_def$plotsize_veg)
...
We hebben enkel de plots weerhouden, waar een (her)opname gebeurd is met 20 jaar tussen.
Ook de tussenliggende opnames werden weerhouden.


```{r}
data_herblayer %>% filter(is.na(year)) %>% nrow() == 0 # 25 plots With heide: geen opname in periode 2

data_herblayer_def <- data_herblayer %>% 
  filter(!is.na(year))

nrow(data_herblayer_def) == nrow(data_herblayer)
```
...
<!-- De opnames van 2000 en 2014 zijn dan R1 en R2 -->
...
```{r only_with_heide, results='hide'}
...
```{r only_bauwens_NIET, results='hide', eval = FALSE}
...
### Withoefse heide - EU_93

With heide is speciaal: enkel R1
...
veg_sampling_EUxx <- veg_sampling_with %>% 
  filter(dataset_ID == EUxx)
...
```{r}
veg_sampling %>% filter(dataset_ID == EUxx) %>% distinct(year)
```


**Export**

```{r export_EU_93, eval = FALSE}
...
Narcissus in kernvlakte opgemeten in 2005 ipv 2004, hoofdopname toch 2004
...
- Bauwens - toch niet, want 1954-2000 zit al in db
1954-2014: geen grote meerwaarde, en zorgt voor verwarring => wachten tot ev. 2000-2028
...
```{r results='hide'}
veg_sampling_EUxx_R2 <- veg_sampling_EUxx %>% 
  select(-B, -R1) %>% 
  spread(plot_origID, R2) 
  
veg_sampling_EUxx_R2 %>% filter(is.na(species)) %>% nrow() == 0
  
names(veg_sampling_EUxx_R2)
veg_sampling_EUxx_R2 %>% ncol() == nrow(veg_sampling_EUxx %>% distinct(plot_origID)) + 4

```

```{r eval = FALSE}
# veg_sampling_EUxx_R2 <- veg_sampling_EUxx_R2 %>% 
#   # NA moet "0" worden
#   mutate(across(everything(), ~ replace_na(., 0)))
```
...
write.xlsx(veg_sampling_EUxx_R2
           , paste0(path_to_datarequest, EUxx, "_data.xlsx")
           , sheetName = "veg_R2"
           , showNA = FALSE
           , append = TRUE) 
...
source_file <- here::here(paste0("Scripts/AanvraagGegevens/UGENT_ForestREplot/ForestREplot_plotinfo_veg_2025.html"))
