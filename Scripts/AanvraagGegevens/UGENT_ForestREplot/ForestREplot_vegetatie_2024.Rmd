...
Ze vragen om enkel de eerste en laatste dataset aan te leveren, niet de tussenliggende opnames.
...
> !! opnames 1991: in kleinere plots (volgens mij 2x2)? Dan misschien beter niet aanleverenen, 
...
table(plotinformation_$VegePlot)
...
```{r}
names(plotinformation)

plotinformation <- plotinformation %>% 
  select(dataset_ID, name, name_fieldmap
         , plotID = plot_id, plot_origID
...
Enkel de plots behouden waar er in periode 1 een veg-opname gebeurd is.
Bij de andere plots immers geen 20 jaar tussen
...
> Gevraagd mail van 29/10/2024

```{r filter}
plotinfo_def <- plotinformation %>% 
  filter(!is.na(year_baseline_survey))

# i!! mogelijks niet correct voor Bauwensplots
```
...
```{r veg_data}
# hoger reeds ingeladen
nrow(data_vegetation) #4228
table(data_vegetation$name_fieldmap)
```
...
data_herblayer <- data_herblayer_ %>% 
  rename(name_fieldmap = forest_reserve) %>% 
  inner_join(list_reserves %>% select(dataset_ID, name_fieldmap), by = c("name_fieldmap")) %>% 
  mutate(plot_origID = paste(plot_id, subplot_id, sep = "_")) %>% 
  left_join(qHerbSpecies, by=c("species" = "ID"))

nrow(data_herblayer)# 42203
...
veg_sampling <- data_herblayer %>% 
  mutate(layer = "herb") %>% 
  select(dataset_ID, species
         , name_sc, layer, plot_origID
         , period, year
         , cover = coverage_class_average_perc)
names(veg_sampling)

```

In tabblad "plot_information" moeten we aangeven welk jaartal elke survey is
...
- baseline is overal periode 1 (!! wegfilteren waar year van periode 1 = NA) 
- resurvey is overal periode 3 !! 
...
```{r}
plots_resurveys <- plotinformation %>% 
  filter(!is.na(year_baseline_survey))
```

Bij intensieve monitoring sites is de tweede survey altijd periode 3
Baseline survey altijd periode 1

```{r only_int_monitoring}
...
veg_sampling_bauwens <- veg_sampling %>% 
  filter(dataset_ID == "EU_85") %>% select(-year) %>% 
  spread(period, cover, sep = "_") %>% 
  select(dataset_ID, species
         , name_sc, layer, plot_origID 
...
> ? wat met de NA's : die zouden allemaal "0" moeten worden
