...
We filteren de plots weg waar geen heropname gebeurd is
...
data_herblayer_def <- data_herblayer %>% 
  filter((dataset_ID != "EU_85" & period %in% c(1, 3)) |
           (dataset_ID == "EU_85" & period %in% c(1, 2))
  )

data_herblayer_def %>% filter(is.na(year)) %>% nrow() == 0
```
...
Starten met één site en bekijken of dat zo makkelijk kan overgebracht worden naar de templates.

--> EU_92	Wijnendalebos	

```{r EU_92, eval = FALSE}
write.xlsx(plotinfo_EU92
           , paste0(path_to_datarequest, "data_EU92.xlsx")
           , sheetName = "plotinfo"
           , showNA = FALSE
           , append = FALSE) 
