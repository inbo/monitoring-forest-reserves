...
We laden hier enkel de plots in waar `survey_veg = TRUE`.
...
# enkele plot svan With heide, Wijnendale en CoolhemQ - klopt


plotinfo <- plotinfo %>% 
  filter(data_processed == TRUE)
```
10 plots zijn niet processed: enkele plots van With heide, Wijnendale en Coolhem A - klopt.
We gaan enkel verder met de processed plots.
...
# processed = TRUE: pas dan wordt gekeken of er een opname gebeurd is van resp dendro, reg of veg
# als processed = TRUE, dan wordt zowel DataProcesssed_YN als Survey_Vegetation_YN gecheckt 
# als processed = FALSE, dan worden alle plots ingeladen, of er nu wel of geen veg-opname gebeurd is
...
data_vegetation %>% filter(is.na(total_herb_cover_id)) %>% nrow() # plots 1eSET + enkel 2eSET
t <- data_vegetation %>% filter(is.na(total_herb_cover_id))
```



# Info voor invullen van templates 

We maken enkele files aan die helpen om de templates van ForestREplot verder in te vullen.
...
dominant_tree_ <- dominant_tree %>% 
  ...
  group_by(forest_reserve, plottype, MaxBasalAreaSpeciesName, name_sc) %>% 
...
hist_per_reserve <- strata_hist %>% 
   # left_join(info_beheer %>% select(plot_id, forest_reserve_fieldmap)) %>% 
  left_join(plotinfo %>% select(plot_id, forest_reserve_fieldmap = forest_reserve) %>% unique()) %>% 
  group_by(forest_reserve_fieldmap, hist_beknopt, boslftd) %>% 
  summarize(n_plots = n(),
            n_CP = sum(plot_id < 10000),
            n_CA = sum(plot_id > 10000),
...
list_overview <- list(
  survey_info = survey_info
  , dominant_tree_n_plots = dominant_tree_
...
save_results_csv(
    ...
    output_dir = paste0(path_to_datarequest, "site_plot_info/"))

write.xlsx(all_info
           , paste0(path_to_datarequest, "site_plot_info/extra_info.xlsx")
...
save_results_csv(
    results = list_overview,
    output_dir = paste0(path_to_datarequest_teamdrive, "site_plot_info/"))

write.xlsx(all_info
           , paste0(path_to_datarequest_teamdrive, "site_plot_info/extra_info.xlsx")
           , sheetName = "extra_info"
           , showNA = FALSE
           , append = FALSE) 
...
source_file <- here::here(paste0("Scripts/AanvraagGegevens/UGENT_ForestREplot/ForestREplot_metadata_2025.html"))
