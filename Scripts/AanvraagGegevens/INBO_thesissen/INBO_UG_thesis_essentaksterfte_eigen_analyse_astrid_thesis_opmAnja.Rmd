...
```{r data inlezen}
boomdata <- read.csv('C:/3BR/3_AanvraagGegevens/INBO_UG_thesis_essentaksterfte/boomdata.csv', sep = ';', dec  = ',')
stemdata <- read.csv('C:/3BR/3_AanvraagGegevens/INBO_UG_thesis_essentaksterfte/stemdata.csv', sep = ';', dec  = ',')
plotdata <- read.csv('C:/3BR/3_AanvraagGegevens/INBO_UG_thesis_essentaksterfte/plotdata_dendro.csv', sep = ';', dec  = ',')
vegetatie <- read.csv('C:/3BR/3_AanvraagGegevens/INBO_UG_thesis_essentaksterfte/vegetatie.csv', sep = ';', dec  = ',')
verjonging <- read.csv('C:/3BR/3_AanvraagGegevens/INBO_UG_thesis_essentaksterfte/verjonging.csv', sep = ';', dec  = ',')
```

**@Astrid**: misschien ok handig om lookuplijsten (betekenis van de gebruikte codes) in te lezen.
Ik had deze gegroepeerd in het access-bestand "lookuplijsten.accdb". 
Je kan die binnenhalen door eerst een connectie te leggen met deze db. 
Dan heb je wel library(RODBC) nodig.
Zie chunk hierna

```{r lookuplijsten_inlezen}
con <- odbcConnectAccess2007("C:/3BR/3_AanvraagGegevens/INBO_UG_thesis_essentaksterfte/lookuplijsten.accdb")

qiufrovitality <- sqlFetch(con, "qiufrovitality")
...
**@Astrid**: hierna heb ik iufro_vital en iufro_vital_shoots gecombineerd
```{r stemdata_vital}
str(stemdata)

stemdata <- stemdata %>% 
  mutate(iufro_vital = ifelse(iufro_vital == 50, iufro_vital_shoots, iufro_vital))

table(stemdata$iufro_vital)
...
**@Astrid**: ik (anja) kom hier iets anders uit, zie onderstaande chunk: 
83 plots met in beide periodes es
38 plots enkel periode 1
41 plots enkel periode 2

```{r}
t <- plotdata_es %>% 
  group_by(plot_id) %>% 
  summarize(aantal_periodes = unique(period), 
            max_periode = max(period)) %>% 
  ungroup()

table(t$aantal_periodes, t$max_periode)
  #    1  2
  # 1 38 41
  # 2  0 83
...
Ondanks het feit dat er over heel Vlaanderen verschillende evoluties van het essen-stamtal waar te nemen zijn, is dit niet het geval in het Muizenbos. Hier kennen alle plots een dalend stamtal es. Omdat enkel het levende essen in rekening worden gebracht, duidt deze trend op een duidelijke sterfte. 
**@Astrid**: niet noodzakelijk: stamtallen (spilniveau) kunnen sterk variëren: het zijn niet noodzakelijk individuele bomen of volledige hakhoutstoven die verminderen.
Bv. hakhoutstoof in periode 1 met 10 dunne spillen, kan mogelijks in periode 2 een hakhoutstoof met 5 zwaardere spillen geworden zijn.


Dit wordt bevestigd door de evolutie van het grondvlak en het volume van de levende en dode essen. 
**@Astrid**: ik zou me dan eerder enkel baseren op grondvlak en volume
...
**@Astrid**: OPGEPAST: iufrovital = 50 zijn hakhoutstoven. de vitaliteit wordt op het niveau van de spillen bepaald. 
Ik bezorg je de ruwe data van de spillen (stemdata.csv).

**@Astrid**: ik zou de eerste plot van onderstaande chunk ook eerder weergeven als een barplot, en periode ev. als factor definiëren eerder dan als integer. 
(het is beetje verwarrend dat op de X-as 1.5 staat voor periode ...)
...
```{r voorstel_anja, fig.width=8, fig.height=6}
vit_es_muiz <- boomdata %>%
  filter(species == c(14),
         forest_reserve == 'Muizenbos') %>%
  select(plot_id, tree_id, year, period, iufro_vital, alive_dead, ind_sht_cop) %>% 
  left_join(qiufrovitality %>% select(ID, vital_txt = Value1), by = c("iufro_vital" = "ID"))
...
ggplot(data = vit_es_muiz %>%
  mutate(period = as.factor(period), 
         vital_txt = as.factor(vital_txt)) %>% 
  group_by(period) %>%
  count(vital_txt), 
  mapping = aes(x = period, y = n)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  # theme_bw() +
  labs(title = 'Evolutie van vitaliteit es in het Muizenbos',
       x = 'Meetronde',
       y = 'Aantal essen') + 
    facet_wrap(~vital_txt)
...
ggplot(data = vit_es_muiz %>%
  ...
  group_by(period) %>%
  count(vital_txt), 
  mapping = aes(x = vital_txt, y = n, fill = period)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  # theme_bw() +
  labs(title = 'Evolutie van vitaliteit es in het Muizenbos',
       x = 'Vitaliteit',
...
# @Astrid: heightclass: eventueel joinen om duidelijkere grafiek te bekomen
view(qheightclass)
...
**@Astrid**: verschillende correlatie en significantie kan komen doordat je minder waarnemingen hebt (door hoogteklasses samen te nemen).
Ook nog klein typfoutje in laatste chunk ;-)
...
test <- verjonging %>%
  filter(forest_reserve == 'Muizenbos',
         species == 14 | species == 20 | species == 28 | species == 39 | species == 54 | species == 69 | species == 87) %>%
  select(plot_id, period, species, mean_number_of_trees_ha, height_class) 

test2 <- plotdata_es_muiz %>%
  select(-species) %>%
  rename(nr_trees_ha_es = number_of_trees_ha) %>% 
  left_join(test, by = c('plot_id','period')) %>%
  group_by(plot_id, period, species, nr_trees_ha_es, as.character(height_class)) %>%
  summarise(som_regeneration = sum(mean_number_of_trees_ha)) %>%
  pivot_wider(names_from = 'species',
              values_from = 'som_regeneration', 
              values_fill = 0) %>%
  rename(es = `14`, GelderseRoos = `20`, hazelaar = `28`, lijsterbes = `39`, RodeKornoelje = `54`, zomereik = `87`, vlier = `69`, height_class = `as.character(height_class)`)


 ggscatter(test2, x = "nr_trees_ha_es", y = "es", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "individuen es per ha", ylab = "individuen verj. es per ha", 
          title = 'es - es (14)') 
...
test3 <- verjonging %>%
  filter(forest_reserve == 'Muizenbos',
         species == 14 | species == 20 | species == 28 | species == 39 | species == 54 | species == 69 | species == 87) %>%
  select(plot_id, period, species, mean_number_of_trees_ha)

test4 <- plotdata_es_muiz %>%
  select(-species) %>%
  rename(nr_trees_ha_es = number_of_trees_ha) %>%
  left_join(test3, by = c('plot_id','period')) %>%
  group_by(plot_id, period, species, nr_trees_ha_es) %>%
  summarise(som = sum(mean_number_of_trees_ha)) %>%
  pivot_wider(names_from = 'species',
              values_from = 'som', 
  # @Astrid: in lijn hierna had je een foutje staan "values_fill=)" je was de 0 vergeten
  # maar maakt niet veel verschil ....
              values_fill = 0) %>%
  rename(es = `14`, GelderseRoos = `20`, hazelaar = `28`, lijsterbes = `39`, RodeKornoelje = `54`, zomereik = `87`, vlier = `69`) 

 ggscatter(test4, x = "nr_trees_ha_es", y = "es", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "individuen es per ha", ylab = "individuen verj. es per ha", 
          title = 'es - es (14)') 
