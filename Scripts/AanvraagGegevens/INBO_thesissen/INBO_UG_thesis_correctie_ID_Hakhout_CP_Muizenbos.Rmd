---
title: "Koppeling van hakhout nader bekeken CP's Muizenbos"
author: "Anja Leyman"
date: "`r Sys.Date()`"
output: 
  html_document:
...
Dit script is een aanvulling bij "Dataverwerking_KV_Muizenbos__afzonderlijk_ID_Hakhout.Rmd".
Daarin werd een unieke tree_id aangemaakt, die correct is voor zowel individuele bomen, als voor hakhout.

Voor individuele bomen en simpel hakhout wordt deze opgebouwd als volgt:
...
Voor hakhoutstoven met een levend en dood deel als volgt: 
- eerste periode waarin gemeten
- plot_id
- coppice_id
- a (levend) of b (dood)
...
Probleem is dat coppice_id niet steeds correct toegekend werd/wordt: zie daarvoor folder "C:/3BR/4_LiedekSevendMuizenbHarras/!Verwerking/4Muizenbos/3Statistieken/": 
- "possible_error_coppiceID_CP_Muizenbos.csv" en 
- "error_coppice_id_CP.xlsx"
...
# deze worden ook in "Setup_ForresCalc.R" gedefinieerd, hier ev. extra "paths" toevoegen indien nodig voor huidig script
      ...
      # 
      # path_to_analysis_set_db <- "C:/3BR/1_DataVerwerkingBR/Output/BR_analysedb.accdb" 
...
# resultaten
path_to_results_csv <- "C:/3BR/4_LiedekSevendMuizenbHarras/!Verwerking/4Muizenbos/3Statistieken/"
path_to_results_db <- "C:/3BR/4_LiedekSevendMuizenbHarras/!Verwerking/4Muizenbos/3Statistieken/CP_Muizenbos_results_R.accdb"
```
...
trees <- sqlFetch(con, "trees_calc") %>% 
  filter(forest_reserve == "Muizenbos" & plottype == 20)
...
nrow(trees_copp_id_3) + nrow(trees_copp_id_4) == nrow(trees_copp_id) 
...
possible_error <- summ_check %>% 
  filter(abs(x_min - x_max) > 1 | abs(y_min - y_max) > 1)

write_csv2(possible_error, paste0(path_to_results_csv, "possible_error_coppiceID_CP_Muizenbos.csv"))
```
...
# L1_1_D1_0_L2_1_D2_1 L1_1_D1_1_L2_0_D2_1 L1_1_D1_1_L2_1_D2_0 
#                  24                   1                  13 
...
nrow(trees) == nrow(trees_old_id) + nrow(trees_copp_id_3) + nrow(trees_copp_id_4)
