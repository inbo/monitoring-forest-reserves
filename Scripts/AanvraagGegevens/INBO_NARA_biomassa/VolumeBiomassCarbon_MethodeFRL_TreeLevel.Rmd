---
title: "Volume, biomassa en koolstofgehalte volgens FRL en INBO-advies A.4103 (methode 4b)"
...
analyseSet <- trees_living6 %>%
  ...
  dplyr::summarise(
            ...
            AboveGrBiomass_t_ha = sum(TotalAboveGrBiomass_t/plotarea_ha, na.rm = TRUE),
            BelowGrBiomass_t_ha = sum(TotalBelowGrBiomass_t/plotarea_ha, na.rm = TRUE),
            TotalBiomass_t_ha = sum(TotalBiomass_t/plotarea_ha, na.rm = TRUE),
            AboveGr_C_t_ha = sum(FC*TotalAboveGrBiomass_t/plotarea_ha, na.rm = TRUE),
            BelowGr_C_t_ha = sum(FC*TotalBelowGrBiomass_t/plotarea_ha, na.rm = TRUE),
            TotalCarbon_t_ha = sum(TotalCarbon_t/plotarea_ha, na.rm = TRUE),
            # test = StemVolume_m3_ha - vol_bole_m3_ha,
            # test_proc = 100*test /vol_bole_m3_ha
            ) %>%
  dplyr::ungroup() 
# %>% 
  # mutate(test = TotalCarbon_t_ha - AboveGr_C_t_ha - BelowGr_C_t_ha)
...
analyseSet_0_biomassa <- zero_volume %>%
  mutate(year = year_dendro,
         ...
         AboveGrBiomass_t_ha = 0,
         BelowGrBiomass_t_ha = 0,
         TotalBiomass_t_ha = 0,
         AboveGr_C_t_ha = 0,
         BelowGr_C_t_ha = 0,
         TotalCarbon_t_ha = 0
...
# wegschrijven
write_excel_csv2(analyseSet,paste0(path_to_request, "analyseSet_living_C.csv"))

# direct naar gdrive
write_excel_csv2(analyseSet,paste0(path_to_gdrive, "analyseSet_living_C.csv"))
...
colnames(analyseSet)
variables_for_statistics <- c("vol_bole_m3_ha", "StemVolume_m3_ha", "TotalVolumeVEF_m3_ha", "VolumeStump_m3_ha",  "TotalBiomass_t_ha", "TotalCarbon_t_ha")
...
analyseSet_wide <- analyseSet %>% 
  select(plot_id, period, TotalCarbon_t_ha, vol_bole_m3_ha) %>% 
  pivot_wider(id_cols = plot_id, 
            names_from = period, 
            values_from = c("TotalCarbon_t_ha", "vol_bole_m3_ha")) %>% 
  mutate(increase_C = `TotalCarbon_t_ha_2` - `TotalCarbon_t_ha_1`,
...
vars_luc <- c("TotalCarbon_t_ha", "vol_bole_m3_ha")
...
analyseSet_wide_pnv <- analyseSet_pnv %>% 
  select(plot_id, PNV, PNVtxt, period, TotalCarbon_t_ha, vol_bole_m3_ha) %>% 
...
```{r table_results_PNV}
Resultaat_wide %>% arrange(variable) %>% 
