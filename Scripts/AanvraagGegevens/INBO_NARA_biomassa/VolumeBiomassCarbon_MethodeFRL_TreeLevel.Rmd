...
Ikv NARA2023 wordt gevraagd naar een ruw cijfer mbt C-opslag in onbeheerde bossen.
...
Als methode gebruiken we de merthode uit het advies van INBO (INBO A.3104) ikv LULUCF rapportering.
...
trees_calc <- read_csv2(paste0(path_to_treelevel_csv, "trees_calc.csv")) %>% 
  filter(plottype == "CP") %>% select(-1)
stems_calc<- read_csv2(paste0(path_to_treelevel_csv, "stems_calc.csv")) %>% 
  filter(plottype == "CP") %>% select(-1)

reg <- read_csv2(paste0(path_to_plotlevel_csv, "regeneration_by_plot.csv")) %>% 
  filter(plottype == "CP") %>% select(-1)
...
We selectreen de bosreservaten met miniumaal twee surveys.
...
Deze plots leiden we af van `regeneration_by_plot_height_species.csv`, cfr advies A3844 voor berekening van totaal volume per boomsoort.
...
- enkel werken met plots waar bomen met DBH > 7 cm voorkomen => bij VBI lagere uptake factoren (zie script "GEG_LULUCF_MeanCarbonUptake_Methodes1234TreeLevel_enkel_plots_met_bomen.Rmd")
- ook plots zonder verjonging toevoegen => bij VBI hogere uptake factoren (zie script "GEG_LULUCF_MeanCarbonUptake_Methodes1234TreeLevel_alle_bosplots.Rmd")

<br>

```{r bosplots_zonder_volume_obv_reg, results='hide'}
names(reg_)

check_zero_volume <- analyseSet %>% 
  full_join(reg_ %>% select(plot_id, period, number_of_tree_species), by = c("plot_id", "period")) %>% 
  filter(is.na(vol_bole_m3_ha))

check_zero_volume
# plot 475: periode 1: geen bomen, ook geen verjonging (approx_nr_reg = 0) 
# => om consequent te zijn met VBI:  NIET MEENEMEN
# TOCH WEL: VBI zal in beide periodes plots verliezene, hier is het maar één plot, één periode 

check_zero_volume$year <- 2002
...
colnames(analyseSet);colnames(check_zero_volume)

analyseSet_0_biomassa <- check_zero_volume %>%
  dplyr::select(-number_of_tree_species) %>%
  mutate(vol_bole_m3_ha = 0,
         StemVolume_m3_ha = 0,
         VolumeStump_m3_ha = 0,
...
- een csv-file: resultVolumeBiomassCarbonUptake_MethodeFRL.csv
...
```{r calc_increase_C_per_plot_PNV}
...
```{r export_to_csv_repre, include=FALSE}
write_excel_csv2(plots_per_pnv,paste0(path_to_request, "forest_reserve_plots_per_pnv.csv"))

write_excel_csv2(plots_per_pnv,paste0(path_to_request_luc, "forest_reserve_plots_per_pnv.csv"))
...
Luc zou eventueel toch PNV toekennen.
