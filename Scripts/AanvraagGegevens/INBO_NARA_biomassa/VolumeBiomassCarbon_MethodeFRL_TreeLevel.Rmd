...
- een csv-file met results-wide

```{r resultatendb_periode1vs2_toestand}
# Toestand

resultsdb <- paste0(path_to_output, "BR_resultaten.accdb")

save_results_statistics(dbHandle = resultsdb
...
results_toename <- as.data.frame(Resultaat_wide) %>%
  dplyr::select(-period_1, -period_2, -increase) %>% 
  mutate(period = NA
         , stratumNaam = NA
         ...
         , n_obs = NA
         , strata = NA
         , lci = NA #as.numeric(as.character(lci))
         , uci = NA #as.numeric(as.character(uci))  
         ) %>%
...
```{r create_results_wide_PNV, results='hide'}
# van long naar wide
# dan aftrekken en delen door 16
names(Resultaat)

Resultaat_wide <- Resultaat %>% 
  dplyr::select(variable, period, PNV, PNVtxt, mean) %>% 
  spread(period, mean, sep = "_") %>% 
  mutate(increase = `period_2` - `period_1`,
         increase_per_yr = round(increase/10, 2), # 10 jaar gemiddeld tss 2 opnames
         period_1 = round(`period_1`, 2),
...
- de resultatendatabank 
- een csv-file: resultVolumeBiomassCarbon_MethodeFRL_TreeLevel.csv
- een csv-file met results-wide: resultVolumeBiomassCarbonUptake_MethodeFRL_TreeLevel.csv
