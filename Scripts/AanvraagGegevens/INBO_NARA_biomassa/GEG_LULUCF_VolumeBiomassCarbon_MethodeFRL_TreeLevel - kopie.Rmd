...
bosopp1 <- bosopp %>% filter(periode == 1 & str_detect(string = beschrijving, pattern = ': 0,')) %>% 
                               pull(gemiddelde) %>% 
                               unique()
                             
bosopp2 <- bosopp %>% filter(periode == 2 & str_detect(string = beschrijving, pattern = ': 0,')) %>%
                               pull(gemiddelde) %>% 
                               unique()
...
bosopp_10 <- bosopp %>% 
      filter(periode == '2' & str_detect(string = beschrijving, pattern = as.character(maxyear2))) %>% 
      pull(gemiddelde)

# enkel relevant voor afgewerkt == c(0), indien niet werken we enkel met de LAATSTE 10 jaar (zolang VBI3 niet 100% klaar is en tijdsverschil voor alle plots exact 10 jaar is)
bosopp_20 <- bosopp %>% 
      filter(periode == '1' & str_detect(string = beschrijving, pattern = as.character(maxyear2))) %>% 
      pull(gemiddelde) 



bosopp1; bosopp2; bosopp_20; bosopp_10; 

# 100% vbi1
# 100% VBI2
# 20 - 10 jaar geleden (afgewerkt % VBI2 en rest VBI1)
# voorbije 10 jaar (afgewerkt % VBI3  en rest VBI2)
...
```{r}
bosopp_productief <- get_prod_bosopp(connectieResultaten = connectieResultaten, afgewerkt = afgewerkt, MeetnetDetails = MeetnetDetails)
```
...
df$VBI <- factor(ifelse(df$periode ==1, a1,a2 ))
...
# finaal is deze grafiek enkel goed als afgewerkt == 0, optimaal zou zijn om een functie te hebben die per afgewerkt (0 en dan bv 3) de berekeningen doet en samenvoegt vooraleer er een grafiek mee te maken. Op de grafiek staat dan VBI1, VBI2, voortschrijdend gemiddelde laatste 10 jaar.
