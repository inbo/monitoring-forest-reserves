...
xy_all_ <- xy_all %>% 
  ...
  mutate(Xproj_m = ifelse(is.na(Xproj_m), x_shp, Xproj_m)
         , Yproj_m = ifelse(is.na(Yproj_m), y_shp, Yproj_m)
  ) %>% 
  select(-x_shp, -y_shp)
xy_all_ %>% filter(is.na(Xproj_m)) %>% nrow() == 0
...
Toevoegen van globale XY-coördinaten van de kernvlaktes.
Berekening van globale coördinaten obv lokale coördinaten. 
...
## Toevoegen globale coördinaten plot

Toevoegen coördinaten aan "dendro_by_plot".    

We nemen de globale coördinaten over van de `GPSReferencePoints` uit Fieldmap.
...
> !! deze van Kluisbos zitten niet in tabel Fieldmap => moeten uit shp gehaald worden
...
## Berekenen globale coördinaten stems

Obv plot-coördinaten:   

- tabel `GPSReferencePoints` uit Fieldmap
- shapefile voor Kluisbos
...
De gegegevens op spilniveau - "result_stems.csv" - worden weggeschreven naar de c-schijf en de teamdrive.
...
#            , paste0(path_to_datarequest, "result_stems_cirkelplots.xlsx")
#            , sheetName = "stems"
#            , showNA = FALSE
#            , append = FALSE) 
...
XY-coordinaten kunnen uit de shapefile gehaald worden
