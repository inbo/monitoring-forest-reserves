---
title: "Prognoses stem data from Sonian Forest"
author: "Anja Leyman"
date: "`r Sys.Date()`"
output: 
  html_document:
...
# paths ---- 
    ### eigen c-schijf
path_to_c <- "C:/03_BR/1_DataVerwerkingBR/Aanvraag_gegevens/INBO_Prognoses/"
...
# path_to_db <- paste0(path_to_aws, "PROGNOSES_EDIT12m/FieldMapData_PROGNOSES_EDIT12m.accdb")
    ...
    ### gdrive
path_to_gdrive <- "G:/Gedeelde drives/Team_Boseco_BR/PRJ_BR_LIFE-PROGNOSES/"
...
Dit script creëertde ruwe tree data op spilniveau ('stems') voor het project "PROGNOSES".
...
summary(data_stems_calc$stem_number_alive_ha)   # max 1 
...
summary(data_dendro_calc$nr_of_stems)  # max 4
summary(data_dendro_calc$number_of_trees_alive_ha)  # max 1

#is er hakhout?
coppice <- data_dendro_calc %>% 
...
#is er hakhout?
coppice <- data_stems_calc %>% 
  filter(ind_sht_cop == 12)

stems_prognoses <- data_stems_calc %>% 
    select(forest_reserve, plot_id, tree_measure_id, shoot_measure_id
           , date_dendro, period
           , contains(c("r_A", "dbh_min_a"))
           , species, alive_dead, ind_sht_cop
           , dbh_mm, height_m, calc_height_m
           , decaystage, intact_snag
           # , nr_of_stems    # enkel op tree-niveau, niet op stemniveau, stem_number_alive_ha   # 1
           , basal_area_m2, vol_bole_m3, vol_crown_m3
    )
...
```{r }
stems_calc_old <- read_csv2("C:/03_BR/1_DataVerwerkingBR/Output/_tree-level-data/stems_calc.csv") %>% 
...
vgl <- stems_prognoses %>% 
  filter(plot_id < 2200) %>% 
  left_join(stems_calc_old, by = c("plot_id", "tree_measure_id", "shoot_measure_id", "period")) %>% 
  select(plot_id, tree_measure_id, shoot_measure_id,
...
write.csv2(stems_prognoses, paste0(path_to_output_c, "stems_prognoses.csv"))
...
write.csv2(stems_prognoses, paste0(path_to_output_gdrive, "stems_prognoses.csv"))
...
my_sheet <- gs4_create(name = "stems_prognoses", sheets = stems_prognoses) 
