...
name_forest <- "zonien"
bosdomein_bosdat <- c("Zoniën ANB Domeinbos", "Zoniën Koninklijke Schenking", 
                      "Zoniën BR Zwaenepoel")
...
```{r list_enkel_zonien, results='hide'}
names(BOSBESTAND_)
names(OPNAME_TABEL_)
...
```{r list_export_gdrive_zonien, eval = FALSE}
# stats
for (i in 1:length(list_export_zonien)){
	  write.csv2(list_export_zonien[[i]], 
	             paste0(output_path_gdrive, names(list_export_zonien)[i], ".csv")) 
}
...
source_file <- here::here(paste0("Scripts/AanvraagGegevens/INBO_bosdat/INBO_oude_bosdat_bevragen_zonien.html"))
