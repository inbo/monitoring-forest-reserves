---
title: "Tabellen uit bosdat ophalen, joinen en exporteren"
subtitle: "Mortagne"
author: "Anja Leyman"
date: "`r Sys.Date()`"
output: 
  html_document:
    keep_md: FALSE
    number_sections: yes
    fig_caption: yes
    code_folding: hide
    toc: TRUE
    toc_float:
      collapsed: FALSE
      smooth_scroll: FALSE
---

# Setup

```{r rm}
rm(list=ls())
```


```{r setup0, include=FALSE}
knitr::opts_chunk$set(
  echo = TRUE,
  eval = TRUE,
  message = FALSE,
  warning = FALSE, 
  fig.width = 9,
  fig.align = TRUE)

library(here)

# libraries & invoergegevens
source(here::here("scripts/Setup.R"))

library(RODBC)
library(readr)
library(dplyr)
library(tidyr)
library(dhcurve)
library(readxl)
library(stringr)
library(writexl)

```


```{r Path}
# deze worden ook in "Setup.R" gedefinieerd (aanroepen van "Paths.R")
# hier ev. extra "paths" toevoegen indien nodig 

path_to_datarequests_gdrive

path_to_datarequest_gdrive <- paste0(path_to_datarequests_gdrive, "INBO_bosdat/")

```


# Ophalen gegevens

**Bosdat-data**       
Opgemeten in kader van de beheerplanning

- dendro (A3A4):     
    - OPNAME_TABEL_T4: omtrekklasse + 1 à 3 hoogtes + totaal aantal bomen in deze omtrekklasse; enkel levende bomen (R = 9 of 18m afh. van omtrek)
    - OPNAME_TABEL_T7: omtrek + hoogte - per boom; levend/dood (geen hoogtes bij dode bomen) (R = 9 of 18m afh. van omtrek)
    - Beide types metingen komen zowel in volopnames als proefvlakopnames voor  
    
    
- verjonging (A1A2):
    - OPNAME_TABEL_T1: verjonging met Ht < 2 m : proefcirkel A1: aantal per hoogteklasse, enkel levend (R = 2,25)
    - OPNAME_TABEL_T2: Verjonging met Ht >= 2 m en C(1,5) < 20cm: proefcirkel A2: aantal of bedekking%, aantal dood, bedekking% dood, (totale bedekking steeds 0) (R = 4.5)      
    _**ik veronderstel dat "aantal" enkel de levende betreft, want soms "aantal dood" > "aantal"**_
    
  
- hakhout (A2-cirkel):    
    - OPNAME_TABEL_T6: proefcirkel A2: C(1,5) >= 20cm: omtrek & levend/dood, geen hoogtes (R = 4.5)      
    
    
- staand dood hout:
    - OPNAME_TABEL_T5: staand dood hout: aantal per omtrekklasse, geen hoogtes (R = 9 of 18m afh. van omtrek)     
    _**ter aanvulling bij T4, opname per omtrekklasse**_      


```{r check, eval = FALSE}
t <- OPNAME_TABEL_T2_ %>% 
  filter(AANTAL_DOOD > AANTAL)
# 8% van alle records => "aantal" enkel de levende, want soms "aantal dood" > "aantal"

t <- OPNAME_TABEL_T5_ %>% 
  filter(HC_BOS_OPNAME_STR %in% OPNAME_TABEL_T7_$HC_BOS_OPNAME_STR)
nrow(t)
t <- OPNAME_TABEL_T5_ %>% 
  filter(HC_BOS_OPNAME_STR %in% OPNAME_TABEL_T4_$HC_BOS_OPNAME_STR)
nrow(t)
# T5 komt enkel voor bij T4 (per omtrekklasse), anders opgenomen in T7 (op boomniveau)
```



## Specifiek bos

```{r paths}
path_to_bosdat <- "C:/05_DHcurves/04_gegevensopkuis_dataverzameling/Bosdat/"
# (staat ook op mijn gdrive: G:\Mijn Drive\80_DHcurves_backup_20230328\04_gegevensopkuis_dataverzameling\Bosdat)
name_db <- "bosdat6.03.accdb"
name_forest <- "mortagne"
bosdomein_bosdat <- c("Mortagnebos")

# automatisc
db <- paste0(path_to_bosdat, name_db)

output_path_gdrive  <- paste0(path_to_datarequest_gdrive, name_forest, "/")

```

**Brondata**: `r db`        

**Output wordt weggeschreven naar**: `r output_path_gdrive`


```{r data_accdb}
con <- odbcConnectAccess2007(db)

overview_tbls <- sqlTables(con) %>% filter(TABLE_TYPE != "SYSTEM TABLE")

# bosbestand - ligging
BOSBESTAND <- sqlFetch(con, "BOSBESTAND")
BOS_BOSPERCEEL <- sqlFetch(con, "BOS_BOSPERCEEL")
BOS_BOSPLAATS <- sqlFetch(con, "BOS_BOSPLAATS")
BOS_BOSDOMEIN <- sqlFetch(con, "BOS_BOSDOMEIN")

# bosbestand - info (lftd, menging, ...)
BOS_BOSBESTAND <- sqlFetch(con, "BOS_BOSBESTAND")

# bosopname - welk proefvlak
OPNAME_TABEL <- sqlFetch(con, "OPNAME_TABEL")
BOS_OPNAME <- sqlFetch(con, "BOS_OPNAME")

# bosopname - metingen
OPNAME_TABEL_T7 <- sqlFetch(con, "OPNAME_TABEL_T7") # Hooghout: proefcirkel A3 + proefcirkel A4 (elke boom dbh en hoogte)
OPNAME_TABEL_T4 <- sqlFetch(con, "OPNAME_TABEL_T4") # Bomen met C(1,5)>=20 cm (A3A4: aantal per omtrekklasse + 3 hoogtes)
OPNAME_TABEL_T1 <- sqlFetch(con, "OPNAME_TABEL_T1") # Verjonging met Ht < 2 m : proefcirkel A1 (aantal per hoogteklasse, enkel levend, geen bedekking (leeg)) (R = 2,25)
OPNAME_TABEL_T2 <- sqlFetch(con, "OPNAME_TABEL_T2") # Verjonging met Ht >= 2 m en C(1,5) < 20cm: proefcirkel A2 (aantal of bedekking%, aantal dood, bedekking% dood, (totale bedekking steeds 0)) (R = 4.5)
OPNAME_TABEL_T5 <- sqlFetch(con, "OPNAME_TABEL_T5") # Staand dood hout (aantal per omtrekklasse)
OPNAME_TABEL_T6 <- sqlFetch(con, "OPNAME_TABEL_T6") # Hakhout: proefcirkel A2: loten met C(1,5) >= 20cm (omtrek & levend/dood)
OPNAME_TABEL_T3 <- sqlFetch(con, "OPNAME_TABEL_T3") # Hakhout: loten met C(1,5) >= 20 cm (leeg; aantal levend en dood per omtrekklasse)

# lookuplijsten
qBOS_BMS <- sqlFetch(con, "BOS_BMS")
qBOS_BOOMGROEP <- sqlFetch(con, "BOS_BOOMGROEP")
qOMTREKKLASSE <- sqlFetch(con, "OMTREKKLASSE") # enkel T4
qBOS_OPNAME_TYPE <- sqlFetch(con, "BOS_OPNAME_TYPE")

qBOS_PROEFCIRKELTYPE <- sqlFetch(con, "BOS_PROEFCIRKELTYPE") # Volopname, Proefvlakopname, Dreef / Randzone
qBOS_TYPE <- sqlFetch(con, "BOS_TYPE")

qBOS_LEEFTIJD <- sqlFetch(con, "BOS_LEEFTIJD")
qBOS_MENGINGSVORM <- sqlFetch(con, "BOS_MENGINGSVORM")
qBOS_SLUITINGSGRAAD <- sqlFetch(con, "BOS_SLUITINGSGRAAD")
qBOS_BESTANDSTYPE <- sqlFetch(con, "BOS_BESTANDSTYPE")
qBOS_BESTAND_LEEFTIJDSKLASSE <- sqlFetch(con, "BOS_BESTAND_LEEFTIJDSKLASSE")
qBOS_BEDRIJFSVORM <- sqlFetch(con, "BOS_BEDRIJFSVORM")

# mbt verjonging
qOPNAME_VERJONGINGSWIJZE <- sqlFetch(con, "OPNAME_VERJONGINGSWIJZE")
qOPNAME_VERSPREIDING <- sqlFetch(con, "OPNAME_VERSPREIDING")
qOPNAME_HOOGTEKLASSE <- sqlFetch(con, "OPNAME_HOOGTEKLASSE")
qOPNAME_ONTSTAAN <- sqlFetch(con, "OPNAME_ONTSTAAN")

# berekende waardes - vaak leeg
totalen <- sqlFetch(con, "BOSBESTAND_PARAMETERS_BMS")

odbcClose(con)

```


# Tabellen

- **OPNAME_TABEL_T1 tem T7**: opnames zelf (hoogtes, dbh, aantallen, ...)
  - T4 en T7 bevatten dendro > 7 cm dbh    
  - T1 en T2 bevatten verjonging
  - T5 bevat dood hout    
  - T6 bevat hakhout      
<br>

- **OPNAME_TABEL**: overzicht van alle opnames (T1 tem T7) met link naar de opnames 
in de opnametabellen via `OPNAME_TABEL_ID`; info over T1/../T7 in `TAB_NAAM`   

- **BOS_OPNAME**: link tss de opname en BOSBESTAND (`BOSBESTAND_ID`) en zo 
verder naar perceel, bosplaats en domein

- **BOSBESTAND**: bevat veld `HC_BOSBESTAND_STR` (bv. gemeentebos bertem_bosreservaat_1_a),
maar ook link naar perceel (`BOS_BOSPERCEEL_ID`) en zo verder naar bosplaats en domein

- **BOS_BOSBESTAND**: bevat alle info over het bestand zoals lftd, bestandstype, bedrijfsvorm, 
mengingsvorm, sluitingsgraad, bedekking avk, liggend dood hout


```{r algemeen, results = 'hide'}
str(OPNAME_TABEL)
table(OPNAME_TABEL$OMSCH, OPNAME_TABEL$TAB_NAAM)

str(BOSBESTAND)
head(BOSBESTAND$HC_BOSBESTAND_STR)

str(BOS_BOSPERCEEL)
str(BOS_BOSPLAATS)
str(BOS_BOSDOMEIN)

```

```{r bestand, results = 'hide'}
# info over bosbestand
str(BOS_BOSBESTAND)

# lookup
str(qBOS_LEEFTIJD)
str(qBOS_MENGINGSVORM)
str(qBOS_SLUITINGSGRAAD)
str(qBOS_BESTANDSTYPE)
str(qBOS_BESTAND_LEEFTIJDSKLASSE)
str(qBOS_BEDRIJFSVORM)
```


```{r opname, results = 'hide'}
# info over de opname zelf
str(BOS_OPNAME)
head(BOS_OPNAME$HC_BOS_OPNAME_STR)

# lookup
str(qBOS_OPNAME_TYPE)
# unique(qBOS_OPNAME_TYPE$OMSCH)
str(qBOS_PROEFCIRKELTYPE)
```

```{r T4T7, results = 'hide'}
str(OPNAME_TABEL_T4)
str(OPNAME_TABEL_T7)

# lookup
str(qBOS_BMS)
str(qBOS_BOOMGROEP)
str(qOMTREKKLASSE)
```

```{r T1T2, results = 'hide'}
str(OPNAME_TABEL_T1)
str(OPNAME_TABEL_T2)

# lookup
str(qBOS_BMS)
str(qOPNAME_VERJONGINGSWIJZE)
str(qOPNAME_VERSPREIDING)
str(qOPNAME_HOOGTEKLASSE)
str(qOPNAME_ONTSTAAN) 
```


```{r T5_dood, results = 'hide'}
str(OPNAME_TABEL_T5)

# lookup
str(qBOS_BMS)
str(qOMTREKKLASSE)

```

```{r T6_HH, results = 'hide'}
str(OPNAME_TABEL_T6)

# lookup
str(qBOS_BMS)

```

# Relaties

**bosbestand - locatie**      
BOSBESTAND (met `HC_BOSBESTAND_STR`) ---> BOS_BOSPERCEEL ---> BOS_BOSPLAATS ---> BOS_BOSDOMEIN      

(`HC_BOSBESTAND_STR`: bv. gemeentebos bertem_bosreservaat_1_a)


**opnames**     
OPNAME_TABEL_T7 ---> OPNAME_TABEL ---> BOS_OPNAME (met info over de opname) ---> BOSBESTAND ---> BOS_BOSBESTAND       

**bosbestand - info**       
BOS_BOSBESTAND (met `HC_BOSBESTAND_STR` en info over het bosbestand) ---> BOSBESTAND      


## ID's omvormen

Vroeger gebruikte ik "b_",  zdd bij openen in excel of access de id's als string 
weergegeven worden (numeric wordt soms afgerond of als e-10 weergegeven wat join bemoeilijkt).      

Nu toch "as.character", lijkt me goed genoeg.

```{r list_ID_as_char}
# Namen van alle dataframes waarvan ik ID as character wil opslaan
dataframe_names <- c("OPNAME_TABEL_T7", "OPNAME_TABEL_T4", "OPNAME_TABEL", 
                     "OPNAME_TABEL_T1", "OPNAME_TABEL_T2", "OPNAME_TABEL_T5",
                     "OPNAME_TABEL_T6",
                     "BOS_BOSBESTAND", "BOS_BOSDOMEIN", "BOS_BOSPERCEEL", "BOS_BOSPLAATS",
                     "BOS_OPNAME", "BOSBESTAND",
                     "qBOS_BMS", "qBOS_BOOMGROEP", 
                     "qBOS_OPNAME_TYPE", "qBOS_PROEFCIRKELTYPE",
                     "qOMTREKKLASSE", "qBOS_TYPE", "qBOS_LEEFTIJD", 
                     "qBOS_MENGINGSVORM", "qBOS_SLUITINGSGRAAD", "qBOS_BESTANDSTYPE", 
                     "qBOS_BESTAND_LEEFTIJDSKLASSE", "qBOS_BEDRIJFSVORM",
                     "qBOS_BMS", "qOPNAME_VERJONGINGSWIJZE", "qOPNAME_VERSPREIDING",
                     "qOPNAME_HOOGTEKLASSE", "qOPNAME_ONTSTAAN") 

# Creëer een lijst van dataframes uit de namen
list_dataframes <- mget(dataframe_names)  
# dan direct naam van df in list (<--> list("OPNAME_TABEL_T7", "OPNAME_TABEL"))
```

```{r ID_as_char, results='hide'}
# functie
convert_ID_to_char <- function(dataframe){
  # Zoek naar velden met "_ID" in de naam en zet ze om naar karakter
  cols_to_convert <- names(dataframe)[grep("_ID", names(dataframe))]
  # Converteer naar karakter
  dataframe[cols_to_convert] <- lapply(dataframe[cols_to_convert], as.character)
  dataframe
}

# for-loop om de functie op elk dataframe toe te passen en de dataframes te overschrijven
for (df_naam in dataframe_names) {
  list_dataframes[[df_naam]] <- convert_ID_to_char(list_dataframes[[df_naam]])
}

# overschrijf de oorspronkelijke dataframes met de gewijzigde dataframes
for (df_naam in dataframe_names) {
  assign(df_naam, list_dataframes[[df_naam]])
}

# check
str(OPNAME_TABEL)
str(OPNAME_TABEL_T7)
str(OPNAME_TABEL_T1)
```


# Resulterende tabellen    

Welke tabellen wil ik?

- **OPNAME_TABEL_T7_** en **OPNAME_TABEL_T4_**: tabellen met metingen A3A4 en boomsoort 
en link naar de opname
- **OPNAME_TABEL_T1_** en **OPNAME_TABEL_T2_**: tabellen met metingen A1A2 en boomsoort 
en link naar de opname
- **OPNAME_TABEL_T5_** : tabel met metingen dood hout en boomsoort en link naar de opname
- **OPNAME_TABEL_T6_** : tabel met metingen hakhout en boomsoort en link naar de opname


- **OPNAME_TABEL_**: een tabel met alle beschikbare opnames en bijhorende info 
(omschrijving van type opname, datum, proefvlaknr, opp. A1234...); deze info moet immers niet bij elke 
individuele boommeting toegevoegd worden        
!! hier ook info over bostype (hooghout, hakhout, middelhout), dit wordt niet 
op bestandsniveau genoteerd


- **BOSBESTAND_**: een tabel met alle info over het bosbestand: naam (nr), 
bestandstype, lftd, menging, sluiting, ....       
!! info over bostype wordt op proefvlakniveau bepaald (hakhout, hooghout, ...)


```{r T7}
# str(BOS_OPNAME)
print("OPNAME_TABEL_T7_")

OPNAME_TABEL_T7_ <- OPNAME_TABEL_T7 %>% 
  left_join(qBOS_BMS %>% select(BOS_BMS_ID, BMS = OMSCH)
            , by = c("BMS_ID" = "BOS_BMS_ID")) %>% 
  left_join(OPNAME_TABEL %>% rename(OPNAME = OMSCH)) %>% 
  left_join(BOS_OPNAME %>% select(BOS_OPNAME_ID, BOSBESTAND_ID, 
                                  PROEFVLAKNUMMER, OPNAME_IDENTIFICATIE,
                                  DATUM_OPNAME, HC_BOS_OPNAME_STR)) %>% 
  left_join(BOSBESTAND %>% select(BOSBESTAND_ID, HC_BOSBESTAND_STR))

str(OPNAME_TABEL_T7_)

```



```{r T4}
# str(BOS_OPNAME)
print("OPNAME_TABEL_T4_")

OPNAME_TABEL_T4_ <- OPNAME_TABEL_T4 %>% 
  left_join(qBOS_BMS %>% select(BOS_BMS_ID, BMS = OMSCH)
            , by = c("BMS_ID" = "BOS_BMS_ID")) %>% 
  left_join(qOMTREKKLASSE %>% select(OMTREKKLASSE_ID, OMTREKKLASSE)) %>% 
  left_join(OPNAME_TABEL %>% rename(OPNAME = OMSCH)) %>% 
  left_join(BOS_OPNAME %>% select(BOS_OPNAME_ID, BOSBESTAND_ID, 
                                  PROEFVLAKNUMMER, OPNAME_IDENTIFICATIE,
                                  DATUM_OPNAME, HC_BOS_OPNAME_STR)) %>% 
  left_join(BOSBESTAND %>% select(BOSBESTAND_ID, HC_BOSBESTAND_STR))

str(OPNAME_TABEL_T4_)

```

```{r T5}
# str(BOS_OPNAME)
print("OPNAME_TABEL_T5_")

OPNAME_TABEL_T5_ <- OPNAME_TABEL_T5 %>% 
  left_join(qBOS_BMS %>% select(BOS_BMS_ID, BMS = OMSCH)
            , by = c("BMS_ID" = "BOS_BMS_ID")) %>% 
  left_join(qOMTREKKLASSE %>% select(OMTREKKLASSE_ID, OMTREKKLASSE)) %>% 
  left_join(OPNAME_TABEL %>% rename(OPNAME = OMSCH)) %>% 
  left_join(BOS_OPNAME %>% select(BOS_OPNAME_ID, BOSBESTAND_ID, 
                                  PROEFVLAKNUMMER, OPNAME_IDENTIFICATIE,
                                  DATUM_OPNAME, HC_BOS_OPNAME_STR)) %>% 
  left_join(BOSBESTAND %>% select(BOSBESTAND_ID, HC_BOSBESTAND_STR))

str(OPNAME_TABEL_T5_)

```


```{r T6}
# str(BOS_OPNAME)
print("OPNAME_TABEL_T6_")

OPNAME_TABEL_T6_ <- OPNAME_TABEL_T6 %>% 
  left_join(qBOS_BMS %>% select(BOS_BMS_ID, BMS = OMSCH)
            , by = c("BMS_ID" = "BOS_BMS_ID")) %>% 
  left_join(OPNAME_TABEL %>% rename(OPNAME = OMSCH)) %>% 
  left_join(BOS_OPNAME %>% select(BOS_OPNAME_ID, BOSBESTAND_ID, 
                                  PROEFVLAKNUMMER, OPNAME_IDENTIFICATIE,
                                  DATUM_OPNAME, HC_BOS_OPNAME_STR)) %>% 
  left_join(BOSBESTAND %>% select(BOSBESTAND_ID, HC_BOSBESTAND_STR))

str(OPNAME_TABEL_T6_)

```

```{r T1}
# str(BOS_OPNAME)
print("OPNAME_TABEL_T1_")

OPNAME_TABEL_T1_ <- OPNAME_TABEL_T1 %>% 
  left_join(qBOS_BMS %>% select(BOS_BMS_ID, BMS = OMSCH)
            , by = c("BMS_ID" = "BOS_BMS_ID")) %>% 
  left_join(qOPNAME_HOOGTEKLASSE %>% select(OPNAME_HOOGTEKLASSE_ID, HOOGTEKLASSE = OMSCH)
            , by = c("OPNAME_HOOGTEKLASSE_ID")) %>% 
  left_join(qOPNAME_VERSPREIDING %>% select(OPNAME_VERSPREIDING_ID, VERSPREIDING = OMSCH)
            , by = c("OPNAME_VERSPREIDING_ID")) %>% 
  left_join(qOPNAME_VERJONGINGSWIJZE %>% select(OPNAME_VERJONGINGSWIJZE_ID, VERJONGINGSWIJZE = OMSCH)
            , by = c("OPNAME_VERJONGINGSWIJZE_ID")) %>% 
  left_join(qOPNAME_ONTSTAAN %>% select(OPNAME_ONTSTAAN_ID, ONTSTAAN = OMSCH)
            , by = c("OPNAME_ONTSTAAN_ID")) %>% 
  left_join(OPNAME_TABEL %>% rename(OPNAME = OMSCH)) %>% 
  left_join(BOS_OPNAME %>% select(BOS_OPNAME_ID, BOSBESTAND_ID, 
                                  PROEFVLAKNUMMER, OPNAME_IDENTIFICATIE,
                                  DATUM_OPNAME, HC_BOS_OPNAME_STR)) %>% 
  left_join(BOSBESTAND %>% select(BOSBESTAND_ID, HC_BOSBESTAND_STR))

str(OPNAME_TABEL_T1_)

```


```{r T2}
# str(BOS_OPNAME)
print("OPNAME_TABEL_T2_")

OPNAME_TABEL_T2_ <- OPNAME_TABEL_T2 %>% 
  left_join(qBOS_BMS %>% select(BOS_BMS_ID, BMS = OMSCH)
            , by = c("BMS_ID" = "BOS_BMS_ID")) %>% 
  left_join(qOPNAME_HOOGTEKLASSE %>% select(OPNAME_HOOGTEKLASSE_ID, HOOGTEKLASSE = OMSCH)
            , by = c("OPNAME_HOOGTEKLASSE_ID")) %>% 
  left_join(qOPNAME_VERSPREIDING %>% select(OPNAME_VERSPREIDING_ID, VERSPREIDING = OMSCH)
            , by = c("OPNAME_VERSPREIDING_ID")) %>% 
  left_join(qOPNAME_VERJONGINGSWIJZE %>% select(OPNAME_VERJONGINGSWIJZE_ID, VERJONGINGSWIJZE = OMSCH)
            , by = c("OPNAME_VERJONGINGSWIJZE_ID")) %>% 
  left_join(qOPNAME_ONTSTAAN %>% select(OPNAME_ONTSTAAN_ID, ONTSTAAN = OMSCH)
            , by = c("OPNAME_ONTSTAAN_ID")) %>% 
  left_join(OPNAME_TABEL %>% rename(OPNAME = OMSCH)) %>% 
  left_join(BOS_OPNAME %>% select(BOS_OPNAME_ID, BOSBESTAND_ID, 
                                  PROEFVLAKNUMMER, OPNAME_IDENTIFICATIE,
                                  DATUM_OPNAME, HC_BOS_OPNAME_STR)) %>% 
  left_join(BOSBESTAND %>% select(BOSBESTAND_ID, HC_BOSBESTAND_STR))

str(OPNAME_TABEL_T2_)

```


```{r opnames}
print("OPNAME_TABEL_")

OPNAME_TABEL_ <- OPNAME_TABEL %>% 
  left_join(BOS_OPNAME) %>% 
  left_join(qBOS_OPNAME_TYPE %>% rename(OPNAME_TYPE = OMSCH)) %>% 
  left_join(qBOS_PROEFCIRKELTYPE %>% rename(PROEFCIRKEL_TYPE = OMSCH)) %>% 
  left_join(qBOS_TYPE %>% rename(BOS_TYPE = OMSCH)) %>% 
  left_join(BOSBESTAND %>% select(BOSBESTAND_ID, HC_BOSBESTAND_STR)) %>% 
  select(OPNAME_TABEL_ID, BOS_OPNAME_ID, BOSBESTAND_ID, 
         TAB_NAAM, TAB_OMSCH = OMSCH, 
         PROEFVLAKNUMMER, OPNAME_IDENTIFICATIE,
         DATUM_OPNAME, OPNAME_TYPE, PROEFCIRKEL_TYPE,
         OPP_A1, OPP_A2, OPP_A3, OPP_A4, OPP_VOL, Opp_Representatief,
         BOS_TYPE,
         POPULIER_X, POPULIER_Y, # ?betekenis
         HC_BOSBESTAND_STR, HC_BOS_OPNAME_STR)

# verschil BOS_OPNAME_TYPE_ID, OPNAME_TYPE_ID: lijkt zelfde te zijn (toch voor Bertembos)
# BOS_OPNAME_TYPE_ID: in tabel qBOSTYPE
# OPNAME_TYPE_ID: in tabel BOS_OPNAME

str(OPNAME_TABEL_)
# names(OPNAME_TABEL_)


```

```{r bosbestand}
print("BOSBESTAND_")

BOSBESTAND_ <- BOS_BOSBESTAND %>% 
  left_join(BOSBESTAND %>% select(contains("_ID"), HC_BOSBESTAND_STR, BESTAND = OMSCH)) %>% 
  left_join(BOS_BOSPERCEEL %>% select(contains("_ID"), PERCEEL = NR)) %>% 
  left_join(BOS_BOSPLAATS %>% select(contains("_ID"), BOSPLAATS = OMSCH)) %>% 
  left_join(BOS_BOSDOMEIN %>% select(contains("_ID"), BOSDOMEIN = OMSCH)) %>% 
  # bestandstype
  left_join(qBOS_BESTANDSTYPE %>% select(BOS_BESTANDSTYPE_ID, BESTANDSTYPE = OMSCH)
            , by = c("BESTANDSTYPE_HOOFD_ID" = "BOS_BESTANDSTYPE_ID")) %>% 
  left_join(qBOS_BESTANDSTYPE %>% select(BOS_BESTANDSTYPE_ID, BESTANDSTYPE_SUB = OMSCH)
            , by = c("BESTANDSTYPE_SUB_ID" = "BOS_BESTANDSTYPE_ID")) %>% 
  left_join(qBOS_BESTANDSTYPE %>% select(BOS_BESTANDSTYPE_ID, BESTANDSTYPE_SUBSUB = OMSCH)
            , by = c("BESTANDSTYPE_SUBSUB_ID" = "BOS_BESTANDSTYPE_ID")) %>% 
  #leeftijd
  left_join(qBOS_LEEFTIJD %>% select(BOS_LEEFTIJD_ID, LEEFTIJD = OMSCH)
            , by = c("BOS_BEH_LEEFTIJD_ID" = "BOS_LEEFTIJD_ID")) %>% 
  left_join(qBOS_BESTAND_LEEFTIJDSKLASSE %>% select(leeftijdsklasse_id, leeftijdsklasse_BE = leeftijdsklasse)
            , by = c("LEEFTIJDSKLASSE_BE_ID" = "leeftijdsklasse_id")) %>% 
  left_join(qBOS_BESTAND_LEEFTIJDSKLASSE %>% select(leeftijdsklasse_id, leeftijdsklasse_NE = leeftijdsklasse)
            , by = c("LEEFTIJDSKLASSE_NE_ID" = "leeftijdsklasse_id")) %>% 
  #bedrijfsvorm, menging, sluiting
  left_join(qBOS_BEDRIJFSVORM %>% select(BOS_BEDRIJFSVORM_ID, BEDRIJFSVORM = OMSCH)) %>% 
  left_join(qBOS_MENGINGSVORM %>% select(BOS_MENGINGSVORM_ID, MENGING = OMSCH)) %>% 
  left_join(qBOS_SLUITINGSGRAAD %>% select(BOS_SLUITINGSGRAAD_ID, SLUITING = OMSCH)) %>% 
  select(BOS_BOS_BESTAND_ID, BOSBESTAND_ID, HC_BOSBESTAND_STR, DATUM_OPNAME, 
         BESTANDSTYPE, BESTANDSTYPE_SUB, BESTANDSTYPE_SUBSUB, 
         LEEFTIJD, leeftijdsklasse_BE, leeftijdsklasse_NE,
         PLANTJAAR, NE_PLANTJAAR,
         BE_LEEFTIJD, NE_LEEFTIJD, 
         BEDRIJFSVORM, MENGING, SLUITING,
         BED_AMER_Z, BED_AMER_S, BED_AMER_B, 
         LIG_DOOD_DUN, LIG_DOOD_DIK, LIG_DOOD_ZD,
         OPMERKINGEN, 
         BESTAND, PERCEEL, BOSPLAATS, BOSDOMEIN
  )

str(BOSBESTAND_)
# names(BOSBESTAND_)

```


Dreef wordt soms vermeld als perceel (ipv een perceelsnr) en hangt dan vaak samen met 
een volopname (zie `OPNAME_TYPE`), OF wordt als afzonderlijk `OPNAME_TYPE` vermeld ("Dreef/Randzone")

Bosreservaat staat bij `BOSPLAATS` vermeld

# Export

```{r list, results='hide'}
str(BOSBESTAND_)
str(OPNAME_TABEL_)
str(OPNAME_TABEL_T4_)
str(OPNAME_TABEL_T7_)

export_names <- c("OPNAME_TABEL_T7_", "OPNAME_TABEL_T4_"
                  , "OPNAME_TABEL_T1_", "OPNAME_TABEL_T2_"
                  , "OPNAME_TABEL_T5_", "OPNAME_TABEL_T6_"
                  , "OPNAME_TABEL_", "BOSBESTAND_")

# Creëer een lijst van dataframes uit de namen
list_export <- mget(export_names)  

```

```{r list_enkel_specifiek_bos, results='hide'}
names(BOSBESTAND_)
names(OPNAME_TABEL_)
names(OPNAME_TABEL_T4_)
names(OPNAME_TABEL_T7_)

# BOSBESTAND_zonien <- BOSBESTAND_ %>% 
#   filter(str_detect(HC_BOSBESTAND_STR, "Zoni"))
# 
# t <- BOSBESTAND_ %>% 
#   filter(str_detect(HC_BOSBESTAND_STR, "Zoni"))

# list_export_zonien <- map(list_export, function(x) x %>% filter(str_detect(HC_BOSBESTAND_STR, "Zoni")))
list_export_selection <- map(list_export, function(x) x %>% filter(str_detect(HC_BOSBESTAND_STR, bosdomein_bosdat)))

```

Rechtstreeks naar gdrive

```{r list_export_gdrive, eval = FALSE}
# stats
for (i in 1:length(list_export_selection)){
	  write.csv2(list_export_selection[[i]], 
	             paste0(output_path_gdrive, names(list_export_selection)[i], ".csv")) 
}

# xlsx
# write.xlsx(info_BR, paste0(path_to_datarequest_gdrive, "BR_niveau/", "site_info.xlsx"))

```

Eénmalig ook alles


```{r list_export_gdrive_alles, eval = FALSE}
name_forest_all <- "all"
output_path_gdrive_all  <- paste0(path_to_datarequest_gdrive, name_forest_all, "/")

# stats
for (i in 1:length(list_export)){
	  write.csv2(list_export[[i]], 
	             paste0(output_path_gdrive_all, names(list_export)[i], ".csv")) 
}

# xlsx
# write.xlsx(info_BR, paste0(path_to_datarequest_gdrive, "BR_niveau/", "site_info.xlsx"))

```


# Copy html to teamdrive

Na knitting - manueel

```{r eval = FALSE}
# Specify the path of the file you want to copy
source_file <- here::here(paste0("Scripts/AanvraagGegevens/INBO_bosdat/INBO_oude_bosdat_bevragen_", name_forest, ".html"))

# Specify the destination directory where you want to copy the file
destination_dir <- output_path_gdrive

# Copy the file to the destination directory
file.copy(from = source_file, to = destination_dir, overwrite = TRUE)
```

