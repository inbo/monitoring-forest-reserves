...
```{r TMP_load_data_OLD, results='hide', eval = FALSE}
# oudere versie
path_to_datarequest_teamdrive <- paste0(path_to_project_teamdrive, "/output_R/versie_feb2025/")


# 
design2 <- read.table(paste0(path_to_datarequest_teamdrive, "INBO_Design.txt")
                           , sep = " ", dec = ".")

plots2_ <- read.table(paste0(path_to_datarequest_teamdrive, "INBO_Plots.txt")
                           , sep = " ", dec = ".")
# only A4-circle + CA (no A3)
plots2 <-  plots2_ %>% filter(CIRCLE_NO == 1 | is.na(CIRCLE_NO))
plots_LPI2 <- plots2_ %>% filter(is.na(CIRCLE_NO))
plots_A3_2 <-  plots2_ %>% filter(CIRCLE_NO == 2)
plots_A4_2 <-  plots2_ %>% filter(CIRCLE_NO == 1)

standing2 <- read.table(paste0(path_to_datarequest_teamdrive, "INBO_Standing.txt")
                           , sep = " ", dec = ".")

lying2 <- read.table(paste0(path_to_datarequest_teamdrive, "INBO_Lying.txt")
                    , sep = " ", dec = ".")
```

```{r TMP_verschil, eval = FALSE}
design_diff <- design %>% 
  anti_join(design2)
...
```{r CONTROLE_SNAGS_ELISABETH, eval = FALSE}
# checks nalv niet intacte d
names(standing)

table(standing$INVENTORY_YEAR)

standing_intact <- standing %>% 
  filter(INTEGRITY == "C") %>% 
  mutate(verh_D_H = DBH_mm * 3.1416/(HEIGHT * 10)
         , verh_H_D = 100*HEIGHT*10/DBH_mm)    #copilot

range(standing_intact$verh_D_H, na.rm = T)
# best tussen 1.5 en 15
# 1.163556 15.824356
standing_intact %>% nrow() #108773
standing_intact %>% filter(verh_D_H > 1.5) %>% nrow()  #30
standing_intact %>% filter(verh_D_H > 14 & DBH_mm < 1000) %>% nrow()  #7
# de bomen met lageverhouding, zijn heel smal en hoog, dus zeker geen stumps
# en tot een verhouding 15 kan in theorie nog een intacte boom zijn

range(standing_intact$verh_H_D, na.rm = T)
# [1]  19.85294 270.00000
# Als h/d < 20–25 → grote kans dat boom afgebroken is (snag). # copilot
# => de intacte kunnen gerust intact zijn
standing_intact %>% filter(verh_H_D < 20) %>% nrow()  #1

standing_fragment <- standing %>% 
  filter(INTEGRITY == "F") %>% 
  mutate(verh_D_H = DBH_mm * 3.1416/(HEIGHT * 10))
range(standing_fragment$verh_D_H) 
# [1]   1.9712 236.6672
standing_fragment %>% nrow() #3374
standing_fragment %>% filter(verh_D_H < 15) %>% nrow()  #2025
standing_fragment %>% filter(verh_D_H > 15) %>% nrow()  #1349

summary(standing_fragment$verh_D_H)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #  1.971   8.796  12.566  16.850  19.897 236.667


summary(standing_intact$verh_D_H)
  # Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
  # 1.164   2.827   3.896   4.374   5.459  15.824   91758
```

```{r height_meas_vgl_modeled}
standing_intact <- standing_intact %>% 
  mutate(height_diff = HEIGHT_model - HEIGHT
         , strange_height = ifelse(height_diff > 0.5*HEIGHT_model, T, F)
         , perc_H_model = 100*height_diff/HEIGHT_model)


t <- standing_intact %>% 
  filter(perc_H_model > 50) %>% 
  select(SITE_NAME, contains("ID"), DBH_mm, contains("HEIGHT"), LIFE
         , DECAY_NUMERIC, TREE_SPECIES_FULL_SCIENTIFIC
         , verh_D_H, height_diff, strange_height, perc_H_model
         , INVENTORY_YEAR)
# 90
table(t$LIFE)
#  A  D 
