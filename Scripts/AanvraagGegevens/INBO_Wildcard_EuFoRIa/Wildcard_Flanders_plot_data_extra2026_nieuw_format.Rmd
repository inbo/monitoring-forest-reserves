...
## Aanpassingen aan gevraagd formaat
...
con <- odbcConnectAccess2007(path_to_strata_db)
  ...
  link_plotid_wildcard <- sqlFetch(con, "link_plotid_wildcard", stringsAsFactors = FALSE) 
odbcClose(con)
...
subarea_sevendonck <- link_plotid_wildcard %>% 
  filter(forest_reserve_fieldmap == "Sevendonck") %>% 
  select(plot_id, forest_reserve_fieldmap, SITE_NAME, subarea = WILDCARD_sub_ID)
```
...
plotdata0 <- plotinfo %>%
  ...
  mutate(forest_reserve_FM_adjusted = 
           ifelse(!is.na(subarea) & subarea == "Alnion", "Sevendonck A",
                  ifelse(!is.na(subarea) & subarea == "Quercion", "Sevendonck Q",
                         forest_reserve))
         ) %>% 
  select(-subarea)
...
design_ <- plotdata %>%
  ...
  # left_join(link_WC_FM, by = c("forest_reserve", plottype))
...
new_data <- design_def_link %>% 
  anti_join(old_designdata, by = c("SITE_ID", "SITE_NAME", "WILDCARD_sub_ID"
                                   , "composed_site_id", "inventory_id"))
table(new_data$SITE_NAME)
```
