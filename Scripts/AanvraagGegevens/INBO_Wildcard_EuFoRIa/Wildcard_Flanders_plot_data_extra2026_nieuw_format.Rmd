...
path_to_datarequest_teamdrive <- paste0(path_to_project_teamdrive, "/output_R/extra_data_2026_nieuw_format/")
...
standing_lying <- rbind(standing, lying)
...
```
```{r link_LYING_STANDING, results='hide'}
names(link_WC_FM)
names(standing_lying)

standing_lying_link <- standing_lying %>% 
  left_join(list_BR %>% select(forest_reserve_FM_adjusted, plottype
                               , period, INVENTORY_YEAR)) %>% 
  left_join(link_WC_FM %>% select(-comment, -subarea, -`LPI_ID / SPI_ID`)
            , by = c("forest_reserve_FM_adjusted", "plottype")) %>% 
...
nrow(standing_lying_link) == nrow(standing_lying) 
```
...
standing_lying_link %>% filter_at(col, any_vars(is.na(.))) %>% nrow() == 0
...
# 3
names(standing_link)
standing_link_new <- standing_link %>% 
  inner_join(new_data)
names(standing_link_new) == names(standing_link)
...
new_vegdata <- ground_veg_link %>% 
  anti_join(old_vegdata, by = c("SITE_ID", "SITE_NAME", "WILDCARD_sub_ID"
                                , "INVENTORY_YEAR"))
table(new_vegdata$SITE_NAME)
with <- old_vegdata %>% filter(str_detect(SITE_NAME, "With"))
heirni <- old_vegdata %>% filter(str_detect(SITE_NAME, "Heirn"))


# ? - Withoefse heide: data van 2023
# - Heirnisse: data van 2024

new_treedata <- standing_link %>% 
  anti_join(old_treedata, by = c("SITE_ID", "SITE_NAME", "WILDCARD_sub_ID"
                                , "INVENTORY_YEAR"))
table(new_treedata$SITE_NAME)
with <- old_treedata %>% filter(str_detect(SITE_NAME, "With"))
heirni <- old_treedata %>% filter(str_detect(SITE_NAME, "Heirn"))
```
...
write.table(standing_link_new %>% select(-all_of(extra_vars), -tree_measure_id, -composed_site_id)
