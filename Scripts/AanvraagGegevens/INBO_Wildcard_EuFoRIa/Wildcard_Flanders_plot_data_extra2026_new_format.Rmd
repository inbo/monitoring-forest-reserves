...
design_geom <- read.table(file.path(path_to_project_teamdrive, "plots_geom")
                         , header = TRUE
                           , sep = ",", dec = ".") %>% 
  unique()

waldo::compare(design_geom, plots_geom)
# ✔ No differences
```
...
nrow(list_plots) == 2059
nrow(list_plots) == 1848 # 1848-1717=131

plotinfo %>% anti_join(list_plots) %>% nrow() == 174 / 167
...
design_ <- plotdata %>%
  ...
  mutate(institute = "INBO"
         ...
         , geom = NA  # over te nemen uit reeds opgeladen data (< shapefile) 
         , other = "\\N"
         ) 
...
design_check <- read.table(paste0(path_to_datarequest_teamdrive
                                  , "UPLOAD_INBO_DESIGN_2026-04-20.txt")
                           , sep = " ", dec = ".")
head(design_check)
names(design_check)
...
plots_check <- read.table(paste0(path_to_datarequest_teamdrive
                                 , "UPLOAD_INBO_PLOTS_2026-04-20.txt")
                           , sep = " ", dec = ".")
head(plots_check)
names(plots_check)
...
#
trees_check <- read.table(paste0(path_to_datarequest_teamdrive
                                 , "UPLOAD_INBO_TREES_2026-04-20.txt")
                           , sep = " ", dec = ".")
...
ground_veg_check <- read.table(paste0(path_to_datarequest_teamdrive
                                      , "INBO_ground_veg_2026-04-20.txt")
                           , sep = " ", dec = ".")
