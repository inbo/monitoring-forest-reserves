...
plotdata0 <- plotinfo %>%
  ...
  anti_join(poplar_kluisbos %>% select(plot_id)) %>% 
...
plotdata_def <- plotdata %>%
  mutate(geom = NA   # over te nemen uit reeds opgeladen data (< shapefile)
...
lying <- lying0 %>%
  left_join(qSpecies %>% select(-name_nl, -names_sc), by = c("species" = "ID")) %>% 
  left_join(qDecaystage, by = c("decaystage" = "ID")) %>% 
...
design_check <- read.table(paste0(path_to_datarequest_teamdrive
                                  , "UPLOAD_INBO_DESIGN_2026-04-09.txt")
                           , sep = " ", dec = ".")
head(design_check)
names(design_check)
...
plots_check <- read.table(paste0(path_to_datarequest_teamdrive
                                 , "UPLOAD_INBO_PLOTS_2026-04-09.txt")
                           , sep = " ", dec = ".")
head(plots_check)
names(plots_check)
...
ground_veg_check <- read.table(paste0(path_to_datarequest_teamdrive
                                      , "INBO_ground_veg_2026-04-09.txt")
                           , sep = " ", dec = ".")
