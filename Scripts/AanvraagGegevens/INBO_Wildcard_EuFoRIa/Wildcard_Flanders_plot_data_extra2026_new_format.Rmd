...
CP_tresholds <- tresholds_ %>% 
  ...
  mutate(CIRCLE_NO = ifelse(name == "r_A4", 1, 
         ...
         , STANDING_DEAD_THRESHOLD =  ifelse(name == "r_A4", dbh_min_a4_dead, 
                            ifelse(name == "r_A4", dbh_min_a3_dead, NA))
...
# !! 1323 plot_id 11000 zonder id bv. oldid 3900 uit 2000
...
lying <- lying0 %>%
  ...
  mutate(INVENTORY_TYPE = ifelse(plottype == "CA", "LPI", 
         ...
         , x_local = NA, y_local = NA, HOM_m = NA, HEIGHT_model = NA
  ...
  select(INVENTORY_TYPE
         ...
         , LENGTH
         , DECAY_NUMERIC, DECAY_TEXT
         , x_local, y_local, HOM_m, HEIGHT_model
         , VOLUME
         , VITALITY
         , all_of(extra_vars)
...
design_def_link <- design_def %>% 
  ...
  select(INSTITUTE, composed_site_id, inventory_id, everything())
...
#
summary(trees_link)
trees_link %>% filter(is.na(VOLUME)) %>% nrow() == 0
...
Het gaat enkel om Terrijst en Coolhem 3de decade.
...
new_vegdata <- ground_veg_link %>% 
  anti_join(old_vegdata, by = c("composed_site_id", "INVENTORY_YEAR"))
...
new_treedata <- trees_link %>% 
  anti_join(old_treedata, by = c("composed_site_id", "INVENTORY_YEAR"))
...
write.table(plotdata_def_link_new %>% select(-all_of(extra_vars), -composed_site_id)
            , paste0(path_to_datarequest_teamdrive, "INBO_Plots.txt")
            , sep = " ", dec = ".")

write.table(trees_link_new %>% select(-all_of(extra_vars), -tree_measure_id, -composed_site_id)
            , paste0(path_to_datarequest_teamdrive, "INBO_Trees.txt")
            , sep = " ", dec = ".")
# lookuplists
...
# write.table(lying_link_new %>% select(-all_of(extra_vars), -composed_site_id)
...
write.table(ground_veg_link_new %>% select(-all_of(extra_vars), -composed_site_id)
...
write_csv2(standing_link_new %>% select(-all_of(extra_vars), -tree_measure_id)
...
write_csv2(lying_link_new %>% select(-all_of(extra_vars))
...
#
trees_check <- read.table(paste0(path_to_datarequest_teamdrive, "INBO_Trees.txt")
                           , sep = " ", dec = ".")
