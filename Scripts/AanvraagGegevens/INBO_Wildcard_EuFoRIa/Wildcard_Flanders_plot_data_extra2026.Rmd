...
path_to_datarequest_old <- paste0(path_to_project_teamdrive
                                  , "/output_R/versie_gecorrigeerde_consistent_id/")
...
plotdata_def <- plotdata %>%
  left_join(link_wildcard_fieldmap
            , by = c("forest_reserve" = "forest_reserve_fieldmap", "plottype")) %>% 
...
t <- list_plots %>% filter(str_detect(forest_reserve_FM_adjusted, "Sevendonck"))
list_plots %>% filter(is.na(forest_reserve_FM_adjusted)) %>% nrow() == 0
list_plots %>% filter(forest_reserve_FM_adjusted == "Everzwijnbad" & period == 3)
...
list_BR %>% filter(str_detect(forest_reserve_FM_adjusted,"Sevendonck"))
...
**!! OPGEPAST**: doordat we hoger de bomen met dbh < 30 cm verwijdered hebben uit 
...
```{r eval = FALSE}
# With heide en Heirnisse al aangeleverd - gecheckt 30/3/2026
# (toch in ...\EUFORIA_WILDCARD_2024\output_R\versie_gecorrigeerde_consistent_id)
