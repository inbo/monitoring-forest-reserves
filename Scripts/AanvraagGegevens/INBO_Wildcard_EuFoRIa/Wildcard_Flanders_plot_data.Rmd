...
path_to_datarequest_teamdrive <- paste0(path_to_project_teamdrive, "/output_R/versie_gecorrigeerde_consistent_id/")
...
# deze databank bevat nog geen "processed" data van Withoefse heide periode 3
```
...
# Deviations "Zonienwoud-Joseph Zwaenepoel reservaat - OldCore" 

## LPI

- standing trees (L+D) in LPI-plot 11000 measured 
in 1986 and 2000 have a treshold of 30 cm DBH (instead of 5 cm)

- standing trees (L+D) measured in LPI-plot 11000 in 1986 only contains trees still standing in 2000
<!-- (dat blijkt nu toch niet zo te zijn, maar we laten het zo voor wildcard) -->
**=> we don't include the inventory of 1986**

- treshold standing trees in 2000 was 30 cm DBH, but there were also two regeneration groups with treshold 5 cm DBH
...
- volume of lying deadwood per decaystage/diameter class is unknown for circular plots 
(SPI_ID: 2001-2065), 1st survey (2000)
...
poplar_kluisbos <- info_survey %>% 
  ...
  filter(forest_reserve_fieldmap == "Kluisbos" & MaxBasalAreaSpeciesName == "Populier species") %>% 
  select(plot_id)
...
plotdata <- plotdata %>% 
  mutate(CONSISTENT_ID_STANDING = 
           ifelse(forest_reserve_fieldmap %in% c("Zoniën UITBR_1995") & 
                    # forest_reserve_fieldmap %in% c("Zoniën UITBR_1995", "Zoniën KERN_1983") &     # WAS FOUT
...
plotdata_def %>% group_by(forest_reserve_FM_adjusted, plottype, LPI_ID) %>% 
  ...
  ungroup() %>% 
  dplyr::select(forest_reserve_fieldmap = forest_reserve_FM_adjusted # Sevendonck A en Q
