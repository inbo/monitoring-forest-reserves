...
plots <- read.table(paste0(path_to_datarequest_teamdrive, "UPLOAD_INBO_PLOTS_2026-05-07.txt")
                           , sep = "\t", dec = ".")
names(plots)

plots_LPI <- plots %>% filter(circle_no == "\\N")
plots_A3 <-  plots %>% filter(circle_no == 2)
plots_A4 <-  plots %>% filter(circle_no == 1)
...
7/9/2026: Overall summary per site, inventory type, **circle_no (A3 vs A4)** and inventory year.
standing and lying apart
...
```{r}
# blijkbaar soms bij lying en LPI circle_no  = 1
lying_ <- lying |> 
  mutate(circle_no = ifelse(inventory_type == "LPI", "\\N", circle_no))
...
nrow(plots_LPI) + nrow(plots_A3) + nrow(plots_A4) == nrow(plots)
```
...
all <- standing %>% 
  inner_join(plots, by = c("composed_site_id", "inventory_id", "inventory_type"
...
qry_standing_per_plot <- standing_all %>% 
  ...
  dplyr::group_by(composed_site_id, inventory_id, inventory_type, plot_id
                  , circle_no, inventory_year) %>% 
  dplyr::summarize(
    STEM_DENSITY = sum(stemnr_ha)
    , BASAL_AREA = sum(basal_area_ha)) %>% 
  ungroup()
# per A3, A4 circle of LPI => 3 queries per reservaat
...
names(plots)
names(qry_standing_per_plot) # 181/331
...
Calculate mean per site, inventory type, circle_no and inventory year
...
Overall summary of diameters per site, inventory type, circle_no (A3 vs A4) and inventory year.

```{r}
qry_standing_diam <- standing %>% 
  group_by(composed_site_id, inventory_id, inventory_type
           , circle_no, inventory_year) %>% 
...
qry_lying_per_plot <- lying %>% 
  ...
  dplyr::group_by(composed_site_id, inventory_id, inventory_type
                  , inventory_year, circle_no, plot_id) %>% 
  dplyr::summarize(
    STEM_DENSITY = sum(stemnr_ha)
...
write_csv2(qry_lying_per_site %>% select(-n_plots)
            , paste0(path_to_datarequest_teamdrive, "csv/INBO_LYING_QUERY", Sys.Date(), ".csv"))

write_csv2(qry_standing_per_site %>% select(-n_plots)
            , paste0(path_to_datarequest_teamdrive, "csv/INBO_STANDING_QUERY", Sys.Date(), ".csv"))
```
...
#
qry_lying_check <- read.table(paste0(path_to_datarequest_teamdrive, "INBO_LYING_QUERY_2026-09-07.txt")
                           , sep = "\t", dec = ".")
head(qry_lying_check)
names(qry_lying_check)

#
qry_standing_check <- read.table(paste0(path_to_datarequest_teamdrive, "INBO_STANDING_QUERY_2026-09-07.txt")
                           , sep = "\t", dec = ".")
