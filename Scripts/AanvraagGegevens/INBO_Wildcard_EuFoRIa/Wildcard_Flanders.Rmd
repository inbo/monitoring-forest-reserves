...
```{r info_processed, results = 'hide', eval = FALSE}
...
Twee bosreservaten nog niet (helemaal) processed: één kernvlaktes (Kerss) en 
...
**Tijdelijk nog geen link leggen - eventueel aan Yannick overlaten**
...
```{r link_WC_FM}
link_WC_FM <- read_xlsx(paste0(path_to_project_teamdrive, "/LINK_WILDCARD_fieldmap_Yannick.xlsx"))
names(link_WC_FM)
...
plotdata0 <- info_survey %>%
  ...
  # left_join(link_WC_FM, by = c("forest_reserve" = "forest_reserve_fieldmap", plottype))
...
# lang formaat maken van CP_radii
CP_radii2 <- CP_radii %>% 
  pivot_longer(cols = c("rA3_m", "rA4_m")) %>% 
  mutate(CIRCLE_NO = ifelse(name == "rA4_m", 1, 2),
         plottype = "CP") %>% 
...
check_rA3A4gelijk <- CP_radii2 %>% 
  group_by(forest_reserve, CIRCLE_RADIUS) %>% 
  summarize(n= n()) %>% 
  ungroup() %>% 
  filter(n > 1)
# Kerss wegens andere rA3 doorheen de tijd
# Liedekerke en Withoefse heide effectief rA3 = rA4
CP_radii3 <- CP_radii2 %>% 
  filter(!(forest_reserve %in% c("Liedekerke", "Withoefse heide") & CIRCLE_NO == 2))

# enkel aan de CP's koppelen
CP_radii4 <- info_survey %>% 
  filter(plottype == "CP") %>% 
  select(forest_reserve, period, plot_id) %>%    #1680
  full_join(CP_radii3)

```


```{r combine_radii, results = 'hide'}
names(plotdata0)
names(CP_radii3)
# groeperen per plot-id (geen)
plotdata <- plotdata0 %>% 
  left_join(CP_radii4, by = c("forest_reserve", "period", "plottype", "plot_id"))

# check
nrow(plotdata) == nrow(CP_radii4) + nrow(plotdata0 %>% filter(plottype == "CA"))
```
...
plotdata <- plotdata %>% 
  mutate(CONSISTENT_ID_STANDING = 
           ifelse(forest_reserve == "Kersselaerspleyn" & period == 2 & plottype == "CP"
                  , NA
...
plotdata_def %>% filter(plot_id == 2001)
...
plotdata_def %>% group_by(forest_reserve = forest_reserve_fieldmap, plottype, LPI_ID) %>% 
  summarize(n_surveys = n_distinct(INVENTORY_YEAR)
            , min_year = min(INVENTORY_YEAR)
            , max_year = max(INVENTORY_YEAR)) %>% 
  ungroup() %>% 
  dplyr::select(forest_reserve, plottype, n_surveys, min_year, max_year) %>%  
...
list_BR <- plotdata_def %>% 
  select(plot_id, period, forest_reserve_fieldmap, plottype
         , X, Y, Z, LPI_ID, SPI_ID, INVENTORY_YEAR, SITE_ID
...
standing0 <- data_stems_calc %>% 
  inner_join(list_BR, by = c("plot_id", "period", "plottype"))

t <- standing0 %>% group_by(forest_reserve, plottype, year) %>% 
...
extra_vars <- c("forest_reserve_fieldmap", "plot_id", "plottype", "period")
```
...
standing <- standing0 %>%
  ...
  select(SITE_ID, LPI_ID,	SPI_ID,	TREE_ID,	STEM_ID,	INVENTORY_YEAR,	CONSISTENT_ID
...
t <- lying0 %>% group_by(forest_reserve, plottype, year) %>% 
  summarise(n_trees = n(), n_plots = n_distinct(plot_id)) %>% ungroup()
...
names(lying0)
unique(list_BR$CONSISTENT_ID_LYING)
names(qSpecies)
names(qDecaystage)
...
lying <- lying0 %>%
  ...
  select(SITE_ID, LPI_ID,	SPI_ID,	TREE_ID,	STEM_ID, PIECE_ID
...
data_LIS <- data_LIS_ %>% 
  mutate(period = 3) %>% 
  inner_join(list_BR %>% select(plot_id, forest_reserve_fieldmap, period))
...
cwd0 <- cwd0 %>% 
  inner_join(list_BR, by = c("plot_id", "period"))
...
cwd <- cwd0 %>%
  ...
  mutate(TREE_SPECIES_FULL_SCIENTIFIC = name_sc
         ...
         , DECAY_NUMERIC = ifelse(decaystage == 0, NA, decaystage)
         , DECAY_TEXT = decaystage_txt
         , VOLUME = volume_m3_m2 * SAMPLED_AREA
         ) %>% 
  select(SITE_ID, SPI_ID
         , INVENTORY_YEAR
         , TREE_SPECIES_FULL_SCIENTIFIC, TREE_SPECIES_CODE
         , DECAY_NUMERIC, DECAY_TEXT
         , VOLUME
...
design_ <- plotdata_def %>%
  group_by(SITE_ID, SITE_NAME, WILDCARD_sub_ID, INVENTORY_YEAR
...
write.table(design_def %>% select(-all_of(extra_vars))
