...
Tijdelijk verwijzen naar fieldmapdb van 13/6/2024, want daar is alles initieel op berekend
...
CONSISTENT_ID_STANDING/LYING: ook aanpassen voor plot_id 11000 (oude kernvlakte Kerss):
survey 1 (1986) niet meegenomen => survey 2 moet ook nog NA zijn 
=> "CONSISTENT_ID_STANDING": "NA" voor 2000
=> "CONSISTENT_ID_LYING": "NA" voor 2000

> indien gevraagd kunnen we nog verder gaan en "CONSISTENT_ID_LYING" ook "NA" 
zetten voor 2010, want dat is de eerste keer dat deadwood opgemeten werd in de KV
...
- consistent_id wordt op pplotniveau bepaald:
...
- in Zoniënwoud, 1stExtension zijn er soms NA trees in 2de periode (2010): dat zijn bomen in plots die 
dan maar de eerste keer opgemeten werden. Bij de eerste opname werden de cirkelplots gelegen in de 
kernvlakte niet afzonderlijk opgemeten.
...
Het coödinatenstelsel wordt immers op plotniveau ingesteld.
...
In eerste instantie nkel vegetatie
...
data_vegetation %>% filter(is.na(total_herb_cover_id)) %>% nrow() == 201 # vooral plots 1eSET
...
data_herblayer %>% filter(is.na(coverage_id)) %>% nrow() # 150
...
data_herblayer %>% filter(is.na(year)) %>% nrow()  # 108
...
table(t$plottype)
# enkel geen jaar beschikbaar bij core area's?
table(t$forest_reserve)
table(t$period)
```
Het lijkt alsof bij twee kernvlaktes (periode 1) de `vegetation_date` ontbreekt, 
...
```
Daarnaast ook 28 cirkelplots waar geen veg-opname gebeurd is.
Ook deze moeten `PLOT_SAMPLED = "N"` krijgen.
...
ground_veg <- data_herblayer %>% 
  ...
  select(LPI_ID, SPI_ID, LPI_sub_ID
         , INVENTORY_YEAR = year  # zelfde als jaartal van date_veg
         , PLOT_SAMPLED, SAMPLED_AREA
...
nrow(ground_veg_link %>% filter(is.na(SITE_ID))) == nrow(ground_veg_link %>% filter(forest_reserve_fieldmap_new == "De heide"))

nrow(ground_veg_link %>% filter(is.na(SITE_ID)))  == 0
ground_veg_link <- ground_veg_link %>% 
  filter(!is.na(SITE_ID))
```
...
ground_veg_link %>% filter(PLOT_SAMPLED == "Y") %>% filter_at(col, any_vars(is.na(.)))
...
```{r error_coord_cfr_tomas, eval = FALSE}
# local coordinates terug afleiden van XY
         # , X = X_centre + x_local
         # , Y = Y_centre + y_local

XY_local <- standing_check %>% 
  left_join(plots_check %>% 
              select(contains(c("NAME", "ID", "YEAR")), X_centre = X, Y_centre = Y) %>% 
              unique()) %>%  # ! 2 cirkesl A3 en A4
  mutate(X_local = X - X_centre,
         Y_local = Y - Y_centre)

XY_local_CP <- XY_local %>% 
  filter(!is.na(SPI_ID))

range(XY_local_CP$X_local)
range(XY_local_CP$Y_local)
```

```{r eval = FALSE}
min(plots_check$X); max(plots_check$X)
min(plots_check$Y); max(plots_check$Y)

# all
min(standing_check$x_local); max(standing_check$x_local)
min(standing_check$y_local); max(standing_check$y_local)
# only CP's / SPI's
standing_check_CP <- standing_check %>% 
  filter(!is.na(SPI_ID))
min(standing_check_CP$x_local); max(standing_check_CP$x_local)
min(standing_check_CP$y_local); max(standing_check_CP$y_local)
