...
# Inladen lookuplijsten, plotinfo en strata
...
Hierna een overzicht van alle tabellen in forrresdat:
...
Vervolgens enkel de tabellen waar we in geïnteresseerd zijn inladen.
...
```{r datatables}
# plotinfo toevoegen aan de datatabellen xxx_by_plot 
# en enkel de CP's selecteren

# tabellen in een list stoppen
datatables <- list(dendro_by_plot = dendro_by_plot
                   , reg_by_plot = reg_by_plot)

tables <- map(datatables, function(x) x %>% 
...
```{r eval = FALSE}
# mbv read_forresdat_table() - COMPLEXER
# tablenames_input <- c("qiufroheight", "qalive_dead", "qdecaystage", "qspecies", "qind_shoot_cop")
# 
# tables <- map(tablenames_input, read_forresdat_table
#                    , join_plotinfo = FALSE
#                    # , plottype = "all"
#                    , git_ref_type = "branch"
#                    , git_reference = "develop")
# 
# tables <- map(tables, function(x) x %>% select(-Value3, -OrderField, -Active))
# 
# #The input was not named
# names(tables) <- tablenames_input
# 
...
**OPGEPAST**: sommige plots nog beheerd na start van monitoringnetwerk
...
management_types <- strata_remaining %>% 
  ...
  group_by(forest_reserve_fieldmap, SiteName, SubArea, Management, Management_type) %>%
  summarize(n_plots = n()) %>% 
  ungroup()

site_subtypes <- strata_remaining %>% 
  filter(plot_id < 10000) %>% 
  group_by(forest_reserve_fieldmap, SiteName, SubArea) %>% 
  summarize(n_plots = n()) %>% 
  ungroup()

# deze zonder naam forest_reserve zitten niet in plotinfo omdat data_processed = FALSE
# geen info over management type van kernvlaktes
```
> waarom geen 13 plots open area , zie verder 
...
> hier nog aantal plots zonder verjonging checken (15?)

```{r eval = FALSE}
reg_by_plot <- read_forresdat_table(tablename = "reg_by_plot"
                                           , join_plotinfo = FALSE
                                           , plottype = "CP"
                                           , git_ref_type = "branch"
                                           , git_reference = "develop")
...
open_ruimte <- dendro_by_plot %>% 
  ...
  filter(vol_dead_standing_m3_ha == 0 & vol_log_m3_ha == 0) %>%  # dan valt plot 475 weg => is bosplot, maar op dat moment zonder bomen/verjonging
...
bosplots <- dendro_by_plot %>% 
  left_join(plotinfo %>% select(plot_id, period, survey_trees, survey_deadw)) %>% 
  select(forest_reserve, plot_id, period, number_of_tree_species
         , survey_trees, survey_deadw) %>% 
  anti_join(open_ruimte)

# check
nrow(bosplots) + nrow(open_ruimte) ==  nrow(dendro_by_plot)
```


> tot hier maar gekomen
...
!! OPGEPAST
# (c) Anja: 18/11/2025: toch nog eens bekijken, want in help bij functie staat als vb om toch forest_reesrve bij gtoupunt vars te nemen
# Eens uitgetest bij With heide (wel maar één BR), en geen verschil in de stat-uitkomst
# alternatief cfr de info bij de functie: 
# "periode" bij comb_vars en "forest_reserve" als grouping_vars 
# dan krijg je bv.in periode 1 waar geen Berk voorkomt, in geen enkele plot), 
# toch in elke plot ook berk met aantal = 0
# als je dan statistics berekent, krijg je hetzelfde resultaat als bij dataset2_0
# (geen vermelding van Berk met waardes 0)
        # dataset2_0test <- add_zeros(dataset = dataset2 %>%
