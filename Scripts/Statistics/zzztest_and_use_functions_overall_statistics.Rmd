...
Vervangen door de functie `get_plotinfo_cp_for_stat()` die enerzijds
enkel cirkelplots selecteert en ook Kluisbos en Ename opsplitst in resp. SFR en 
communal forets, en open en bosplots.

Kluisbos wordt dus NIET opgesplitst in beheerd en onbeheerd, maar eerder BR versus
gemeentebos (waar ook 6 plots niet beheerd zijn).
...
dendro_by_plot %>% group_by(forest_reserve) %>% 
  summarize(n_plots = n_distinct(plot_id)) %>% 
  ungroup() %>% 
  filter(str_detect(forest_reserve, "Kluisbos|Ename"))
...
Er zijn immers enkel open plekken in Enamebos. Zie hoger
