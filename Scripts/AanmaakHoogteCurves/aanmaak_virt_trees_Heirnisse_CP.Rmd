...
forestreserve_folder <- "2_Heirnisse_DEELS"
...
- hoogtes van 1e, 2e en 3e SET samen (soms hoogtecurves obv 1e en 2e SET afzonderlijk)
...
Het gaat om volgende plots: **`r plots_to_exclude`**
...
overview_HH_height %>% DT :: datatable(options = list(dom = 'rt'), rownames = FALSE, filter = "top") 
...
```{r include=FALSE}
stems_height_pop <- stems_modeled_height %>% filter(name_nl == "Populier species")

stems_height_pop_p1 <- stems_height_pop %>% filter(period == 1)
stems_height_pop_p2 <- stems_height_pop %>% filter(period == 2)
stems_height_pop_p3 <- stems_height_pop %>% filter(period == 3)


mean(stems_height_pop_p1$height_m)
mean(stems_height_pop_p2$height_m)
mean(stems_height_pop_p3$height_m)
```


```{r vgl_pop}
ggplot(stems_height_pop, aes(dbh_mm, height_m)) + 
...
```{r vgl_pop_histo, eval = FALSE}
# hoogtes
h1 <- ggplot(stems_height_pop_p1, aes(height_m)) + geom_histogram()
h2 <- ggplot(stems_height_pop_p2, aes(height_m)) + geom_histogram()
h3 <- ggplot(stems_height_pop_p3, aes(height_m)) + geom_histogram()
...
Zit er verschil op de hoogtes van Corsicaanse den?

JA, het lijkt toch of er een verschuiving van ca 2.5 m gebeurd is over de 20 jaar tussen de
eerste en derde decade.

> eens voorleggen aan rest van het team

```{r include=FALSE}
stems_height_cors <- stems_modeled_height %>% filter(name_nl == "Corsicaanse den")

stems_height_cors_p1 <- stems_height_cors %>% filter(period == 1)
stems_height_cors_p2 <- stems_height_cors %>% filter(period == 2)
stems_height_cors_p3 <- stems_height_cors %>% filter(period == 3)

mean(stems_height_cors_p1$height_m)
mean(stems_height_cors_p2$height_m)
mean(stems_height_cors_p3$height_m)

```


```{r vgl_lork}
ggplot(stems_height_cors, aes(dbh_mm, height_m)) + 
  geom_point(aes(color = fperiod)) + 
  geom_smooth(aes(y = height_per_period, group = fperiod, color = fperiod))  
...
source_file <- here::here(paste0("Scripts/AanmaakHoogteCurves/aanmaak_virt_trees_Heirnisse_CP.html"))
