...
Meer metingen in 2003, te weinig in 2013 (maar 10 metingen) => beide gebruiken
...
Niet veel verschil, 2e SET zeker nodig, anders extrapolatie: 3 dikkere opgemeten.
...
Periode 2 werden dikkere populieren opgemeten, zelfde max. hoogte.


## Gewone esdoorn

Enkel opgemeten 1e SET.
...
In 2eSET maar heel weinig esdoorn opgemeten => verschil in meean.
...
## Wilg smalbladige species

```{r include=FALSE}
stems_height_wilg <- stems_modeled_height %>% filter(name_nl == "Wilg smalbladige species")

stems_height_wilg_p1 <- stems_height_wilg %>% filter(period == 1)
stems_height_wilg_p2 <- stems_height_wilg %>% filter(period == 2)
stems_height_wilg_p3 <- stems_height_wilg %>% filter(period == 3)

mean(stems_height_wilg_p1$height_m, na.rm = T) # 19
mean(stems_height_wilg_p2$height_m, na.rm = T) # 23
mean(stems_height_wilg_p3$height_m, na.rm = T) 
```


```{r vgl_wilg}
ggplot(stems_height_wilg, aes(dbh_mm, height_m)) + 
  geom_point(aes(color = fperiod)) + 
  geom_smooth(aes(y = height_per_period, group = fperiod, color = fperiod))  
...
```{r include=FALSE}
stems_height_els <- stems_modeled_height %>% filter(name_nl == "Witte els (grauwe)")

stems_height_els_p1 <- stems_height_els %>% filter(period == 1)
stems_height_els_p2 <- stems_height_els %>% filter(period == 2)
stems_height_els_p3 <- stems_height_els %>% filter(period == 3)


mean(stems_height_els_p1$height_m, na.rm = T) # 19
mean(stems_height_els_p2$height_m, na.rm = T) # 23
mean(stems_height_els_p3$height_m, na.rm = T) 
```


```{r vgl_els}
ggplot(stems_height_els, aes(dbh_mm, height_m)) + 
  geom_point(aes(color = fperiod)) + 
  geom_smooth(aes(y = height_per_period, group = fperiod, color = fperiod))  
...
source_file <- here::here(paste0("Scripts/AanmaakHoogteCurves/aanmaak_virt_trees_Heirnisse_KV.html"))
