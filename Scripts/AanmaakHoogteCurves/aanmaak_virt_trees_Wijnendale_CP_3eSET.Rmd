...
# resultaten
path_to_datarequest <- paste0(forestreserve_folder, "/2AanmaakDHcurves_3eSET/VirtualTreesHeightCurves")
...
Dat zou met een bijwerkquery kunne ngebeuren in de db of vanuit R. Nu  mbv een bijwerkquery in access (qbw_processed).
...
maar toch ook enkele hakhutspillen gemeten
...
# Periode 1 en/of periode 3 gebruiken?

Geen metingen in periode2
...
periode <- c("periode 1", "periode 3")
n_bomen <- c(n1, n3)
n_hoogtes <- c(n1_h, n3_h)
...
- 1eSET: data 1eSET
- 2eSET: data 1eSET  (immers geen hoogtes gemeten in 2de decade)
- 3eSET: data 1e + 3eSET

MAAR best ook eens bekijken voor afzonderlijke boomsoorten. 
...
De extra metingen waren zeker noodzakelijk, want voor bepaalde soorten waren geen hoogtes beschikbaar van de zwaardere diameters: bv esdoorn, zomereik, berk, ...
...
De resultaten worden weggeschreven naar **TblVirtueleTrees_CP_1e3eSET.xlsx**

```{r WegschrijvenOutput}
write_xlsx(output, paste0(path_to_datarequest, "/TblVirtueleTrees_CP_1e3eSET.xlsx"))
