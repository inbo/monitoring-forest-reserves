...
JA. 
Berk en lijsterbes toch veel als hakhout.

Voor 't simpelste standaard meenemen tenzij als afwijkend => zie onderstaande grafieken.
...
write_csv2(overview_heights_KV,paste0(forestreserve_folder, "/Overzichten/overview_heights_KV_Q.csv"))
...
write.csv2(overview_HH_height, paste0(forestreserve_folder, "/Overzichten/overview_HH_height_KV_Q.csv"))
```
...
# per soort
p1 <- ggplot(stems_height  %>% filter(species %in% c(87, 39, 86, 87, 63, 57)), aes(dbh_mm, height_m)) + geom_point(aes(color = ind_sht_cop)) + geom_smooth() + facet_wrap(~species)
p1

```

```{r vgl_individ_coppice_curve2, eval = FALSE}
ggplot(stems_height %>% filter(species %in% c(87, 39, 86, 87, 63, 57)), aes(dbh_mm, height_m)) + geom_point(aes(color = ind_sht_cop)) + geom_smooth(aes(color = ind_sht_cop)) 

ggplot(stems_height %>% filter(species %in% c(87, 39, 86, 87, 63, 57)), aes(dbh_mm, height_m)) + geom_point() + geom_smooth() + facet_wrap(~ind_sht_cop)

ggplot(stems_height %>% filter(species %in% c(87, 39, 86, 87, 63, 57)), aes(dbh_mm, height_m)) + geom_point(aes(color = ind_sht_cop)) + geom_smooth() 
...
p2 <- ggplot(stems_height %>% filter(species %in% c(87, 39, 86, 87, 63, 57) & ind_sht_cop == 10), aes(dbh_mm, height_m)) + geom_point(aes(color = ind_sht_cop)) + geom_smooth() + facet_wrap(~species)
p2
...
De eiken zijn gemiddeld `r mean(stems_height_eik_p2$height_m) - mean(stems_height_eik_p1$height_m)` m gegroeid.
<!-- 3.7 -->
...
```{r vgl_eik_1_vs_2_histogram}
# hoogtes
h1 <- ggplot(stems_height_eik_p1, aes(height_m)) + geom_histogram()
h2 <- ggplot(stems_height_eik_p2, aes(height_m)) + geom_histogram()
grid.arrange(h1, h2)

# diameters
d1 <- ggplot(trees %>% filter(species == 87 & period == 1), aes(dbh_mm)) + geom_histogram()
d2 <- ggplot(trees %>% filter(species == 87 & period == 2), aes(dbh_mm)) + geom_histogram()
...
Mean_height verschilt `r mean(stems_height_es_p2$height_m) - mean(stems_height_es_p1$height_m)` m.
...
write_xlsx(output, paste0(path_to_datarequest, "/TblVirtueleTrees_KV_Q_1e2eSET.xlsx"))
