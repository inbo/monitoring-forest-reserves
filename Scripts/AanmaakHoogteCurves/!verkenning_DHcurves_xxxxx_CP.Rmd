...
forestreserve_folder <- paste0(path_to_reserves, "Jansheide_DEELS")
...
```{r}
nr_measured <- stems %>% 
  mutate(height_m = ifelse(common_remark == 90 | common_remark_shoots == 90, NA, height_m)  # uitgebroken top
        ) %>% 
  # filter(alive_dead == 11 & intact_snag == 11) %>% 
  group_by(species) %>% 
  summarize(n = n(),
