...
Omdat er te weinig metingen zijn van de eerste en tweede decade, laden we ook de 
**hoogtes van het transect** (plot 61000) in (opgemeten in juni 2003).
Bij de dataverwerking van 2022 gebruikten we immers enkel deze hoogtes. 
Het is beter om een menging te hebben van de verschillende periodes.
...
trees <- load_data_dendrometry(database = path_to_fieldmap_db, extra_variables = TRUE) %>% 
  # filter(forest_reserve == forestreserve & !plot_id %in% plots_to_exclude) %>% 
  filter(forest_reserve == forestreserve |
           forest_reserve == "Withoefse heide T") %>% 
...
write.csv2(nr_measured, paste0(path_to_datarequest, "/nr_measured_CP_T.csv"))
...
write_csv2(overview_heights_CP, paste0(path_to_datarequest, "/overview_heights_CP_T.csv"))
```
...
write.csv2(overview_HH_height, paste0(path_to_datarequest, "/overview_HH_height_CP_T.csv"))
```
...
Immers geen metingen in tweede decade en deze van eerste decade komen vooral uit het transect.
(de vorige dh-curves werden aangemaakt op basis van metingen uit het transect).
...
write_xlsx(output, paste0(path_to_datarequest, "/TblVirtueleTrees_CP_T_1e2e3eSET.xlsx"))
