...
specieslist_coppice <- c(64, 16, 89, 83, 39, 86)
...
specieslist_coppice <- c(64, 16, 89)
...
Meer metingen in 2003, te weinig in 2012 => beide gebruiken
...
mean(stems_height_beuk_p1$height_m) # 22
mean(stems_height_beuk_p2$height_m) # 25
...
mean(stems_height_esd_p1$height_m, na.rm = T) # 13.7
mean(stems_height_esd_p2$height_m, na.rm = T) # 15.6
...
NEE: mean_height verschilt `r mean(stems_height_eik_p1$height_m) - mean(stems_height_eik_p2$height_m)` m.
<!-- 0.36 -->
...
Periode 2 misschien ietsje lager?? 

! Gecheckt en de twee hoogste bomen uit 1eSET, staan er in 2eSET ook nog en zijn wat lager => wellicht overschatting => verderop bij output niet meenemen.
...
> Twee hoogste hoogtes van Zomereik 1eSET worden verwijderd
...
output <- stems %>% 
  ...
  mutate(height_m = ifelse((is.na(common_remark_shoots) | common_remark_shoots != 90)
                           ...
                           , NA)) %>%
         # 90: uitgebroken top
  mutate(height_m = ifelse(species == 87 & period == 1 & height_m > 32, NA, height_m)) %>% 
  # géén hoogtes van zomereik boven de 31 m - 1eSET meenemen         
