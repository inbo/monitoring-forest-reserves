...
```{r Setup2}
forestreserve <- "Liedekerke Transekt"
# (Sevendonck Q in ander script)

forestreserve_folder <- paste0(path_to_reserves, "4_Liedekerke_TR")
...
# resultaten
path_to_datarequest <- paste0(forestreserve_folder, "/VirtualTreesHeightCurves")
...
JA. Anders geen metingen van tKa en ook Ruwe berk is half/half individueel en hakhout.
...
write_csv2(overview_heights_KV,paste0(forestreserve_folder, "/Overzichten/overview_heights_KV_A.csv"))
...
write.csv2(overview_HH_height, paste0(forestreserve_folder, "/Overzichten/overview_HH_height_KV_A.csv"))
```
...
specieslist_coppice <- c(57, 83, 63, 87, 3, 89, 86)
...
Meer metingen in 2007, maar ook nog vrij veel in 2017 => beide gebruiken
...
Lijkt er wel op dat er een evolutie is, maar geen optie om op te splitsen.
Immers onvoldoende metingen in periode 2. 
...
# Wat met Amerikaanse eik?


```{r vgl_els_1_vs_2_mean, include=FALSE}
stems_height_aEik <- stems_height %>% filter(name_nl == "Amerikaanse eik")

stems_height_aEik_p1 <- stems_height_aEik %>% filter(period == 1)
stems_height_aEik_p2 <- stems_height_aEik %>% filter(period == 2)

mean(stems_height_aEik_p1$height_m, na.rm = T) # 26.4
mean(stems_height_aEik_p2$height_m, na.rm = T) # 28.4

```

Mean_height verschilt `r mean(stems_height_aEik_p1$height_m) - mean(stems_height_aEik_p2$height_m)` m.

Amerikaanse eik is gemiddeld 2 à 2.5 m gegroeid.


(periode 1 : `r mean(stems_height_aEik_p1$height_m)` m - 
periode 2: `r mean(stems_height_aEik_p2$height_m)` m )

## Histogram

```{r vgl_aEik_1_vs_2_histogram}
# hoogtes
h1 <- ggplot(stems_height_aEik_p1, aes(height_m)) + geom_histogram()
h2 <- ggplot(stems_height_aEik_p2, aes(height_m)) + geom_histogram()
grid.arrange(h1, h2)

# diameters
d1 <- ggplot(trees %>% filter(species == 3 & period == 1), aes(dbh_mm)) + geom_histogram()
d2 <- ggplot(trees %>% filter(species == 3 & period == 2), aes(dbh_mm)) + geom_histogram()
...
```{r vgl_aEik_1_vs_2_curve}
ggplot(stems_height_aEik, aes(dbh_mm, height_m)) + geom_point(aes(color = fperiod)) + geom_smooth(aes(color = fperiod)) 

# ggplot(stems_height_aEik, aes(dbh_mm, height_m)) + geom_point() + geom_smooth() + facet_wrap(~fperiod)

ggplot(stems_height_aEik, aes(dbh_mm, height_m)) + geom_point(aes(color = fperiod)) + geom_smooth()

```

Indicatie dat aEik doorgeschoven is, maar onvoldoende metigen om twee afzonderlij kecurves aan te maken


# Wat met Zachte berk?

Zit er verschil op de hoogtes van zachte berk?

```{r vgl_berk_1_vs_2_mean, include=FALSE}
stems_height_berk <- stems_height %>% filter(name_nl == "Zachte berk")
...
JA: mean_height verschilt `r mean(stems_height_berk_p2$height_m) - mean(stems_height_berk_p1$height_m)` m.
De berken zijn gemiddeld 2.5 m gegroeid.

(periode 1 : `r mean(stems_height_berk_p1$height_m)` m - 
periode 2: `r mean(stems_height_berk_p2$height_m)` m )


## Histogram

```{r vgl_berk_1_vs_2_histogram}
# hoogtes
h1 <- ggplot(stems_height_berk_p1, aes(height_m)) + geom_histogram()
h2 <- ggplot(stems_height_berk_p2, aes(height_m)) + geom_histogram()
grid.arrange(h1, h2)

# diameters
d1 <- ggplot(trees %>% filter(species == 86 & period == 1), aes(dbh_mm)) + geom_histogram()
d2 <- ggplot(trees %>% filter(species == 86 & period == 2), aes(dbh_mm)) + geom_histogram()
grid.arrange(d1, d2, nrow = 1)
...
Te weinig metingen periode 1 om afzonderlijke curve op te stellen.

```{r vgl_berk_1_vs_2_curve}
ggplot(stems_height_berk, aes(dbh_mm, height_m)) + geom_point(aes(color = fperiod)) + geom_smooth(aes(color = fperiod)) 

ggplot(stems_height_berk, aes(dbh_mm, height_m)) + geom_point() + geom_smooth() + facet_wrap(~fperiod)

ggplot(stems_height_berk, aes(dbh_mm, height_m)) + geom_point(aes(color = fperiod)) + geom_smooth()
...
mean(stems_height_eik_p1$height_m, na.rm = T) # 20.1
mean(stems_height_eik_p2$height_m, na.rm = T) # 23.4
...
In onderstande grafiek is te zien dat dit vooral te wijten is aan het beperkt aantal metingen in 2e periode (slechts 4 metingen, zwaardere en dus hogere bomen)
...
Niet zoveel verschil tss de curves, wel al iets meer zwaardere (en dus hogere) eiken.
...
output <- stems %>% 
  ...
  mutate(height_m = ifelse((is.na(common_remark_shoots) | common_remark_shoots != 90)
                           ...
                           , NA)) %>%
         # 90: uitgebroken top
  # mutate(height_m = ifelse(ind_sht_cop == 10, height_m, NA)) %>% 
  # géén hoogtes van hakhout meenemen         
