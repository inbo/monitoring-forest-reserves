...
# resultaten
path_to_datarequest <- paste0(forestreserve_folder, "/VirtualTreesHeightCurves/Zwaenepoel_CP")
...
plotinfo_ <- plotinfo %>% filter(forest_reserve == forestreserve) %>% select(plot_id, contains("Treshold"))
...
trees <- load_data_dendrometry(database = path_to_fieldmap_db, 
  ...
  filter(forest_reserve == forestreserve & plottype == 20) 
...
Het betreft weliswaar maar twee metingen, maar daardoor wel een extra meting van wEls (8 hoogtes ipv 7).
...
write_csv2(overview_heights, paste0(forestreserve_folder, "/Overzichten/overview_heights_CP_Zwaenepoel.csv"))
```
...
write.csv2(overview_HH_height, paste0(forestreserve_folder, "/Overzichten/overview_HH_height_Zwaenep_CP.csv"))
```
...
De resultaten worden weggeschreven naar **TblVirtueletrees_CP_3eSET.xlsx**

```{r WegschrijvenOutput, eval=FALSE}
write_xlsx(output, paste0(path_to_datarequest, "/TblVirtueletrees_CP_3eSET.xlsx"))
