...
```
Aanmaak virtuele bomen voor kernvlakte van **`r forestreserve`**.
...
Eens datacontrole afgerond is, moet veld `processed` aangepast worden.

```{r check_processed}
plotinfo <- load_plotinfo(path_to_fieldmap_db, plottype = plot_type
                         , processed = FALSE) %>% 
...
plotinfo_ <- plots %>% filter(str_detect(forest_reserve, forestreserve) & plottype == "CA") 
...
write.csv2(nr_measured, paste0(path_to_datarequest, "/nr_measured_", subtype, ".csv"))
...
write_csv2(overview_heights_KV,paste0(path_to_datarequest, "/overview_heights_", subtype, ".csv"))
...
write.csv2(overview_HH_height, paste0(path_to_datarequest, "/overview_HH_height_", subtype, ".csv"))
...
Meer metingen in 2004, minder in 2014 en 2024 => alledrie gebruiken


## Grafiek

```{r vgl_all_1_vs_2_curve}
ggplot(stems_height, aes(dbh_mm, height_m)) + geom_point(aes(color = fperiod)) + 
...
Niet veel verschil, enkel bij de hogere diameters meer metingen uit 1ste survey.
Curves enkel obv 2de of 3de set zijn minder goed voor hogere diameters.
...
- 1eSET: data 1e + 2eSET + 3eSET
- 2eSET: data 1e + 2eSET + 3eSET
- 3eSET: data 1e + 2eSET + 3eSET
...
```{r include=FALSE}
stems_height_esd <- stems_modeled_height %>% filter(name_nl == "Zwarte els")
...
```{r include=FALSE}
stems_height_tka <- stems_modeled_height %>% filter(name_nl == "Zachte berk")

stems_height_tka_p1 <- stems_height_tka %>% filter(period == 1)
stems_height_tka_p2 <- stems_height_tka %>% filter(period == 2)
stems_height_tka_p3 <- stems_height_tka %>% filter(period == 3)

mean(stems_height_tka_p1$height_m, na.rm = T) # 22.8
mean(stems_height_tka_p2$height_m, na.rm = T) # 25
mean(stems_height_tka_p3$height_m)
```


```{r vgl_berk}
ggplot(stems_height_tka, aes(dbh_mm, height_m)) + 
  geom_point(aes(color = fperiod)) + 
  geom_smooth(aes(y = height_per_period, group = fperiod, color = fperiod))  
...
De resultaten worden weggeschreven naar **`r paste0("VirtualTreesHeightCurves/TblVirtueleTrees_", subtype, "_1e2e3eSET.xlsx")`**

```{r WegschrijvenOutput}
write_xlsx(output, paste0(path_to_datarequest, "VirtualTreesHeightCurves/TblVirtueleTrees_", subtype, "_1e2e3eSET.xlsx"))
```
...
source_file <- here::here(paste0("Scripts/AanmaakHoogteCurves/aanmaak_virt_trees_Coolhem_A_2026.html"))
source_file <- here::here(paste0("Scripts/AanmaakHoogteCurves/aanmaak_virt_trees_Coolhem_Q_2026.html"))
