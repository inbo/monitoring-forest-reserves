...
# dh-parameters
heightparam <- "HeightParams_Hannecart_KV.xlsx"

heightparam2 <- "HeightParams_Hannecart_KV2.xlsx"
heightparam3 <- "HeightParams_Hannecart_KV3.xlsx"
```
...
dh_parameters2 <- read_xlsx(paste0(path_to_height_models, "/", heightparam2)) %>% 
  rename(species = Species, forest_reserve = BR) %>% 
  filter(species != "<ALL>") %>% 
  mutate(species = as.integer(species),
         period = 2)
dh_parameters3 <- read_xlsx(paste0(path_to_height_models, "/", heightparam3)) %>% 
  rename(species = Species, forest_reserve = BR) %>% 
  filter(species != "<ALL>") %>% 
  mutate(species = as.integer(species),
         period = 3)
dh_parameters <- rbind(dh_parameters2, dh_parameters3)
```
...
# [1] "Hannecart"    "Hannecartbos"


stems_height <- stems_height %>% 
  filter(period > 1) %>% 
  mutate(forest_reserve = "Hannecart")
```
...
stems_height_ <- stems_height %>% 
  left_join(dh_parameters, by = c("species", "forest_reserve", "period")) %>% 
