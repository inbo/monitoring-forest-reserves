...
# dh-parameters
# heightparam <- "HeightParams_CoolhemA_KV.xlsx"
# heightparam <- "HeightParams_CoolhemQ_KV.xlsx"
heightparam <- "HeightParams_CoolhemAQ_KV.xlsx"
```
...
p5 <- ggplot(stems_height_ %>% filter(species %in% listspecies),
  ...
  geom_smooth(aes(x = dbh_mm, y = pred_height, color = forest_reserve), linetype = "solid", size=0.5) +
  facet_wrap(~name_nl)
