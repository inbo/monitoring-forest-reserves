...
Obv onderstaand overzicht voor Everzwijnbad KV: JA

Zeker voor Hazzelaar (100% hakhout), maar ook voor lijsterbes en tKa kan die ene extra meting waardevol zijn (niet veel hoogtes).

Bij esdoorn veel andere metingen, daar niet echt noodzakelijk, mlaar voor de eenvoud toch meenemen.
Gaat om ca 13%. 
...
# per periode
# stems_height %>% group_by(period, name_nl) %>% 
#   summarize(aantal_hoogtes = n(), 
#             aantal_hoogtes_hakhout = sum(ind_sht_cop == 12),
#             perc_hoogtes_hakhout = round(100*aantal_hoogtes_hakhout/aantal_hoogtes, 1)) %>% 
#   ungroup() %>% 
#   DT :: datatable(options = list(pageLength = 10), rownames = FALSE, filter = "top",
#                   colnames = c('period', 'Soort', 'aantal hoogtes', 'aantal hoogtes hakhout', 'aandeel hakhout (%)')) 
```
...
Zit er verschil op de hoogtes van Lijsterbes?

```{r vgl_rb_1_vs_3_mean, include=FALSE}
stems_height_es <- stems_height %>% filter(name_nl == "Lijsterbes")
...
# diameters
d1 <- ggplot(trees %>% filter(species == 39 & period == 1), aes(dbh_mm)) + geom_histogram() + 
  ggtitle("diameterverdeling")
d3 <- ggplot(trees %>% filter(species == 39 & period == 3), aes(dbh_mm)) + geom_histogram()
grid.arrange(d1, d3, nrow = 1)
...
Wat betreft Everzwijnbad KV:

- er zijn geen metingen gebeurd in de tweede decade en ook in de derde decade zijn er minder hoogtes gemeten dan in de eerste decade

- Voor de minder voorkomende boomsoorten is het duidelijk een meerwaarde om hoogtes uit de twee periodes samen te nemen
...
- 1eSET: data 1e + 3eSET
- 2eSET: data 1e + 3eSET  (geen hoogtes gemeten in 2de decade)
- 3eSET: data 1e + 3eSET
