...
Er zijn geen erraneous records in de kernvlakte van Heirnisse. 
...
hazelaar2 <- load_data_dendrometry(database = path_to_fieldmap_db) %>% 
  filter(alive_dead == 11 & intact_snag == 11) %>% 
  mutate(dbh_cm = dbh_mm/10) %>% 
  filter(species == 28)
...
- **Hazelaar (28)**: meting bij CP's: 14 cm dbh is 12m hoog
=> Exp functie van de CP's gebruiken (idem als vorige keer)
(15 is 17m hoog bij de log)

- **Spork (63)**: 10 cm dbh en 15m hoog
exp van CP's wordt 13m
log van CA wordt 12m
=> Logaritmische functie gebruiken
...
Mocht dit toch het geval zijn, dan moeten we die manueel toevoegen aan de excel-file "HeightParams_Heirnisse_CA.xlsx", met de algemene curve als curve, en n = 0.
...
Bij de CA van Heirnisse is dit niet nodig. 

Cfr. verwerking 2022 (1e en 2e SET):     

- Breedbladige bij smalbladige? (77, 78) NEE, andere hoogtes, ook niet geclusterd bij CP’s
- Ruwe en zachte berk? (57, 86): 
    o	voldoende metingen elk afzonderlijk, maar bij zachte berk is er extrapolatie
    o	toch geen optie om samen te voegen, andere groeivorm


# Aanpassen parameterfile

De excel-file "HeightParams_Heirnisse_CA.xlsx" aanpassen
...
De plotmodellen (plotID = 91001) van soorten waar ook een globaal model (plotID = <ALL>) 
