...
```{r Setup2}
forestreserve <- "Sevendonck A"
# (Sevendonck Q in ander script)

forestreserve_folder <- paste0(path_to_reserves, "4_Sevendonck_KV")
...
# per soort
p1 <- ggplot(stems_height  %>% filter(species %in% c(14, 39, 86, 87, 89)), aes(dbh_mm, height_m)) + geom_point(aes(color = ind_sht_cop)) + geom_smooth() + facet_wrap(~species)
p1

```

```{r vgl_individ_coppice_curve2, eval = FALSE}
ggplot(stems_height %>% filter(species %in% c(14, 39, 86, 87, 89)), aes(dbh_mm, height_m)) + geom_point(aes(color = ind_sht_cop)) + geom_smooth(aes(color = ind_sht_cop)) 

ggplot(stems_height %>% filter(species %in% c(14, 39, 86, 87, 89)), aes(dbh_mm, height_m)) + geom_point() + geom_smooth() + facet_wrap(~ind_sht_cop)

ggplot(stems_height %>% filter(species %in% c(14, 39, 86, 87, 89)), aes(dbh_mm, height_m)) + geom_point(aes(color = ind_sht_cop)) + geom_smooth() 
...
p2 <- ggplot(stems_height %>% filter(species %in% c(14, 39, 86, 87, 89) & ind_sht_cop == 10), aes(dbh_mm, height_m)) + geom_point(aes(color = ind_sht_cop)) + geom_smooth() + facet_wrap(~species)
p2
...
Meer metingen in 2007, maar ook nog extra in 2016 => beide gebruiken
...
stems_height_els <- stems_height %>% filter(name_nl == "Zwarte els")

stems_height_els_p1 <- stems_height_els %>% filter(period == 1)
stems_height_els_p2 <- stems_height_els %>% filter(period == 2)

mean(stems_height_els_p1$height_m, na.rm = T) # 17.2
mean(stems_height_els_p2$height_m, na.rm = T) # 18.6

```

Mean_height verschilt `r mean(stems_height_els_p1$height_m) - mean(stems_height_els_p2$height_m)` m.

Zwarte els is gemiddeld 1.33 m gegroeid.


(periode 1 : `r mean(stems_height_els_p1$height_m)` m - 
periode 2: `r mean(stems_height_els_p2$height_m)` m )

## Histogram

```{r vgl_els_1_vs_2_histogram}
# hoogtes
h1 <- ggplot(stems_height_els_p1, aes(height_m)) + geom_histogram()
h2 <- ggplot(stems_height_els_p2, aes(height_m)) + geom_histogram()
grid.arrange(h1, h2)
...
ggplot(stems_height_els, aes(dbh_mm, height_m)) + geom_point(aes(color = fperiod)) + geom_smooth(aes(color = fperiod)) 

# ggplot(stems_height_els, aes(dbh_mm, height_m)) + geom_point() + geom_smooth() + facet_wrap(~fperiod)

ggplot(stems_height_els, aes(dbh_mm, height_m)) + geom_point(aes(color = fperiod)) + geom_smooth()

```

Beide curves verschillen niet teveel
...
JA: mean_height verschilt `r mean(stems_height_es_p2$height_m) - mean(stems_height_es_p1$height_m)` m.
De essen zijn gemiddeld `r mean(stems_height_es_p2$height_m) - mean(stems_height_es_p1$height_m)` m gegroeid.
...
Te weinig metingen periode 2 om afzonderlijke curve op te stellen.
...
mean(stems_height_eik_p1$height_m, na.rm = T) # 16.6
mean(stems_height_eik_p2$height_m, na.rm = T) # 20.4
...
Obv de curves kan niet besloten worden dat er een groei is of een verschuiving van curves, te weinig metingen 2de SET.
...
write_xlsx(output, paste0(path_to_datarequest, "/TblVirtueleTrees_KV_A_1e2eSET.xlsx"))
