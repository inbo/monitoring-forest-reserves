...
```{r load_plotinfo, results = 'hide', eval = FALSE}
...
unique(data_stems_calc$forest_reserve)
```
```{r}
# kleine check
data_stems_calc %>% filter(is.na(calc_height_m)) %>% nrow() == 0
t <- data_stems_calc %>% filter(is.na(calc_height_m))
```
...
check <- trees_standing4 %>% 
  dplyr::select(plot_id, NameNl, tree_id, period, d130, c130
                , c0, c10_, htot
...
analyseSet_plot0 <- trees_standing6 %>%
  ...
  dplyr::summarise(    
            ...
            StemVolume_m3_ha_alive = 
              sum(StemVolume_m3*(alive_dead == 11), na.rm = TRUE),
            VolumeStump_m3_ha_alive = 
              sum(VolumeStump_m3*(alive_dead == 11), na.rm = TRUE),
            TotalVolumeAGB_m3_ha_alive = 
              sum(VolumeAGB_m3*(alive_dead == 11), na.rm = TRUE),

            BiomassAboveGr_t_ha_alive = 
              sum(BiomassAboveGr_t*(alive_dead == 11), na.rm = TRUE),
            BiomassBelowGr_t_ha_alive = 
              sum(BiomassBelowGr_t*(alive_dead == 11), na.rm = TRUE),
            Biomass_t_ha_alive = 
              sum(TotalBiomass_t*(alive_dead == 11), na.rm = TRUE),
            
            CarbonAboveGr_t_ha_alive = 
              sum(CarbonAboveGr_t*(alive_dead == 11), na.rm = TRUE),
            CarbonBelowGr_t_ha_alive = 
              sum(CarbonBelowGr_t*(alive_dead == 11), na.rm = TRUE),
            Carbon_t_ha_alive = 
              sum(TotalCarbon_t*(alive_dead == 11), na.rm = TRUE),
            
            # DEAD
            StemVolume_m3_ha_dead = 
              sum(StemVolume_m3*(alive_dead == 12), na.rm = TRUE),
            VolumeStump_m3_ha_dead = 
              sum(VolumeStump_m3*(alive_dead == 12), na.rm = TRUE),
            TotalVolumeAGB_m3_ha_dead = 
              sum(VolumeAGB_m3*(alive_dead == 12), na.rm = TRUE),

            BiomassAboveGr_t_ha_dead = 
              sum(BiomassAboveGr_t*(alive_dead == 12), na.rm = TRUE),
            BiomassBelowGr_t_ha_dead = 
              sum(BiomassBelowGr_t*(alive_dead == 12), na.rm = TRUE),
            BiomassStanding_t_ha_dead = 
              sum(TotalBiomass_t*(alive_dead == 12), na.rm = TRUE),
            
            CarbonAboveGr_t_ha_dead = 
              sum(CarbonAboveGr_t*(alive_dead == 12), na.rm = TRUE),
            CarbonBelowGr_t_ha_dead = 
              sum(CarbonBelowGr_t*(alive_dead == 12), na.rm = TRUE),
            CarbonStanding_t_ha_dead = 
              sum(TotalCarbon_t*(alive_dead == 12), na.rm = TRUE)
            ) %>%
  dplyr::ungroup() 
