...
`VolumeInclTwigs_m3`, maar voor snags gelijk is aan `vol_bole_m3 + vol_stump_m3`.
...
trees_standing6 <- trees_standing6 %>% 
  mutate(VolumeInclTwigs_m3 = VEF * (vol_bole_m3 + vol_stump_m3)
...
check <- trees_standing6 %>% 
  mutate(test = VolumeInclTwigs_m3/(vol_bole_m3 + vol_stump_m3)) %>% 
...
names(trees_standing6)
trees_standing6 %>% filter(is.na(decaystage) & alive_dead != 11) %>% nrow()  #118 (of 0.8% van de dode)
...
trees_standing6 %>% filter(alive_dead != 11) %>% nrow()
trees_standing6_ <- trees_standing6 %>% 
  mutate(R = R_FRL
         ...
         , biomass_standing_bgb_t = VolumeInclTwigs_m3 * WD * R * decomposition
