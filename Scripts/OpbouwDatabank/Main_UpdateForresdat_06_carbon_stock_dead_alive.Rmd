...
trees_standing1 <- trees_standing0 %>% 
  ...
  mutate(FC = FC_value)   # 0.47 in access-txt van Sébastien, maar 0.5 algemener aanvaard (ook cfr VMM LULUCF rapportage 2021)
...
trees_standing2 <- trees_standing1 %>% 
  mutate(htot=calc_height_m,  #?? of height_modeled voor snags om ondergronds biomassa te kunnen berekenen
         d130=dbh_mm/10,
         c130=pi*dbh_mm/10)

trees_standing2 %>% filter(is.na(htot)) %>% pull(Periode) %>% unique() == 1
# Height_modeled ontbreekt enkel bij VBI1
```
...
t <- trees_standing3 %>% filter(IntactTreeCode == 20) %>% 
  select(contains("height"), htot, IntactTreeCode, everything())
```

> chunck hierna wel BR
...
trees_standing4  %>% filter(c10 == 0 | is.na(c10)) %>% nrow() == 0
...
Vanaf 23/10/2023 is minimaaal volume op 0.001 m gezet (ipv 0), cfr werkwijze oude dataverwerking VBI1.
Dat vangt ook al een deel op.
> is dat bij BR ook al geïmplementeerd??
...
trees_standing6 <- trees_standing6 %>% 
  mutate(VolumeInclTwigs_m3 = VEF * (StemVolume_m3 + VolumeStump_m3)
         , VolumeAGB_m3 = ifelse(is.na(IntactTreeCode) | IntactTreeCode == 10    # intact
                                 , VolumeInclTwigs_m3
...
mean(trees_standing6$VEF)
median(trees_standing6$VEF)
```
...
bovengronds volume x soort-specifieke wood density (ton/m³) x 0.75 (deadwood 
decomposition: if decomposition was in progress)
...
(in Vlaanderen bepaald mbv line intersect)
...
trees_standing6 <- trees_standing6 %>% 
  mutate(R = R_FRL,
         WD = WD_FRL) %>% # FC = 0.5 is hoger reeds gedefinieerd
  mutate(BiomassAboveGr_t = VolumeAGB_m3*WD
         # we veronderstellen voor staand dood hout een decompositiefactor van 0.75
         # (if decomposition was in progress)
         , deadw_decomp = ifelse(StatusTree == "levend", 1, 0.75) 
         # ook bij dode bomen wordt ondergrondse biomassa afgeleid van VolumeInclTwigs_m3
         , BiomassBelowGr_t = VolumeInclTwigs_m3*WD*R*deadw_decomp
...
save(trees_standing6, file = paste0(here::here(),"/scripts/Analysedatabank/Rdata/trees_standing6.Rdata"))
...
> hierna nog checken of select obv kolomnrs klopt (< VBI code)

```{r plotniveau, results = 'hide'}
colnames(trees_standing6)

test <- trees_standing6 %>% filter(IDPlots %in% testplot) %>% 
  select(1:23, 53:63)
...
analyseSet_plot0 <- trees_standing6 %>%
  ...
  dplyr::summarise(    
            ...
            StemVolume_m3_ha_alive = 
              sum(StemVolume_m3*Fext_Ha*(StatusTreeCode == 1), na.rm = TRUE),
            VolumeStump_m3_ha_alive = 
              sum(VolumeStump_m3*Fext_Ha*(StatusTreeCode == 1), na.rm = TRUE),
            TotalVolumeAGB_m3_ha_alive = 
              sum(VolumeAGB_m3*Fext_Ha*(StatusTreeCode == 1), na.rm = TRUE),

            BiomassAboveGr_t_ha_alive = 
              sum(BiomassAboveGr_t*Fext_Ha*(StatusTreeCode == 1), na.rm = TRUE),
            BiomassBelowGr_t_ha_alive = 
              sum(BiomassBelowGr_t*Fext_Ha*(StatusTreeCode == 1), na.rm = TRUE),
            Biomass_t_ha_alive = 
              sum(TotalBiomass_t*Fext_Ha*(StatusTreeCode == 1), na.rm = TRUE),
            
            CarbonAboveGr_t_ha_alive = 
              sum(CarbonAboveGr_t*Fext_Ha*(StatusTreeCode == 1), na.rm = TRUE),
            CarbonBelowGr_t_ha_alive = 
              sum(CarbonBelowGr_t*Fext_Ha*(StatusTreeCode == 1), na.rm = TRUE),
            Carbon_t_ha_alive = 
              sum(TotalCarbon_t*Fext_Ha*(StatusTreeCode == 1), na.rm = TRUE),
            
            # DEAD
            StemVolume_m3_ha_dead = 
              sum(StemVolume_m3*Fext_Ha*(StatusTreeCode == 2), na.rm = TRUE),
            VolumeStump_m3_ha_dead = 
              sum(VolumeStump_m3*Fext_Ha*(StatusTreeCode == 2), na.rm = TRUE),
            TotalVolumeAGB_m3_ha_dead = 
              sum(VolumeAGB_m3*Fext_Ha*(StatusTreeCode == 2), na.rm = TRUE),

            BiomassAboveGr_t_ha_dead = 
              sum(BiomassAboveGr_t*Fext_Ha*(StatusTreeCode == 2), na.rm = TRUE),
            BiomassBelowGr_t_ha_dead = 
              sum(BiomassBelowGr_t*Fext_Ha*(StatusTreeCode == 2), na.rm = TRUE),
            BiomassStanding_t_ha_dead = 
              sum(TotalBiomass_t*Fext_Ha*(StatusTreeCode == 2), na.rm = TRUE),
            
            CarbonAboveGr_t_ha_dead = 
              sum(CarbonAboveGr_t*Fext_Ha*(StatusTreeCode == 2), na.rm = TRUE),
            CarbonBelowGr_t_ha_dead = 
              sum(CarbonBelowGr_t*Fext_Ha*(StatusTreeCode == 2), na.rm = TRUE),
            CarbonStanding_t_ha_dead = 
              sum(TotalCarbon_t*Fext_Ha*(StatusTreeCode == 2), na.rm = TRUE)
            ) %>%
  dplyr::ungroup() %>%
...
# ?? onderstaande nodig?? < VBI
analyseSet_plot_no_biomass <- zero_volume_bosplots %>%
  anti_join(plots_only_dead_biomass %>% select(IDPlots, Periode))  # 208

# meerdere kolommmen allemaal 0 toekennen
colnames(analyseSet_plot0)
# NOG CHECKEN OF kolomnrs OK ZIJN
biomass_columns <- colnames(analyseSet_plot0[6:23])
analyseSet_plot_no_biomass[biomass_columns] <- 0

# combine
analyseSet_plot1 <- rbind(analyseSet_plot0, analyseSet_plot_no_biomass)
# 6838
...
> nog aanpassen cfr BR

### Volume VBI1

Voor VBI1 is er geen info over NH/LH en ook niet over afbraakstadium, 
daar maken we gebruik van `tbl6Deadwood` uit de analysedatabank..

- afbraakstadium: we onderstellen een gemiddelde afbraak => decompositiefactor = 0.75         
- onderscheid NH/LH: op basis van tbl2BestandskaraktKwal - v2_StandType (NH, LH;, gem. NH, gem. LH)     
(alternatief zou zijn obv hoofdbms of bms-samenstalling, maar is ook maar vals 
gevoel van nauwkeurigheid; in Wallonie wordt ook enkel LH/NH onderscheiden bij liggend dood hout)

    * LH => WD van ander loofhout: WD 0.52 (80% LH)    
    * NH => WD van ander naaldhout: WD 0.42   
<!-- gem. LH => WD van ander loofhout: WD 0.52 (50 a 80% LH => ca 65% LH en 35% NH  -->
<!--     => WD = 0.52*0.65 + 0.42*0.35 = 0.485 -->
<!-- gem. NH => WD van ander naaldhout: WD 0.42  -->
<!--   => WD = 0.42*0.65 + 0.52*0.35 = 0.455 -->
<!-- OF:  -->
    * gem LH/NH: gemiddelde van beide => WD =  0.47     
  
Volumeberekening is ruwe benadering, obv drie velden bij veg-opname:      

•	LogLength_Diam_7_22_cm    
•	LogLength_Diam_22_40_cm     
•	LogLength_Diam_plus40_cm    

=> obv een gemiddelde diameter (14.5, 31 en 60 cm) en geschatte lengte wordt een volume berekend.

```{r logsVBI1}
names(tbl_kwal)
unique(tbl_kwal$v2_StandType)
names(tbl_deadw)
...
logsVBI1 <- tbl_deadw %>% 
  filter(Periode == 1) %>% 
  left_join(tbl_kwal) %>% 
  select(IDPlots, Periode
         # , IDSegments   # niet meenemen, want VBI1 geen segmenten
         , v3_VolumeLogs
         , v2_StandType) %>% 
  mutate(WD = case_when(
    v2_StandType == "loofhout" ~ 0.52,
    v2_StandType == "naaldhout" ~ 0.42,
    v2_StandType %in% c("gemengd loofhout", "gemengd naaldhout") ~ 0.47
    )
    , decomposition = 0.75
    , BiomassLyingDeadw_t_ha = v3_VolumeLogs * WD * decomposition) 

NA_logsVBI1 <- logsVBI1 %>% 
  filter(is.na(v3_VolumeLogs))
nrow(NA_logsVBI1); nrow(logsVBI1) 
# [1] 1383
# [1] 2713
# voor ongeveer de helft van de plots zijn er geen gegevens - klopt
...
Het liggende dood hout wordt - sinds VBI2 - opgemeten mbv LIS (met info over decaystage en NH_LH).

We kunnen ons niet enkel baseren op tbl6Deadwood, want daar is NH_LH en decaystage niet in opgenomen.
We gebruiken tbl6Deadwood wel om onderscheid te maken tss nulwaardes en NA's.

- NH vs LH:    

    - ander loofhout: WD 0.52
    - ander naaldhout: WD 0.42
    - niet gekend (NA of "onbekend"): WD 0.47
<!-- 1 = loofhout, 2 = naaldhout, 3 = onbekend -->

- decaystage:   

    - 10: levend, knoppen --> decompositiefactor 1
    - 20: hout hard, schors intact  --> decompositiefactor 1
    - 30: gedeeltelijk verteerd  --> decompositiefactor 0.75
    - 40: grotendeels verteerd --> decompositiefactor 0.5
    
    
```{r lim_VBI2}
# inlezen
query_LIMVBI2<-"
SELECT LIM_data.IDPlots, LIM_data.IDLine_intersect_method, LIM_data.Diameter_cm, LIM_data.Angle_degrees, LIM_data.DoubleMeasure, LIM_data.Decay_stage, LIM_data.DeadWoodCategory
FROM LIM_data;
"

connectieVBI2 <-   odbcDriverConnect(paste0(
  "Driver=", "{Firebird ODBC Driver}",
  ";Dbname=", dbVBI2Val_path,
  ";Client=", "C:\\Program Files\\Firebird\\Firebird_5_0\\fbclient.dll",
  ";User=", "SYSDBA",
  ";Password=", "masterkey"
))
  LIMVBI2Orig<-sqlQuery(connectieVBI2, query_LIMVBI2, stringsAsFactors = TRUE)
...
colnames(LIMVBI2)
colnames(LIMVBI2) <- c("IDPlots", "IDLine_intersect_method", "Diameter_cm", "Angle_degrees", "DOUBLEMEASURE", "decaystage", "LH_NH")
LIMVBI2$Periode<-2

str(LIMVBI2)
nrow(LIMVBI2[is.na(LIMVBI2$Diameter_cm),]) #37
# blijkbaar meerdere NA's voor Diameter_cm => wegfilteren (? lege records, vaak per ongeluk aangemaakt)
LIMVBI2 <- LIMVBI2 %>% filter(!is.na(Diameter_cm))

# ook soms voor degrees niks ingevuld => onderstel "0" (anders geen berekening mogelijk voor die plot)
LIMVBI2$Angle_degrees <- ifelse(is.na(LIMVBI2$Angle_degrees),0, LIMVBI2$Angle_degrees)

summary(LIMVBI2) # ok
unique(LIMVBI2$decaystage)
# [1] 20 30 40 NA 10
  # 10: levend, knoppen --> decompositiefactor 1
  # 20: hout hard, schors intact  --> decompositiefactor 1
  # 30: gedeeltelijk verteerd  --> decompositiefactor 0.75
  # 40: grotendeels verteerd --> decompositiefactor 0.5
unique(LIMVBI2$LH_NH)
# [1]  1  2 NA  3
# 1 = loofhout, 2 = naaldhout, 3 = onbekend
...
```{r lim_VBI3}
# inlezen
LIMVBI3 <- get_logs_detail()
LIMVBI3 <- LIMVBI3 %>% 
    dplyr::select(IDPlots = idplots, 
                  , Diameter_cm = diameter_cm, Angle_degrees = angle_degrees
                  , deadwoodinventorydone  # 1 = ja / 2 = nee
                  , decaystage = decay_stage
                  , LH_NH = deadwoodcategory) %>%
    mutate(IDLine_intersect_method = 1
           , Periode = 3)

str(LIMVBI3)
nrow(LIMVBI3 %>% 
    filter(is.na(Diameter_cm) & deadwoodinventorydone !=1)) 
nrow(LIMVBI3 %>% 
    filter(deadwoodinventorydone !=1)) 
# 3 * 15m van dezelfde plot 162002 waar geen liggend dood hout inventaris werd gedaan
# 321131: dood hout genoteerd, maar niet afgewerkt (?) (deadwoodinventorydone = 2)

# enkel records mét diameter behouden
LIMVBI3 <- LIMVBI3[!is.na(LIMVBI3$Diameter_cm),]

# ook soms voor degrees niks ingevuld => onderstel "0" (anders geen berekening mogelijk voor die plot)
LIMVBI3 %>% filter(is.na(Angle_degrees)) %>% nrow()   # 4
LIMVBI3$Angle_degrees <- ifelse(is.na(LIMVBI3$Angle_degrees),0, LIMVBI3$Angle_degrees)

unique(LIMVBI3$decaystage)
# [1] 20 30 40 NA 10
  # 10: levend, knoppen --> decompositiefactor 1
  # 20: hout hard, schors intact  --> decompositiefactor 1
  # 30: gedeeltelijk verteerd  --> decompositiefactor 0.75
  # 40: grotendeels verteerd --> decompositiefactor 0.5
unique(LIMVBI3$LH_NH)
# [1]  1  2 NA  3
# 1 = loofhout, 2 = naaldhout, 3 = onbekend

```
```{r combine_lim}
names(LIMVBI2) # veld DOUBLEMEASURE wordt niet gebruikt
names(LIMVBI3) # veld deadwoodinventorydone wordt niet gebruikt

LIMVBI2_3 <- rbind(LIMVBI2 %>% select(-DOUBLEMEASURE)
                    , LIMVBI3 %>% select(-deadwoodinventorydone))


# Formule - berekening volume per ha
Li<-45
...
# NA versus 0? - wordt verder opgevangen op plotniveau
...
Eerst `logsVBI1` en `logs_VBI2_3` combineren.   
Dan sommeren per plot (want nu nog per decaystage en NH/LH apart).    
Vervolgens combineren met `analyseSet_plot1` tot `analyseSet_plot2`.    

```{r logs_all}
names(logsVBI1)
names(logsVBI2_3)

logsVBI1 <- logsVBI1 %>% select(-v2_StandType)
logsVBI2_3 <- logsVBI2_3 %>% select(-LH_NH, -decaystage)
logs <- rbind(logsVBI1, logsVBI2_3)

#
logs_per_plot_ <- logs %>% 
  dplyr::group_by(IDPlots, Periode) %>%   
  dplyr::summarise(
    VolumeLogs_m3_ha = sum(v3_VolumeLogs, na.rm = TRUE)
    , BiomassLying_t_ha_dead = sum(BiomassLyingDeadw_t_ha, na.rm = TRUE)
  ) %>% 
  ungroup() %>% 
  mutate(CarbonLying_t_ha_dead = BiomassLying_t_ha_dead * FC_value)
# LIM berekent volume op plotniveau (altijd 3x15m transect), zelfs al liggen er foa's in de plot
# wordt verrekend door plotweight te gebruiken om statistieken op toe te passen
# in het stukje bos is volume logs = xx/ha, idem in stuk foa
# @leen: dat klopt toch?
```
```{r}
# 0 versus NA obv tbl_deadw
names(tbl_deadw)

logs_per_plot <- tbl_deadw %>% 
  filter(!is.na(v3_VolumeLogs)) %>% 
  select(IDPlots, Periode, v3_VolumeLogs) %>% 
  unique() %>% 
  # testje: maar één record per plot => op plotniveau
  # group_by(IDPlots, Periode) %>% 
  # summarize(n = n()) %>% 
  # ungroup()
  left_join(logs_per_plot_) %>% 
  mutate(across(c(VolumeLogs_m3_ha, BiomassLying_t_ha_dead, CarbonLying_t_ha_dead),
    ~ coalesce(.x, 0))  # alle NA's vervangen door 0
    )
# %>% 
#   mutate(test = v3_VolumeLogs - VolumeLogs_m3_ha)
# range(logs_per_plot$test, na.rm = TRUE) # OK

# 4899 wordt 5536
t <- logs_per_plot %>% 
  anti_join(logs_per_plot_, by = c("IDPlots", "Periode"))
range(t$v3_VolumeLogs)
# [1] 0 0
# klopt, zijn enkel nulwaardes die er extra bijkomen
```
...
analyseSet_plot2 <- analyseSet_plot1 %>% 
  left_join(logs_per_plot %>% select(-v3_VolumeLogs),
            by = c("IDPlots", "Periode")) %>% 
