...
```{r}
logs_by_decay_plot_species <- read.csv2(paste0(path_to_plotlevel_csv, "logs_by_decay_plot_species.csv")) %>% 
...
```{r}
colnames(logs_by_decay_plot_species)
colnames(BiomassFactors4_FRL)

logs1 <- logs_by_decay_plot_species %>% 
  left_join(BiomassFactors4_FRL, by = c("species" = "IDTreeSp")) %>% 
  mutate(FC = FC_value)  
        # FC = 0.47 in access-txt van Sébastien, maar 0.5 algemener aanvaard 
        # (ook cfr VMM LULUCF rapportage 2021)
