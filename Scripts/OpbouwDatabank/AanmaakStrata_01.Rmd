...
```{r import_data_shp_c_schijf}
# c-schijf: net iets anders !!
...
info_forest_reserve_EUFORIA <- read_xlsx(paste0(path_to_EUFORIA_algemeen, "INBO_info_forest_reserve_ikv_EUFORIA.xlsx")) %>% 
  select(1:13) %>% 
...
names(info_forest_reserve_EUFORIA)

info_forest_reserve_EUFORIA_ <- info_forest_reserve_EUFORIA %>% 
  left_join(forest_reserve_list_EUFORIA, 
  ...
  select(-SiteID_EUFORIA)

names(info_forest_reserve_EUFORIA_)
info_forest_reserve_EUFORIA <- info_forest_reserve_EUFORIA_
```
...
ook nog een extra velde forest_reserve_FM_adjusted.
...
- info_forest_reserve_EUFORIA (opp. volledig reservaat, forest type, mean annual temp/precipitation)
...
strata <- list(
      ...
      info_forest_reserve_EUFORIA = info_forest_reserve_EUFORIA,
...
# TEMP
sqlDrop(con, "link_wildcard_fieldmap")
sqlSave(con, link_WC_FM, "link_wildcard_fieldmap")
