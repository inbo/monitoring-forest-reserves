...
# plotinfo_0 <- load_plotinfo(database = path_to_fieldmap_db, processed = TRUE) %>% 
#   filter(plottype != "OTHER") %>% 
#   filter(!(forest_reserve == "Bos Terrijst" & period == 3))
    # enig verschil is survey_deadw in zonien, moet idd true zijn, weliswaar niet in fieldmap,
    # maar wel opgemeten => beter inlezen vanuit forresdat!

# t <- plotinfo_0 %>% 
#   anti_join(plotinfo)
# t2 <- plotinfo %>% 
#   anti_join(plotinfo_0)
# waldo::compare(plotinfo, plotinfo_0)
```


# Load dendro data - voorlopig OK - 26/03/2025
...
## Staande bomen 1ste decade KV Zoniën KERN_1983 pas vanaf 30 cm
...
```{r eval = FALSE}
trees_old <- read.csv2(file = paste0(path_to_treelevel_csv, "trees_calc.csv"))
nrow(trees_old) - nrow(data_dendro_calc) == -2514  # 36
...
check <- data_dendro_calc %>% 
  anti_join(trees_old, by = c("period", "plot_id", "tree_measure_id"))
table(check$period, check$forest_reserve)
  #   Bos Terrijst Coolhem A Coolhem Q
  # 3         1607       440       467
# ok, nieuwe reservaten

#--------
stems_old <- read.csv2(file = paste0(path_to_treelevel_csv, "stems_calc.csv"))          
nrow(stems_old) - nrow(data_stems_calc) == -3086
...
check2 <- data_stems_calc  %>% 
  anti_join(stems_old, by = c("period", "plot_id", "tree_measure_id"))
#3086
```
...
# Create unique tree-id - 26/3/2026 laatste keer gerund (11/3/2025 voorlaatste keer)

> doel is om in 2026 coppice om te vormen zdd levend en dood deel aan één boom toegekend wordt,
daarom hier geen strenge datacontrole wat betreft hakhout. 
> nieuwe opnamemethode vanaf Liedekerke, Pruikemakers in voege.
...
Nu zijn daar 3de decade wijnendale, everzwijnbad, with heide, heirnisse, jansheide,
terrijst, coolhem bijgekomen
? welke 2 nog extra? 
...
# [1] 17224 - 18015 - 18481 - 21583 - 22361
...
summ_coppice_id <- tree_id %>% 
  ...
  summarise(aantal = n()) %>% 
  ungroup()
# 16138 - 16536 - 19253  - 19265 - 19962

table(summ_coppice_id$aantal)  
# per periode max 2 (voor levend en dood deel zelfde coppice-ide)=> OK
#     1     2 
# 14645  1918
#     1     2 
# 16922  2331 
...
summ_coppice_id %>% filter(aantal > 2) %>% nrow() == 0
...
names(tree_id)
tree_id %>%  filter(is.na(tree_id)) %>% nrow() == 0
data_stems_calc %>% filter(tree_measure_id == 0) %>% nrow() == 0
...
> 22/12/2025: geen dubbele tree-id's
