---
title: "Update lijsten Florabank"
author: "Anja Leyman"
date: "`r Sys.Date()`"
output: 
  html_document:
...
DOEL van dit script is om de "kenmerken"-lijsten uit de florabank te updaten (Ellenberg, oud bos, rode lijst, .....).

Zo is er nu ook de optie om "bron" mee te nemen.
...
Soortenlijsten waar reeds TaxonID in zit (Value3)
...
test_taxongroep <- all_BR %>% 
  left_join(relTaxonTaxonGroep, by = c("TaxonID"))

test_taxongroep %>% filter(is.na(TaxonGroepID)) %>% nrow() == 22
# [1] 0 - OK

test_taxongroep %>% filter(is.na(TaxonGroepID)) %>% dplyr::pull(NameNl)
# allemaal "species" of combinaties van soorten => ok
```
...
mogelijke_kenmerken <- get_florabank_traits(
 ...
 trait_name = "", 
 collect = FALSE) %>% 
 distinct(Kenmerk, Bron) %>% 
 collect() 

names(mogelijke_kenmerken)
...
sqlSave(con, mogelijke_kenmerken, "tblFlorabank_overzicht_kenmerken_20210212")
sqlSave(con, fb_kenmerken_vaatplanten, "tblFlorabank_kenmerken_vaatplanten_20210212")
