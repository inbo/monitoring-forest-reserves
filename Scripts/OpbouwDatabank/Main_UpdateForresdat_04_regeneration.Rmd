...
# Load regeneration - OK - 8/4/2022 - 19/1/2023 - 18/3/2025 - 22/12/2025
...
reg_by_plot_JHB_gdrive <- read_xlsx(paste0(path_to_JHB, "reg_by_plot.xlsx"))

reg_by_plot_JHB <- reg_by_plot %>% 
  left_join(plotinfo %>% select(forest_reserve, plot_id, period)) %>% 
  filter(forest_reserve == "Jansheideberg") 
                           
# vgl
reg_by_plot_JHB_gdrive <- reg_by_plot_JHB_gdrive %>% 
  select(-1, -survey_number) 

nrow(reg_by_plot_JHB_gdrive);nrow(reg_by_plot_JHB)
summary(reg_by_plot_JHB_gdrive)
names(reg_by_plot_JHB_gdrive) == names(reg_by_plot_JHB)
summary(reg_by_plot_JHB)
waldo::compare(reg_by_plot_JHB_gdrive, reg_by_plot_JHB)

t <- reg_by_plot_JHB_gdrive %>% 
  anti_join(reg_by_plot_JHB)

t <- reg_by_plot_JHB %>% 
  anti_join(reg_by_plot_JHB_gdrive)

head(reg_by_plot %>% arrange(desc(plot_id), period)); 
head(reg_by_plot_JHB %>% arrange(desc(plot_id), period))
