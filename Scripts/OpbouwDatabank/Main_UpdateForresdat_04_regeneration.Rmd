...
# Load regeneration - OK - 8/4/2022 - 19/1/2023 - 18/3/2025 - 22/12/2025 - 30/3/2026
...
write.xlsx(too_high_gamedamage
           , file = paste0(path_to_dataverwerking_teamdrive, 
                         "30_Datacontrole/regeneration_20260330.xlsx")
           , showNA = FALSE
           , append = TRUE)
...
**Extra info uit de dataverwerking 2012:**     
...
# OF (< VBI code)
list2env(regeneration, envir = .GlobalEnv)
...
## TEMP: Check perc_rubbing op plot-niveau
...
# plot 410000, subplot 22: geen verjonging => er is een species "NA", die in één subplot voorkomt ...
# plot 141100, subplot 21, 23 en 34: geen verjonging => er is een species "NA", die in drie subplots voorkomt ... 

# in totaal 9  kernvlaktes waar er subplots zijn zonder verjonging, dus zonder soort en hoogteklasse,
# dat is OK, maar misschien wat verwarrend => wegfilteren

check_rubbing_perc_NA <- reg_by_core_area_species %>% 
  left_join(plotinfo %>% select(plot_id, period, game_impact_reg)) %>%
  filter(is.na(approx_rubbing_damage_perc_established) & nr_of_subplots_with_regeneration > 0) %>% 
  filter(game_impact_reg == TRUE) 

check_rubbing_perc_NA %>% nrow() == 0
```
## Wegfilteren species NA in de kernvlaktes

```{r CA_wegfilteren_species_NA}
# wegfilteren van species = NA
reg_by_core_area_height_species <- reg_by_core_area_height_species %>% 
  filter(!is.na(species))

reg_by_core_area_species <- reg_by_core_area_species %>% 
  filter(!is.na(species))
...
# Vergelijking met recente verwerking JHB/Wijn/Terrijst

```{r eval = FALSE}
path_to_X <- "G:/Gedeelde drives/Team_Boseco_BR/PRJ_BR_Gegevensverwerking/20_Per_gebied/2026_Terrijst/03_output_R_plotlevel/"
reg_by_plot_X_gdrive <- read_xlsx(paste0(path_to_X, "reg_by_plot.xlsx"))

reg_by_plot_X <- reg_by_plot %>% 
  left_join(plotinfo %>% select(forest_reserve, plot_id, period)) %>% 
  filter(forest_reserve == "Bos Terrijst") 
                           
# vgl
reg_by_plot_X_gdrive <- reg_by_plot_X_gdrive %>% 
  select(-1, -survey_number) 

nrow(reg_by_plot_X_gdrive);nrow(reg_by_plot_X)
summary(reg_by_plot_X_gdrive)
names(reg_by_plot_X_gdrive) == names(reg_by_plot_X)
summary(reg_by_plot_X)
waldo::compare(reg_by_plot_X_gdrive, reg_by_plot_X)

t <- reg_by_plot_X_gdrive %>% 
  anti_join(reg_by_plot_X)

t <- reg_by_plot_X %>% 
  anti_join(reg_by_plot_X_gdrive)

head(reg_by_plot_X %>% arrange(desc(plot_id), period)); 
head(reg_by_plot_X_gdrive %>% arrange(desc(plot_id), period))

head(reg_by_plot_X %>% arrange(plot_id, desc(period))); 
head(reg_by_plot_X_gdrive %>% arrange(plot_id, desc(period)))       
...
reg_by_core_area_height_species_gdrive <- read_xlsx(paste0(path_to_wijn, "reg_by_core_area_height_species.xlsx"))

reg_by_core_area_height_species_wijn <- reg_by_core_area_height_species %>% 
  left_join(plotinfo %>% select(forest_reserve, plot_id, period)) %>% 
  filter(forest_reserve == "Wijnendalebos") 
                           
# vgl
reg_by_core_area_height_species_gdrive <- reg_by_core_area_height_species_gdrive %>% 
  select(-1, -survey_number) 

nrow(reg_by_core_area_height_species_gdrive);nrow(reg_by_core_area_height_species_wijn)
names(reg_by_core_area_height_species_gdrive) == names(reg_by_core_area_height_species_wijn)
waldo::compare(reg_by_core_area_height_species_gdrive, reg_by_core_area_height_species_wijn)

t <- reg_by_core_area_height_species_gdrive %>% 
  anti_join(reg_by_core_area_height_species_wijn, by = c("plot_id", "period", "species", "height_class"))

t <- reg_by_core_area_height_species_wijn %>% 
  anti_join(reg_by_core_area_height_species_gdrive, by = c("plot_id", "period", "species", "height_class"))

head(reg_by_core_area_height_species_wijn %>% arrange(desc(plot_id), period)); 
head(reg_by_core_area_height_species_gdrive %>% arrange(desc(plot_id), period))
...
```{r eval = FALSE}
path_to_wijn <- "G:/Gedeelde drives/Team_Boseco_BR/PRJ_BR_Gegevensverwerking/20_Per_gebied/2024_Wijnendale/03_output_R_plotlevel/"
reg_by_plot_height_species_gdrive <- read_xlsx(paste0(path_to_wijn, "reg_by_plot_height_species.xlsx"))

reg_by_plot_height_species_wijn <- reg_by_plot_height_species %>% 
  left_join(plotinfo %>% select(forest_reserve, plot_id, period)) %>% 
  filter(forest_reserve == "Wijnendalebos") 
                           
# vgl
reg_by_plot_height_species_gdrive <- reg_by_plot_height_species_gdrive %>% 
  select(-1, -survey_number) 

nrow(reg_by_plot_height_species_gdrive);nrow(reg_by_plot_height_species_wijn)
names(reg_by_plot_height_species_gdrive) == names(reg_by_plot_height_species_wijn)
waldo::compare(reg_by_plot_height_species_gdrive, reg_by_plot_height_species_wijn)

t <- reg_by_plot_height_species_gdrive %>% 
  anti_join(reg_by_plot_height_species_wijn, by = c("plot_id", "period", "species", "height_class"))

t <- reg_by_plot_height_species_wijn %>% 
  anti_join(reg_by_plot_height_species_gdrive, by = c("plot_id", "period", "species", "height_class"))

head(reg_by_plot_height_species_wijn %>% arrange(desc(plot_id), period)); 
head(reg_by_plot_height_species_gdrive %>% arrange(desc(plot_id), period))
```

# Vergelijking met oude data

30/3/2026 vergeleken met data op git (dec 2025)

```{r eval = FALSE, results='hide'}
# 
reg_by_plot_git <- read_forresdat_table(tablename = "reg_by_plot"
                                           , join_plotinfo = FALSE
                                           , plottype = "all"
                                           , git_ref_type = "branch"
...
nrow(reg_by_plot);nrow(reg_by_plot_git)
summary(reg_by_plot)
names(reg_by_plot) == names(reg_by_plot_git)
summary(reg_by_plot_git)
waldo::compare(reg_by_plot, reg_by_plot_git)

t <- reg_by_plot %>% 
  anti_join(reg_by_plot_git, by = c("plot_id", "period"))
# plot 414: plotdetails waren niet ingevuld, nu wel, OK dat dat mee opgenomen is nu

head(reg_by_plot %>% arrange(desc(plot_id), period)); 
head(reg_by_plot_git %>% arrange(desc(plot_id), period))

```

```{r eval = FALSE, results='hide'}
# 
reg_by_core_area_species_git <- read_forresdat_table(tablename = "reg_by_core_area_species"
                                           , join_plotinfo = FALSE
                                           , plottype = "all"
                                           , git_ref_type = "branch"
                                           , git_reference = "develop")
...
# vgl
reg_by_plot_git <- reg_by_core_area_species_git %>% 
  select(plot_id, year, everything()) 
# %>% 
#   select(names(dendro_by_plot))    # vol_log_min40cm van naam veranderd!
nrow(reg_by_core_area_species);nrow(reg_by_core_area_species_git)
summary(reg_by_core_area_species)
names(reg_by_core_area_species) == names(reg_by_core_area_species_git)
summary(reg_by_core_area_species_git)
waldo::compare(reg_by_core_area_species, reg_by_core_area_species_git)

t <- reg_by_core_area_species %>% 
  anti_join(reg_by_core_area_species_git, by = c("plot_id", "period"))
t <- reg_by_core_area_species %>% 
  anti_join(reg_by_core_area_species_git)
            
head(reg_by_core_area_species %>% arrange(desc(plot_id), period)); 
head(reg_by_core_area_species_git %>% arrange(desc(plot_id), period))
...
```{r new_list_reg}
# opnieuw list van maken na correcties om te exporteren en plotinfo aan te koppelen
regeneration <- list(
      reg_by_plot = reg_by_plot,
      reg_by_plot_height = reg_by_plot_height,
      reg_by_plot_species = reg_by_plot_species,
