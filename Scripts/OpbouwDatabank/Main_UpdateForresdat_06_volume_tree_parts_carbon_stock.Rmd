...
data_deadwood %>% filter(max_diam_mm < 100) %>%  nrow()
...
stems_db_old <- read_csv2(paste0(path_to_treelevel_csv, "stems_calc.csv"))
...
trees_standing0 <- stems_calc %>%
  dplyr::select(-contains(c("_dead_", "_alive_"))) 
                # verwijderen van de berekeningen per hectare, hier is dat immers dubbel en niet nodig
                # verder wordt alles per hectare berekend
...
Het stamvolume (`vol_bole_m3`) wordt verder aangevuld met het volume van de stomp.  
...
check <- trees_standing4 %>%
  select(1:4, alive_dead, intact_snag, species, contains(c("height", "htot", "c0", "c1")))
check %>% filter(c10_ < c130) %>% nrow() # 66518
t <- check %>% filter((c130 - c10_ > 1) & htot <= 13) # 13
# ?? kan c10_ veel kleiner zijn dan C130 bij zware bomen?
# JA, c10_ is eigenlijk c10%: omtrek op 10% van de hoogte van de boom
# als calc_height_m = htot > 13 m, dan zal c10 kleiner zijn dan c130
...
# bij de smalste boompjes (< 24 cm omtrek) geeft deze vergelijking een onderschatting
t <- trees_standing4 %>%
  select(1:4, alive_dead, intact_snag, species, contains(c("height", "htot", "c0", "c1"))) %>% 
  filter((c130 - c0 > 0)) |> 
  mutate(diff_c = c130 - c0)

trees_standing4 <- trees_standing4 |> 
  mutate(c0 = ifelse(c0 < c130, c130, c0))
```
...
trees_standing4 <- trees_standing4 %>% 
  mutate(vol_stump_m3 = 0.1 * (c0/100)^2 / (4*pi))

trees_standing4 %>% filter(vol_stump_m3 == 0 | is.na(vol_stump_m3)) %>% nrow() == 0

check <- trees_standing4 %>% 
  dplyr::select(plot_id, NameNl, period, d130, c130
                , c0, c10_, htot
                , vol_bole_m3, vol_stump_m3) %>% 
  mutate(verh_stump_stam_perc = round(100*vol_stump_m3/vol_bole_m3, 2))
...
```{r tmp_replace_volume_0, results='hide', eval = FALSE}
range(trees_standing4$vol_bole_m3)

trees_standing4 <- trees_standing4 %>% 
  mutate(vol_bole_m3 = ifelse(vol_bole_m3 == 0, 0.001, vol_bole_m3))
```
...
We gebruiken hiervoor het veld `tmp_VolumeInclTwigs_m3`.

Daarvan wordt het veld `vol_tot_standing_agb_m3` afgeleid, dat voor intacte bomen gelijk is aan 
`tmp_VolumeInclTwigs_m3`, maar voor snags gelijk is aan `vol_bole_m3 + vol_stump_m3`.
(`vol_bole_m3` voor snags in `tbl0Boom` is berekend als een cilinder (dbh x hoogte), 
...
```{r tmp_VolumeInclTwigs_m3, results = 'hide'}
# vtotaer= varb * vef
names(trees_standing6)

trees_standing6 <- trees_standing6 %>% 
  mutate(tmp_VolumeInclTwigs_m3 = VEF * (vol_bole_m3 + vol_stump_m3)
         , vol_tot_standing_agb_m3 = ifelse(is.na(intact_snag) | intact_snag == 11    # intact
                                 , tmp_VolumeInclTwigs_m3
                                 , vol_bole_m3 + vol_stump_m3))

trees_standing6 %>% filter(is.na(tmp_VolumeInclTwigs_m3)) %>% nrow() == 0
range(trees_standing6$tmp_VolumeInclTwigs_m3)
range(trees_standing6$vol_bole_m3)

check <- trees_standing6 %>% 
  mutate(test = tmp_VolumeInclTwigs_m3/(vol_bole_m3 + vol_stump_m3)) %>% 
...
names(trees_standing6)
trees_standing6 %>% filter(is.na(decaystage) & alive_dead != 11) %>% nrow()  #113 (of 0.8% van de dode)
t <- trees_standing6 %>% filter(is.na(decaystage) & alive_dead != 11)
table(t$period)
# 1  2  3 
# 91  4 18 
table(trees_standing6$alive_dead)

# => rekening houden met ontbrekende decaystage
# onderscheid maken levend en dood
trees_standing6 %>% filter(alive_dead != 11) %>% nrow()  # 14282
trees_standing6_ <- trees_standing6 %>% 
  mutate(R = R_FRL
         ...
         , biomass_standing_agb_t = vol_tot_standing_agb_m3 * WD * decomposition
         # ook bij dode bomen wordt ondergrondse biomassa afgeleid van tmp_VolumeInclTwigs_m3
         , biomass_standing_bgb_t = tmp_VolumeInclTwigs_m3 * WD * R * decomposition
         , total_biomass_standing_t = biomass_standing_agb_t + biomass_standing_bgb_t)
...
trees_standing6 <- trees_standing6 %>% 
  mutate(carbon_standing_agb_t = biomass_standing_agb_t*FC,
         carbon_standing_bgb_t = biomass_standing_bgb_t*FC,
         total_carbon_standing_t = total_biomass_standing_t*FC
         ) 
...
Tijdelijk gebruik maken van de R-factor om van bovengrondse biomassa naar ondegrondse biomassa te gaan.
Is niet helemaal correct, maar nog geen alternatief gevonden.

**Copilot:**
Voor volume (m³) is er géén standaard equivalent die rechtstreeks zegt:
“ondergronds volume = x × bovengronds volume”

Waarom?     
- Wortels hebben sterk variabele vorm en vertakking
- Holtes, porositeit en onregelmatige geometrie
- Volume is minder ecologisch relevant dan biomassa → minder onderzocht

Alternatief: met behulp van worteldichtheid 
ondergronds volume = ondergrondse biomassa / dichtheid wortelhout

👉 Let op:

- wortelhout dichtheid ≠ stamhout dichtheid
- vaak 5–20% lager
- soms gelijk aangenomen bij gebrek aan data

(c) Anja: in het laatste geval (bij gelijke dichtheid van wortelhout vs stamhout) komt
dat neer op het gebruik van de R voor biomassa.

```{r results = 'hide'}
names(trees_standing6)
trees_standing7 <- trees_standing6 %>% 
  mutate(vol_tot_standing_bgb_m3 = tmp_VolumeInclTwigs_m3*R*decomposition)
        # op deze manier wordt rekening gehouden met een decompositiefactor

# (zelfde als vol_tot_standing_bgb_m3 = BiomassBelowGr_t/WD)

```

### Volume top

Volume top kan afgeleid worden met behulp van goniometrische formules, ervan uitgaande 
dat een boom een kegel is.

Als twee rechthoekige driehoeken een gelijke scherpe hoek hebben, dan zijn de verhoudingen 
van overeenkomstige zijden hetzelfde:     

R1/H1 = r2/h2 => h2 = r2*H1/R1

Volume top = 0 voor dode snags

```{r }
trees_standing7 %>% filter(is.na(intact_snag)) %>% nrow()
trees_standing7_ <- trees_standing7 %>% 
  mutate(R1 = dbh_mm/(2*10)
         , H1 = htot - 1.3
         , r2 = 7/2
         , h2 = r2*H1/R1
         , check = (round(H1/R1,3) == round(h2/r2, 3))
         , vol_top_m3 = ifelse(alive_dead == 11 | intact_snag == 11 | is.na(intact_snag)
                               # inhoud kegel = 1/3 x oppervlakte grondvlak x hoogte = 1/3 x π r2 x h
                               , pi * (r2/100)^2 * h2 / 3
                               , ifelse(intact_snag == 10 & alive_dead == 12
                                 , 0, 9999)
                               )
         )

trees_standing7_ %>% filter(is.na(vol_top_m3)) %>% nrow() == 0
trees_standing7_ %>% filter(vol_top_m3 == 9999) %>% nrow() == 0
mean(trees_standing7_$vol_top_m3); median(trees_standing7_$vol_top_m3)
trees_standing7 <- trees_standing7_
```

```{r eval = FALSE}
t <- trees_standing7 %>% select(period, NameNl, alive_dead, intact_snag
                                , dbh_mm, height_m, htot
                                , R1, H1, r2, h2, check
                                , vol_bole_m3, vol_top_m3) %>% 
  mutate(verh_h = h2/H1)

g <- ggplot(t, aes(x = dbh_mm, y = verh_h)) + geom_point ()
g

g <- ggplot(t, aes(x = htot, y = h2)) + geom_point ()
g

```

```{r}
trees_standing7 <- trees_standing7 %>% 
  select(-R1, -H1, -r2, -h2, -check)
```

### Volgorde attributen

```{r stems_all_parts, results = 'hide'}
names(trees_standing7)

trees_standing8 <- trees_standing7 %>% 
  select(plot_id, plottype, period, year, date_dendro
         , totalplotarea_ha, plotarea_ha
         , tree_measure_id, old_id, shoot_measure_id
         , x_local, y_local
         , species
         , dbh_mm, height_m, calc_height_m
         , alive_dead, intact_snag, ind_sht_cop, 
         , decaystage, dbh_class_5cm
         , crown_volume_reduction, branch_length_reduction
         , contains(c("iufro", "remark", "subcircle"))
         , basal_area_m2
         , vol_bole_m3, vol_large_branches_m3 = vol_crown_m3
         # , vol_tot_m3
         , vol_stump_m3, vol_top_m3
         , VEF
         , vol_tot_standing_agb_m3
         , R
         , vol_tot_standing_bgb_m3
         , WD, FC, decomposition
         , biomass_standing_agb_t, biomass_standing_bgb_t, 
         , total_biomass_standing_t
         , carbon_standing_agb_t, carbon_standing_bgb_t
         , total_carbon_standing_t
         )

# tbl_boom_notNA %>% filter(IntactTreeCode %in% c(20, 30)) %>% select(vol_crown_m3) %>% unique()
# # OK, enkel 0
```
```{r}
names(trees_standing8)
```

```{r check_geen_kroon, eval = FALSE}
t <- trees_standing8 %>% 
  filter((intact_snag == 10 | ind_sht_cop == 12) & 
          alive_dead == 12 & vol_large_branches_m3 != 0)
# dood hakhout heeft nog een kroonvolume, bij VBI wordt dat op 0 gezet
# maar elders bij BR gebeurt dat niet, dus hier beter ook niet doen.
# Op termijn ev. bij VBI terug kroon dood hakhout invoeren?
# Om hoeveel gaat het?

100*sum(t$vol_large_branches_m3)/(sum(trees_standing8$vol_large_branches_m3) + sum(trees_standing8$vol_bole_m3))
# 0.02 %  tov totaal volume, eventueel wel hoger in 
100*sum(t$vol_large_branches_m3)/(sum(trees_standing8[trees_standing8$alive_dead == 12, ]$vol_large_branches_m3) + sum(trees_standing8[trees_standing8$alive_dead == 12, ]$vol_bole_m3))
# 0.67% van het dode houtvolume

    # trees_standing8_ <- trees_standing8 %>% 
    #   mutate(vol_crown_m3 = ifelse(ind_sht_cop == 20 & alive_dead == 2
    #                                         , 0, vol_crown_m3)
    #          , vol_tot_m3 = vol_bole_m3 + vol_crown_m3)
    # 
    # trees_standing8_ %>% filter((intact_snag %in% c(20, 30) | ind_sht_cop == 20) & 
    #            alive_dead == 2 & vol_crown_m3 != 0) %>% nrow() == 0
    # 
    # trees_standing8_ %>% filter((intact_snag %in% c(20, 30) | ind_sht_cop == 20) & 
    #            alive_dead == 2 & (vol_stump_m3 + vol_bole_m3 != vol_tot_standing_agb_m3)) %>% nrow() == 0
    # 
    # trees_standing8 <- trees_standing8_
```
...
Onderscheid levende en dode volumes/biomassa per hectare

```{r plotniveau, results='hide'}
colnames(trees_standing8)

plot_level0 <- trees_standing8 %>%
  dplyr::group_by(plot_id, period) %>%   
  dplyr::summarise(    
            # vol_bole_m3_ha_all = sum(vol_bole_m3*Fext_Ha, na.rm = TRUE),
            vol_bole_m3_ha_alive = 
              sum(vol_bole_m3*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            vol_large_branches_m3_ha_alive = 
              sum(vol_large_branches_m3*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            vol_stump_m3_ha_alive = 
              sum(vol_stump_m3*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            vol_top_m3_ha_alive = 
              sum(vol_top_m3*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            vol_tot_standing_agb_m3_ha_alive = 
              sum(vol_tot_standing_agb_m3*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            vol_tot_standing_bgb_m3_ha_alive = 
              sum(vol_tot_standing_bgb_m3*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),

            biomass_standing_agb_t_ha_alive = 
              sum(biomass_standing_agb_t*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            biomass_standing_bgb_t_ha_alive = 
              sum(biomass_standing_bgb_t*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            biomass_standing_t_ha_alive = 
              sum(total_biomass_standing_t*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            
            carbon_standing_agb_t_ha_alive = 
              sum(carbon_standing_agb_t*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            carbon_standing_bgb_t_ha_alive = 
              sum(carbon_standing_bgb_t*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            carbon_standing_t_ha_alive = 
              sum(total_carbon_standing_t*(alive_dead == 11)/plotarea_ha, na.rm = TRUE),
            
            # DEAD
            vol_bole_m3_ha_dead = 
              sum(vol_bole_m3*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            vol_large_branches_m3_ha_dead = 
              sum(vol_large_branches_m3*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            vol_stump_m3_ha_dead = 
              sum(vol_stump_m3*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            vol_top_m3_ha_dead = 
              sum(vol_top_m3*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            vol_tot_standing_agb_m3_ha_dead = 
              sum(vol_tot_standing_agb_m3*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            vol_tot_standing_bgb_m3_ha_dead = 
              sum(vol_tot_standing_bgb_m3*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),

            biomass_standing_agb_t_ha_dead = 
              sum(biomass_standing_agb_t*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            biomass_standing_bgb_t_ha_dead = 
              sum(biomass_standing_bgb_t*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            biomass_standing_t_ha_dead = 
              sum(total_biomass_standing_t*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            
            carbon_standing_agb_t_ha_dead = 
              sum(carbon_standing_agb_t*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            carbon_standing_bgb_t_ha_dead = 
              sum(carbon_standing_bgb_t*(alive_dead == 12)/plotarea_ha, na.rm = TRUE),
            carbon_standing_t_ha_dead = 
              sum(total_carbon_standing_t*(alive_dead == 12)/plotarea_ha, na.rm = TRUE)
            ) %>%
  dplyr::ungroup() 
...
Op dit moment is dat enkel plot 475, periode 1.
...
zero_volume <- plot_level0 %>% 
  # full_join met alle plots, want plot_level0 bevat enkel staande bomen 
  full_join(plotinfo %>% select(plot_id, period
                                , survey_trees, year_dendro, data_processed)
            , by = c("plot_id", "period")) %>% 
  filter(is.na(biomass_standing_agb_t_ha_alive) & survey_trees == TRUE & data_processed == TRUE) 
  # als is.na(biomass_standing_agb_t_ha_alive), dan ook is.na(biomass_standing_agb_t_ha_dead)
...
# open_area <- get_open_area(datapackage)
...
colnames(plot_level0);colnames(zero_volume_bosplots)

# meerdere kolommmen allemaal 0 toekennen
colnames(plot_level0)

biomass_columns <- colnames(plot_level0[3:26])
zero_volume_bosplots[biomass_columns] <- 0

names(zero_volume_bosplots); names(plot_level0)

# combine
plot_level1 <- rbind(plot_level0, zero_volume_bosplots %>% 
                            select(-survey_trees, -year_dendro, -data_processed))
# 2009

```

## Omzetting naar plot-species-niveau

Vertrekkende van *stems_calc*, cfr plot_level0

Onderscheid levende en dode volumes/biomassa per hectare

```{r plot_speciesniveau, results='hide'}
colnames(trees_standing8)

plot_specieslevel_0 <- trees_standing8 %>%
...
```{r results='hide'}
names(plotinfo)
names(plot_specieslevel_0)
names(zero_volume_bosplots)
zero_volume_bosplots$species <- NA

# combine
plot_specieslevel_1 <- rbind(plot_specieslevel_0
                          , zero_volume_bosplots %>% 
                            select(-survey_trees, -year_dendro, -data_processed))
# 2009
...
Met **data afkomstig van LIS** moeten we voorzichtig omgaan, we kunnen niet de data op soort en afbraakniveau zomaar summarizen tot plotniveau, zoals we doen met de logs 
uit FAS. 
Anderzijds hebben we wel info van soort en afbraak nodig om biomassa te kunnen berekenen,
dus we kunnen ook niet verder met `vol_log_m3_ha` uit `dendro_by_plot(_species)`.
We zouden de plot-waardes en plot-species-waardes direct van de ruwe data 
moeten afleiden en niet - zoals we voor FAS kunnen doen - verder werkend op de 
geclusterde volumes in de tabel "deadw_by_decay_plot_species".
De volumes moeten op elk niveau apart berekend worden, tgv aard van de formule.

Sowieso zal een inschatting van biomassa obv een combinatie van LIS en FAS een héél ruwe
schatting zijn, die nooit op plotniveau mag gebruikt worden.

Bij de **VBI** worden de intersects gegroepeerd per NH/LH en afbraak, en dan wordt 
de formule DeVries toegepast.  
Dan een WD voor  NH/LH en een decomposition obv decaystage op dat volume.
Vervolgens sommeren per plot.

```
Li<-45
logsVBI2_3_ <- LIMVBI2_3 %>% 
  dplyr::group_by(IDPlots, Periode, LH_NH, decaystage) %>%
  dplyr::summarise(v3_VolumeLogs=pi^2/8/Li*sum((Diameter_cm^2)/cos(Angle_degrees*pi/180),na.rm=TRUE)) %>% 
  ungroup()
  
logsVBI2_3 <- logsVBI2_3_ %>% 
  mutate(
    WD = case_when(
  LH_NH == 1 ~ 0.52,
  LH_NH == 2 ~ 0.42,
  LH_NH %in% c(NA, 3) ~ 0.47
  )
  , decomposition = case_when(
  decaystage %in% c(10,20) ~ 1,
  decaystage == 30 ~ 0.75,
  decaystage == 40 ~ 0.5
  )
  , BiomassLying_t_ha = v3_VolumeLogs * WD * decomposition) 
  
logs_per_plot_VBI2_3 <- logsVBI2_3 %>% 
  dplyr::group_by(IDPlots, Periode) %>%   
  dplyr::summarise(
    vol_logs_m3_ha = sum(v3_VolumeLogs, na.rm = TRUE)
    , BiomassLying_t_ha_dead = sum(BiomassLying_t_ha, na.rm = TRUE)
  ) %>% 
  ungroup() %>% 
...
**Voor de huidige analyse gaan we enkel verder met de volopnames**

We berekenen naast `biomass_lying_deadw_t_ha` (wanneer dat NA is) 

- `biomass_lying_deadw_above30cm_t_ha`: LIS ingevoerd Wijnendale, Everzwijnbad, Heirnisse en Jansheideberg => FAS pas vanaf 30 cm in 3de decade, enkel `vol_log_above30cm_m3_ha` beschikbaar         
- `biomass_lying_deadw_above30cm_t_ha`: KV Zoniën KERN_1983 - periode 2 & 3 pas vanaf 40 cm opgemeten (2010 en 2020) => vol_log_m3_ha = NA, enkel `vol_log_above40cm_m3_ha` beschikbaar
...
logs2 <- logs1 %>% 
    mutate(WD = WD_FRL
         ...
         , biomass_lying_deadw_above30cm_t_ha = 
              ifelse(!is.na(vol_log_above30cm_m3_ha)
                     , vol_log_above30cm_m3_ha * WD * decomposition
                     , NA)
         , biomass_lying_deadw_above40cm_t_ha = 
...
t <- logs2 %>% 
  filter(is.na(biomass_lying_deadw_t_ha))
table(t$period, t$forest_reserve)
...
```{r logs_plot, results='hide'}
names(logs2)
logs_per_plot1 <- logs2 %>% 
  dplyr::group_by(plot_id, period) %>%   
  dplyr::summarise(
    vol_log_m3_ha = sum(vol_log_m3_ha, na.rm = FALSE)
    , vol_log_above30cm_m3_ha = sum(vol_log_above30cm_m3_ha, na.rm = FALSE)
    # OPGEPAST: niet correct om vol_lis te berekenen via summarize
    # , vol_lis_m3_ha = sum(vol_lis_m3_ha, na.rm = FALSE)
    # , vol_lis_10_30cm_m3_ha = sum(vol_lis_10_30cm_m3_ha, na.rm = FALSE)
    , vol_log_above40cm_m3_ha = sum(vol_log_above40cm_m3_ha, na.rm = FALSE)
    , biomass_lying_t_ha_dead = sum(biomass_lying_deadw_t_ha, na.rm = FALSE)
    , biomass_lying_above30cm_t_ha_dead = sum(biomass_lying_deadw_above30cm_t_ha, na.rm = FALSE)
    , biomass_lying_above40cm_t_ha_dead = sum(biomass_lying_deadw_above40cm_t_ha, na.rm = FALSE)
  ) %>% 
  ungroup() %>% 
  mutate(carbon_lying_t_ha_dead = biomass_lying_t_ha_dead * FC_value
         , carbon_lying_above30cm_t_ha_dead = biomass_lying_above30cm_t_ha_dead * FC_value
         , carbon_lying_above40cm_t_ha_dead = biomass_lying_above40cm_t_ha_dead * FC_value) %>% 
...
t3 <- logs_per_plot1 %>% 
  filter(is.na(vol_log_m3_ha)) %>% 
  left_join(plotinfo)
t3 %>% select(forest_reserve, period, plottype) %>% unique()
```

### Plot-species-niveau

Sommeren per plot en species (want nu nog per decaystage apart).    

Oppassen met NA's: sommeren van "niks" geeft waarde '0' => na.rm = FALSE . 

```{r logs_plot_species, results='hide'}
names(logs2)
logs_per_plot_species1 <- logs2 %>% 
  dplyr::group_by(plot_id, period, species) %>%   
  dplyr::summarise(
    vol_log_m3_ha = sum(vol_log_m3_ha, na.rm = FALSE)
    , vol_log_above30cm_m3_ha = sum(vol_log_above30cm_m3_ha, na.rm = FALSE)
...
logs_per_plot_extra <- dendro_by_plot %>% 
  ...
  anti_join(logs_per_plot1 %>% 
              filter(!is.na(biomass_lying_t_ha_dead) | 
                       !is.na(biomass_lying_above30cm_t_ha_dead) |
                       !is.na(biomass_lying_above40cm_t_ha_dead))
            , by = c("plot_id", "period"))

names(logs_per_plot_extra)
logs_per_plot_extra %>% select(forest_reserve, period) %>% unique()
# 160 extra records/plots waar deadwood = 0 of NA (wegens LIS) of uit monitoringrapport
...
logs_per_plot_extra_ <- logs_per_plot_extra %>% 
  mutate(WD = 0.52
         ...
         , biomass_lying_above30cm_t_ha_dead = 
              ifelse(!is.na(vol_log_above30cm_m3_ha)
                     , vol_log_above30cm_m3_ha * WD * decomposition
                     , NA)
         , biomass_lying_above40cm_t_ha_dead = 
              ifelse(!is.na(vol_log_above40cm_m3_ha)
                     , vol_log_above40cm_m3_ha * WD * decomposition
                     , NA)
         , carbon_lying_t_ha_dead = biomass_lying_t_ha_dead * FC_value
         , carbon_lying_above30cm_t_ha_dead = biomass_lying_above30cm_t_ha_dead * FC_value
         , carbon_lying_above40cm_t_ha_dead = biomass_lying_above40cm_t_ha_dead * FC_value
         ) %>% 
  select(forest_reserve
         ...
         , biomass_lying_t_ha_dead, biomass_lying_above30cm_t_ha_dead
         , biomass_lying_above40cm_t_ha_dead
         , carbon_lying_t_ha_dead, carbon_lying_above30cm_t_ha_dead
         , carbon_lying_above40cm_t_ha_dead
         , survey_deadw, survey_lis
  )
        
names(logs_per_plot1)  ==  names(logs_per_plot_extra_ %>% select(-forest_reserve))
# names(logs_per_plot1)  ;  names(logs_per_plot_extra_)

# enkel bij Zonien effectieve waardes verschillend van 0 of NA
logs_per_plot_extra_ %>% filter(!is.na(vol_log_m3_ha) & vol_log_m3_ha > 0) %>% select(forest_reserve, period) %>% unique()
# rest zijn 0-waardes of NA's die ook moeten toegevoegd worden aan carbon_by_plot
# 
...
logs_per_plot2 <- rbind(logs_per_plot1 %>% 
                          filter(!is.na(biomass_lying_t_ha_dead) | 
                                   ...
                                   !is.na(biomass_lying_above40cm_t_ha_dead))
                        , logs_per_plot_extra)
nrow(logs_per_plot2) # 2075
nrow(logs_per_plot1) # 1946
nrow(logs_per_plot_extra) # 171

# 171+1946 == 2117
# 2117-2075 == 42
# logs_per_plot1 %>% filter(!is.na(biomass_lying_t_ha_dead) | 
#                                    !is.na(biomass_lying_above30cm_t_ha_dead) | 
#                                    !is.na(biomass_lying_above40cm_t_ha_dead)) %>% nrow()
# # 1904
# t <- logs_per_plot1 %>% filter(is.na(biomass_lying_t_ha_dead) & 
#                                    is.na(biomass_lying_above30cm_t_ha_dead) & 
#                                    is.na(biomass_lying_above40cm_t_ha_dead))
# nrow(t) # 42

nrow(dendro_by_plot) == nrow(logs_per_plot2)
...
# NA NA
range(t$vol_log_m3_ha, na.rm = T)
# [1]  0.0 77.8

# ok, goed aangevuld

logs_per_plot2 %>% filter(is.na(biomass_lying_t_ha_dead) & 
                                    is.na(biomass_lying_above30cm_t_ha_dead) & 
                                    is.na(biomass_lying_above40cm_t_ha_dead)) %>% 
  left_join(plotinfo %>% select(forest_reserve, period, plot_id, plottype)) %>% 
  select(forest_reserve, period, plottype) %>% unique()
# OK, enkel ZOnien CA (periode 0 en 1) en Kluisbos
```

#### Plot-soortniveau

```{r corr_logs_extra_per_species, results='hide'}
names(dendro_by_plot_species); names(logs_per_plot_species1)

logs_per_plot_species_extra <- dendro_by_plot_species %>% 
  # filter(!is.na(vol_log_m3_ha)) %>% 
  select(forest_reserve, plot_id, period
         , species
         , survey_lis, survey_deadw
  ...
  # summarize(n = n()) %>%
  # ungroup()
  anti_join(logs_per_plot_species1 %>% 
              filter(!is.na(biomass_lying_t_ha_dead) | 
                       !is.na(biomass_lying_above30cm_t_ha_dead) |
                       !is.na(biomass_lying_above40cm_t_ha_dead))
            , by = c("plot_id", "period", "species"))

names(logs_per_plot_species_extra)
logs_per_plot_species_extra %>% select(forest_reserve, period) %>% unique()

# 4399 extra records/plots waar deadwood = 0 of NA (LIS) of uit monitoringrapport
# en bijgevolg niet opgenomen in deadw_by_decay_plot_species
...
```{r results = 'hide'}
# nullen aanvullen + waarde van CP's zoniën overnemen
logs_per_plot_species_extra_ <- logs_per_plot_species_extra %>% 
  mutate(WD = 0.52
         , decomposition = 0.75
         , biomass_lying_t_ha_dead = 
              ifelse(!is.na(vol_log_m3_ha)
...
names(logs_per_plot_species1)  ==  names(logs_per_plot_species_extra_ %>% select(-forest_reserve))
# names(logs_per_plot1)  ;  names(logs_per_plot_extra_)


# enkel bij Zonien effectieve waardes verschillend van 0 of NA
logs_per_plot_species_extra_ %>% filter(!is.na(vol_log_m3_ha) & vol_log_m3_ha > 0) %>% select(forest_reserve, period) %>% unique()
# rest zijn 0-waardes of NA's die ook moeten toegevoegd worden aan carbon_by_plot
# 

logs_per_plot_species_extra <- logs_per_plot_species_extra_  %>% 
  select(-forest_reserve)

```

```{r results='hide'}
logs_per_plot_species2 <- rbind(logs_per_plot_species1 %>% 
                          filter(!is.na(biomass_lying_t_ha_dead) |
                                   !is.na(biomass_lying_above30cm_t_ha_dead) |
                                   !is.na(biomass_lying_above40cm_t_ha_dead))
                        , logs_per_plot_species_extra)
nrow(logs_per_plot_species2)
nrow(logs_per_plot_species1)

nrow(dendro_by_plot_species) == nrow(logs_per_plot_species2)
```


```{r results='hide'}
t <- logs_per_plot_species2 %>% 
  anti_join(logs_per_plot_species1, by = c("plot_id", "period", "species"))
range(t$vol_log_m3_ha)
# NA NA
range(t$vol_log_m3_ha, na.rm = T)
# [1]  0.0 77.8
...
logs_per_plot_species2 %>% filter(is.na(biomass_lying_t_ha_dead) & 
                                    is.na(biomass_lying_above30cm_t_ha_dead) & 
                                    is.na(biomass_lying_above40cm_t_ha_dead)) %>% 
  left_join(plotinfo %>% select(forest_reserve, period, plot_id, plottype)) %>% 
  select(forest_reserve, period, plottype) %>% unique()
...
## Combineren staand en liggend

```{r biomass_all_plot, results='hide'}
names(logs_per_plot2)
names(plot_level1)

nrow(logs_per_plot2); nrow(plot_level1); nrow(dendro_by_plot)

# waarom niet zelfde aantallen?
# 13 plots open ruimte die reeds weggefilterd zijn bij plot_level1

# t <- logs_per_plot2 %>% 
#   anti_join(plot_level1, by = c("plot_id", "period"))
# # => dubbels
# t <- logs_per_plot2 %>% 
#   group_by(plot_id, period) %>% 
#   summarise(n = n()) %>% 
...
nrow(plot_level1) == nrow(logs_per_plot2) - 13
# anders worden NA's toegevoegd 
# ! checken of dat echt NA's zijn, of eerder nullen?

plot_level2 <- plot_level1 %>% 
  left_join(logs_per_plot2, by = c("plot_id", "period"))

names(plot_level2)
nrow(plot_level2) == nrow(plot_level1)
```

```{r biomass_all_plot_species, results='hide'}
names(logs_per_plot_species2)
names(plot_specieslevel_1)

nrow(logs_per_plot_species2); nrow(plot_specieslevel_1); nrow(dendro_by_plot_species)

# waarom niet zelfde aantallen?
# 13 plots (869 records) open ruimte die reeds weggefilterd zijn bij plot_level1
# ook soorten waarvan wel liggend dood hout voorkomt, maar geen staand
# bv. plot 101 en soort 57: vol_bole = 0; plot 109, soort 14, enkel LIS
# deze moeten wel toegevoegd worden
# deze zitten ook in dendro_by_plot_species

t <- logs_per_plot_species2 %>%
  anti_join(plot_specieslevel_1, by = c("plot_id", "period", "species"))
# => dubbels
t <- logs_per_plot_species2 %>%
  group_by(plot_id, period, species) %>%
  summarise(n = n()) %>%
  ungroup()

nrow(plot_specieslevel_1)
nrow(logs_per_plot_species2)
# anders worden NA's toegevoegd 
# ! checken of dat echt NA's zijn, of eerder nullen?

plot_specieslevel_2 <- plot_specieslevel_1 %>% 
  full_join(logs_per_plot_species2, by = c("plot_id", "period", "species")) 
# full join, want soms een soort enkel als log beschikbaar maar niet als staand

names(plot_specieslevel_2)
nrow(plot_specieslevel_2) # 10580
nrow(dendro_by_plot_species) # 10579
nrow(plot_specieslevel_1)

t <- plot_specieslevel_2 %>% 
  anti_join(dendro_by_plot_species %>% select(period, plot_id, species))
# plot 475 - OK 
```

```{r corr_NA_standing, results='hide'}
# correctie voor extra toegevoegde soorten waarvan staand niet voorkomt
# daar moeten alle variabelen van op 0 gezet worden

t <- plot_specieslevel_2 %>% 
  anti_join(plot_specieslevel_1 %>% select(period, plot_id, species))
summary(t) # allemaal NA's

names(plot_specieslevel_2)
col2cvt_0 <- colnames(plot_specieslevel_2[, 4:27])

plot_specieslevel_3 <- plot_specieslevel_2 %>%
  left_join(plotinfo %>% select(plot_id, period, survey_trees)) %>%
      # klein testje - OK
      # mutate(survey_trees = FALSE) %>% 
  # mutate(across(all_of(col2cvt_0), ~ replace_na(.x, 0)))
  mutate(across(all_of(col2cvt_0), ~ ifelse(is.na(.x) & survey_trees, 0, .x))) %>% 
  select(-survey_trees)
# mutate_at(col2cvt_0, ~ ifelse(is.na(.) & survey_trees, 0, .))
# across() is de modernere vervanger van mutate_at() en wordt aanbevolen

t <- plot_specieslevel_3 %>% 
  anti_join(plot_specieslevel_1 %>% select(period, plot_id, species))
summary(t) 
# OK, allemaal nul nu
# ook allemaal survey_trees = T
```


# Finale analyseset

Op plotniveau en op plot-soortniveau.

```{r results='hide'}
carbon_by_plot <- plot_level2 %>% 
  select(-survey_deadw, -survey_lis) %>% 
...
```{r}
carbon_by_plot_species <- plot_specieslevel_3 %>% 
  select(-survey_deadw, -survey_lis) %>% 
  left_join(plotinfo %>% select(plottype, plot_id, year = year_dendro, period)) %>% 
  select(plottype, plot_id, year, period, everything())

names(carbon_by_plot_species)
```
...
Wegschrijven trees_standing8 als `stems_all_parts`.
...
stems <- list(
      stems_all_parts = trees_standing8
    )
```

```{r join_plottype_surveynr_BR_stems}
stems_plotinfo <- lapply(stems
...
# Export plot(-species)-level-data

carbon_by_plot en carbon_by_plot_species

**!!!! ENKEL PLOTS EXPORTEREN WAARVAN DATA PROCESSED ZIJN!!**
Is standaard parameter bij inladen dendro-data 

```{r new_list_carbon_plot}
# opnieuw list van maken om te exporteren en plotinfo aan te koppelen
carbon <- list(
      carbon_by_plot = carbon_by_plot
      , carbon_by_plot_species = carbon_by_plot_species
    )
```

```{r join_plottype_surveynr_BR_plot}
...
```{r save_carbon_plot_git, eval = FALSE}
...
```{r save_carbon_plot_access_csv, eval = FALSE}
# hier wel plotinfo (plottype, BR, surveynr mee wegschrijven)
...
```{r save_carbon_plot_gdrive, eval = FALSE}
save_results_csv(
    results = carbon_plotinfo,
...
source_file <- here::here(paste0("Scripts/OpbouwDatabank/Main_UpdateForresdat_06_volume_tree_parts_carbon_stock.html"))
