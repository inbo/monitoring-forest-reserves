...
plot_species_level0 <- trees_standing8 %>%
...
```{r results='hide'}
names(plotinfo)
names(plot_species_level0)
names(zero_volume_bosplots)
zero_volume_bosplots$species <- NA

# combine
plot_species_level1 <- rbind(plot_species_level0
                          , zero_volume_bosplots %>% 
                            select(-survey_trees, -year_dendro, -data_processed))
# 2009
...
# 160 extra records/plots waar deadwood = 0 of NA (LIS) of uit monitoringrapport
...
summary(logs_per_plot_species2)
summary(logs_per_plot_species1)
```
...
names(plot_species_level1)

nrow(logs_per_plot_species2); nrow(plot_species_level1); nrow(dendro_by_plot_species)
...
t <- logs_per_plot_species2 %>%
  anti_join(plot_species_level1, by = c("plot_id", "period", "species"))
# => dubbels
t <- logs_per_plot_species2 %>%
...
nrow(plot_species_level1) == nrow(logs_per_plot_species2) - 13
# anders worden NA's toegevoegd 
# ! checken of dat echt NA's zijn, of eerder nullen?


plot_species_level2 <- plot_species_level1 %>% 
  full_join(logs_per_plot_species2, by = c("plot_id", "period", "species")) 

names(plot_species_level2)
nrow(plot_species_level2) == nrow(plot_species_level1)
```
...
```{r}
carbon_by_plot_species <- plot_species_level2 %>% 
  select(-survey_deadw, -survey_lis) %>% 
  left_join(plotinfo %>% select(plottype, plot_id, year = year_dendro, period)) %>% 
  select(plottype, plot_id, year, period, everything())
