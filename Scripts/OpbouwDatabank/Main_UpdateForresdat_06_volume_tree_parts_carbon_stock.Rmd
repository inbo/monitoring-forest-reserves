...
Dat zouden we moeten doortrekken voor de LIS-opnames: Wijnendale, Everzwijnbad, 
Heirnisse en Jansheideberg periode 3
...
t3 <- logs_per_plot1 %>% 
  filter(is.na(vol_log_m3_ha))
t3 <- logs_per_plot1 %>% 
  filter(is.na(vol_log_m3_ha) & is.na(vol_lis_m3_ha)) %>% 
  left_join(plotinfo)
...
logs_per_plot_extra_ <- logs_per_plot_extra %>% 
  ...
  select(forest_reserve
         ...
         , biomass_lying_t_ha_dead, biomass_lying_above40cm_t_ha_dead
         , carbon_lying_t_ha_dead, carbon_lying_above40cm_t_ha_dead
...
plot_level2 <- plot_level1 %>% 
  ...
  mutate(vol_agb_m3_ha_dead = ifelse(!is.na(vol_log_m3_ha)
                     , vol_tot_standing_agb_m3_ha_dead + vol_log_m3_ha
                     , ifelse(!is.na(vol_log_above30cm_m3_ha) & survey_lis
                              , vol_tot_standing_agb_m3_ha_dead + 
                                vol_log_above30cm_m3_ha + 
                                vol_lis_10_30cm_m3_ha
                              , vol_tot_standing_agb_m3_ha_dead + vol_lis_m3_ha)
                     )
         , biomass_agb_t_ha_dead = biomass_standing_t_ha_dead + biomass_lying_t_ha_dead
         , carbon_agb_t_ha_dead = carbon_standing_t_ha_dead + carbon_lying_t_ha_dead)

names(plot_level2)
nrow(plot_level2) == nrow(plot_level1)
```
> beter geen som maken van LIS en FAS om tot totaal volume t ekomen
iedereen dat zelf laten doen, duidelijker
...
names(plot_species_level_species2)
nrow(plot_species_level_species2) == nrow(plot_species_level_species1)
```
...
# Info mbt biomassa liggend dood hout
...
Voor meer info zie script `jansheideberg_dendro_2025.Rmd`.      
Immers correctie nodig voor       

- `vol_log_m3_ha`, `biomass_lying_t_ha_dead`, `carbon_lying_t_ha_dead` corrigeren: NA voor LIS-plots     
- Extra velden toevoegen `vol_log_above_30cm_m3_ha`, `biomass_lying_above_30cm_t_ha_dead`, `carbon_lying_above_30cm_t_ha_dead`       
