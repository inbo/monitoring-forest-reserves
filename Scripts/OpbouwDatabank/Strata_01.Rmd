...
- Management_type: Kluisbos productiebos
- plot_unmanaged_since: Kluisbos productiebos; sommige plots 
- reserve_unmanaged_since: enkele foutjes

> moet nog aangepast worden, TSA niet steeds correct 
Kris gaat afstemmen tss tabel en Methodiekrapport (zie mail van 5/12/2024)
...
We schrijven beide file sweg naar de straat-db.
...
Zie onderstaande code (in comment) uit script "Wildcard_Flanders_plot_data.Rmd".


```{r results = 'hide', eval = FALSE}
names(link_WC_FM)
# forest_reserve_FM_adjusted: werd aangemaakt om één op één link te hebben
# als we deze tabel willen gebruiken, moeten we dus in onze code forest_reserve
# vervangen door forest_reserve_FM_adjusted, met behulp van strata_soil
...
Info overgenomen uit methodiekrapport.
...
con <- odbcConnectAccess2007(path_to_strata_db)
  management_methodiekrapport <- sqlFetch(con, "info_forest_reserve_management", stringsAsFactors = FALSE)  # reserve establishment date
odbcClose(con)
...
> Kris gaat `00_referentiefile-meestcompleet-Bosreservaten_allecirkels.gsheet` 
corrigeren en beter afstemmen tss tabel (`plot_unmanaged_since`) en Methodiekrapport (`reserve_unmanaged_since`) 
(zie mail van 5/12/2024)
Nu nog even verder doen met zoals nu is.
Weliswaar nog fouten mbt "onbeheerd sinds" van o.a. Harras en CP's die in oude KV (1983) liggen 
! op reservaatsniveau in tabel info_EUFORIA: year_reserve en year_abanonment
moet ook eens afgeklopt worden => overleg op 6/3/25
...
- Management_type == "Non-intervention" (< gsheet Kris VdK)  
...
strata_overige_ <- strata_overige %>% 
  ...
  left_join(management_methodiekrapport %>% rename(reserve_unmanaged_since = unmanaged_since), by = c("forest_reserve_fieldmap" = "forest_reserve", "plottype")) %>% 
...
plotdetails2 %>% filter(data_processed) %>% nrow() == 1892 # 1725
plotinfo %>% filter(!is.na(survey_number)) %>% nrow() == 1892 # 1725
...
strata <- list(
      ...
      info_forest_reserve_EUFORIA = info_EUFORIA,
...
sqlDrop(con, "info_forest_reserve_EUFORIA")
sqlSave(con, LINK_EUFORIA_wildcard, "info_forest_reserve_EUFORIA")
