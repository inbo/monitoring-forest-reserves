...
con <- odbcConnectAccess2007(path_to_strata_db)
  management_methodiekrapport <- sqlFetch(con, "forest_reserve_management", stringsAsFactors = FALSE)  # reserve establishment date
odbcClose(con)
...
- Everzwijnbad: op niveau van plots doorgegegevn aan Euforia --> 1995 ipv 2000
- Sevendonck Alnus en Quercus samen bij Euforia --> 1995 (wij Alnus: 1987)
...
check <- info_EUFORIA %>% 
  select(forest_reserve_FM_adjusted, plottype, year_reserve
         , year_abandonment) %>% 
  left_join(management_methodiekrapport %>% select(forest_reserve_FM_adjusted, plottype
...
check %>% filter(year_abandonment != plots_unmanaged_since) %>% 
  ...
  filter(plottype == "CP")
# wij hebben sevendonck Alnus afgesplitst van Sevendocnk Quercus, volledig reesrvaat idd 1995

check %>% filter(year_abandonment != reserve_unmanaged_since) %>% 
  select(-year_reserve, - reserve_establishment_yr) %>% 
  rename(year_abandonment_EUFORIA = year_abandonment) %>% 
  filter(plottype == "CP")
# Everzwijnbad: op niveau van plots doorgegegevn aan Euforia: OK
```
...
strata_bodem <- df_strata_shp %>%
  select(plot_id, 65:111) %>%    # shp via vpn
