...
strata_klimaat <- df_strata_shp %>%
  ...
  select(all_of(basic_variables), contains(c("Regen", "Temp")))
...
strata_bodem <- df_strata_shp %>%
  ...
  select(basic_variables, 65:110) %>%    # shp via vpn
...
dominant_tree_ %>% filter(MaxStamtal == 0 & !is.na(MaxStamtalSpeciesID)) %>% nrow() == 0
# JA
