...
strata_klimaat <- df_strata_shp %>% 
  filter(!Unieknr %in% c(32000, 11001:11006)) %>% 
...
**"Unmanaged_since" uit methodiekrapport is verschillend van info uit gsheet**

Beide behouden, andere naam geven: `plot_unmanaged_since` en `reserve_unmanaged_since` .

Overlegd met team BR en afgeklopt op 20/3/2025.
...
**Zoniën (mail van Yannick aan mij 2/12/2024)**

(c) Kris: Voor zonien zijn de aparte stukken ook aparte 'sites' in de betekenis van Wildcard (andere boomsoortensamenstelling en TSA). De kernvlakte (= paarse polygoon met bufferzone (dus tot aan de grens en de paden) en de 11 plots die er in liggen zijn dus ook een aparte site. het roze driehoekje is twee jaar na het oorspronkelijke bosreservaat toegevoegd aan Kersselaerspleyn, maar naar TSA en beheer maakt dat geen verschil: dat mag dus mee met de zone eronder.
Je hebt dus 4 'sites' :      

- het oorspronkelijke reservaat (= TSA 40 jaar),    
- het stuk links ervan en eronder (beide beukgedomineerd en TSA 28 jaar),    
- het eikgedomineerde Harras (TSA ook 28 jaar) en    
- het donkerbruine deel = uitbreiding 2010 (TSA ca 15j)   
...
```{r strata_overige_gsheet, results='hide'}
names(input_gsheet)
...
```{r}
table(strata_overige_gsheet$Management_type)
table(strata_overige_shp$Management)
# table(strata_overige_shp$Beheer)  # zelfde als "Management", maar nederlands
...
Uit methodiekrapport ook nog info over `unmanaged_since` en `establishment_yr`
...
strata_overige_ <- strata_overige %>% 
  ...
  left_join(management_methodiekrapport %>% select(-plots_unmanaged_since), by = c("forest_reserve_fieldmap" = "forest_reserve", "plottype")) %>% 
  select(-plottype, -Id) %>% 
...
# een deel van Kluisbos is productiebos (maar alles onder zelfd enoemer in fieldmap)
...
strata_overige_ <- strata_overige %>% 
  mutate(reserve_unmanaged_since = ifelse(SubArea %in% c("Kluisbos Communal Forest", "Kluisbos State Forest")
         ...
         , last_commercial_intervention = ifelse(SubArea == "Kluisbos SFR" |
...
strata_overige_ <- strata_overige %>% 
  mutate(reserve_unmanaged_since = ifelse(forest_reserve_fieldmap == "Ename" &
                                            Management != "unmanaged"
                                          , NA, reserve_unmanaged_since)
         , last_commercial_intervention = ifelse(forest_reserve_fieldmap == "Ename" &
                                            Management != "unmanaged"
