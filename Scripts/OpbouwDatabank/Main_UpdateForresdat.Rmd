...
naar **/PRJ_BOSECO_ALGEMEEN/PRJ_BR_AanvraagGegevens/!METADATA-ALL_PLOTS/forresdat_data/csv/"**

```{r plotinfo_save_gdrive, eval = FALSE}
output_gdrive  <- "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dQ2w5ZFhSZEpCU0E/PRJ_BOSECO_ALGEMEEN/PRJ_BR_AanvraagGegevens/!METADATA-ALL_PLOTS/forresdat_data/csv/"

# test <- read.csv2(paste0(output_gdrive, "dendro_by_diam_plot.csv"))

save_results_csv(
    results = list(plotinfo = plotinfo),
    output_dir = output_gdrive
    )
...
## !! check calc_volume deadwood

### CalcVolume berekenen in FM/access - voorlopig OK 20/9/2022
De volumeberekening van liggen dood hout baseert zich op het veld `calc_volume_m3`, 
dat een kopie is van Volume_m3.
Belangrijk is dat het volume_m3 berekend wordt in FM-DC als "inside plot".

Voor 3de set was calc_volume_m3 niet steeds berekend (in acces).
Nu (19/9/2022) mbv query (qupdAnja_deadwood_CalcVolume_3eSET) in access bijgewerkt.
**Idealiter gebeurt dit met een updatequery in R (op termijn te programmeren).**


```{r check_dead1}
check_deadw <- data_deadwood %>% 
  filter(is.na(calc_volume_m3))
# nog maar 2 (intact, plot 43.000, nog te programmeren mbv tarieven dagnelie)
...
### Nulwaarde ipv NA - voorlopig OK 20/9/2022
Logs_by_decay_plot: nemen geen NA-waardes op => OK
...
Plot 475, periode 1, bevat geen bomen en is niet opgenomen in dendro_by_plot.
Hoewel het niet zinvol is om in elke dataset nullen toe te voegen (bv. per_species), 
leek het me zinvol dit bij dendro_per_plot wel te doen.
...
al toegevoegd zijn.
Het gaat dus enkel nog om de staande bomen waarbij we een , dus

In geval van dendro_by_plot zelf de ene plot zonder bomen toevoegen 
(!! misschien zijn er wel nog, maar kwam deze eruit bij analyse van 2 surveys; 
ev. nog waar maar één survey is, of bij 3de decade ...).

!!er is wel deadwood opgemeten, waarom staat dat niet in dendro_by_plot?
Gecheckt en left_join ipv full_join: correctie voorgesteld in forrescalc (21/11/2022).

Als dit opgelost is, is onderstaande code niet meer nodig.
!! tenzij er toch nog plots zijn waar géén liggend deadwood is en géén bomen staan.

Misschien gewoon heel simpel: obv plotinfo survey_trees en survey_deadw nulwaarden toevoegen.
...
## Correctie deadwood 0 ipv NA - voorlopig OK 20/9/2022

!! deadwood bv. plot 11000: period 1 en 0 niet opgemeten => moet NA worden ipv 0
