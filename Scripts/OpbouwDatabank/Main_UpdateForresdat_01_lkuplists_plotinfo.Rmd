---
title: "Update forresdat 1 - lookuplists en plotinfo"
author: "Anja Leyman"
date: "`r Sys.Date()`"
output: 
  html_document:
...
- forresdat-folder (nadien nog te pushen naar github - develop branch!!!!!)
...
Als we info willen over nog niet verwerkte data, moeten we mbv `load_plotinfo()` 
...
- 4/6/2025: foutje plot 414 tresholds
...
- mrt 2026: idem, met toevoeging van Terrijst en Coolhem
...
Forresdat opnieuw opgevuld op 15/12/2024
...
Lookuplijsten pushen naar develop branch in forresdat: 


## Vanuit forresdat naar analysedb - ok

Functie `from_forresdat_to_access()` is gebaseerd op de functies `read_forresdat()` en `read_forresdat_table()`. 
Deze kunnen ook data binnenhalen van de develop branch.
...
from_forresdat_to_access(tables = c("qalive_dead", "qbranch_lenght_reduction",  
                                    ...
                                    "qplot_type", "qspecies" , "qvdagn_vfm", "ql_iangle"),
...
con <- odbcConnectAccess2007(path_to_fieldmap_db)
  ...
  qLIangle <- sqlFetch(con, "qLIangle", stringsAsFactors = FALSE)
...
```{r plotinfo_load_all}
plotinfo_0 <- load_plotinfo(database = path_to_fieldmap_db, processed = FALSE) %>% 
...
nrow(plotinfo) # 2629 / 2628 / 2721

plotinfo %>% 
  filter(survey_trees == FALSE & data_processed == TRUE) %>% nrow() == 0
t <- plotinfo %>% 
  filter(survey_trees == TRUE & data_processed == FALSE) %>% nrow()
nrow(t)  # 0
...
# table(plotinfo$forest_reserve, plotinfo$data_processed)
# Zoniën UITBR_1995: altijd één plot niet-processed = 1999 CA (Rogister+)
# wijnendale: 4 plots
# 10/3/2025: Coolhem en Bos Terrijst: periode 3 zit er al in, maar nog niet verwerkt, klopt
...
TR Liedekerke 3de decade (plot 131000: nog geen forest_reesrve ingevuld)
...
plotinfo %>% filter(is.na(forest_reserve)) %>% nrow() == 0
nrow(plotinfo) # 2023 / 2075
```

```{r tmp_check_lis_survey, eval = FALSE}
# check waar survey_lis niet correct ingevuld staat 
# functie load_plotinfo() zet een NA om naar FALSE
plotinfo %>% filter(survey_lis == TRUE) %>% 
...
t <- plotinfo %>% 
  group_by(forest_reserve, plottype, period, survey_lis) %>% 
  summarise(n_lis = sum(survey_lis)
            , n_niet_lis = sum(!survey_lis)) %>% 
  ungroup()


plotinfo %>% filter(is.na(survey_lis)) %>% 
  group_by(forest_reserve, plottype, period) %>% 
  summarise(n = n()) %>% 
  ungroup()

# gevraagd aan Peter (mail 16/12/2025):
# plotdetails_2eSET: Lineintersect staat op "ja" bij volgende plots:
#     Kluisbos, CPs
#     Zoniën KERN_1983
#     Zoniën UITBR_1995
```
...
Grens ligt op 1/5.
Soms dendro effectief in twee verschillende groeiseizoenen opgemeten, maar dat is OK
...
# Correctie plotinfo Kersselaerspleyn
...
plotinfo_ <- plotinfo %>% 
  dplyr::mutate(survey_deadw = ifelse(forest_reserve == "Zoniën UITBR_1995" & plottype == "CP" 
                    ...
                    , TRUE
                    , survey_deadw)
                # extra tov laatste update (nog niet gerund (27/2/2026))
                , diam_min_a4_logs = ifelse(forest_reserve == "Zoniën UITBR_1995" & plottype == "CP" 
                    & period == 1 & survey_trees
                    , 100
                    , diam_min_a4_logs))
...
25/3/2026 vergeleken met data op git (17/12/2025)

```{r eval = FALSE, results='hide'}
# 
plotinfo_git <- read_forresdat_table(tablename = "plotinfo"
                                           , join_plotinfo = FALSE
                                           , plottype = "all"
                                           , git_ref_type = "branch"
                                           , git_reference = "develop")
...
# vgl
plotinfo_git <- plotinfo_git %>% 
  select(plot_id, year_dendro, everything()) 
# %>% 
#   select(names(dendro_by_plot))    # vol_log_min40cm van naam veranderd!
nrow(plotinfo) == nrow(plotinfo_git)
names(plotinfo) == names(plotinfo_git)
waldo::compare(plotinfo, plotinfo_git)

plotinfo %>% select(-year_dendro) %>% arrange(plot_id, period) == plotinfo_git %>% select(-year_dendro) %>% arrange(plot_id, period)
# OK denk ik andere ordening van variabelen (3/2025)
# 12/2025: andere namen,  niet OK dus
plotinfo == plotinfo_git

head(plotinfo %>% arrange(desc(plot_id), period)); head(plotinfo_git %>% arrange(desc(plot_id), period))
# lijkt OK

t <- plotinfo %>% 
  anti_join(plotinfo_git)

head(plotinfo %>% arrange(desc(plot_id), period) %>% filter(plot_id == 830)); 
head(plotinfo_git %>% arrange(desc(plot_id), period) %>% filter(plot_id == 830))

plotinfo %>% filter(forest_reserve == "Zoniën UITBR_1995" & plottype == "CP" 
                    & period == 1); plotinfo_git %>% filter(forest_reserve == "Zoniën UITBR_1995" & plottype == "CP" 
                    & period == 1)


w <- waldo::compare(plotinfo %>% filter(forest_reserve == "Zoniën UITBR_1995" & plottype == "CP" 
                    & period == 1), plotinfo_git %>% filter(forest_reserve == "Zoniën UITBR_1995" & plottype == "CP" 
                    & period == 1))
# afronding is anders 
...
Na wegschrijven naar forresdat, moet "forresdat.Rproj" geopend worden en 
commits gepusht naar github.
...
source_file <- here::here(paste0("Scripts/OpbouwDatabank/Main_UpdateForresdat_01_lkuplists_plotinfo.html"))
