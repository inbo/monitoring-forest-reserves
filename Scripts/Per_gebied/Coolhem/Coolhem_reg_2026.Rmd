...
nrow(checkA1) # 295 - allemaal periode 1 - hier wordt dan een exact aantal gebruikt ipv een klasse
...
species_NA <- data_regeneration %>% filter(is.na(species) | is.na(height_class)) %>% 
  mutate(name_plot = paste0(plot_id, "_", subplot_id))
nrow(species_NA) 
# 1 

```
Voor `r forestreserve_short` gaat het om volgende subplots:
`r species_NA$name_plot`, resp. uit periode `r species_NA$period`.
...
reg_by_plot_height_species %>% filter_at(col, any_vars(is.na(.))) %>% nrow()  
