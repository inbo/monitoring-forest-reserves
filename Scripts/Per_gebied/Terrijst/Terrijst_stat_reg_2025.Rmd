...
subtitle: "Jansheideberg - verjonging"
author: "Anja Leyman"
date: "`r Sys.Date()`"
output:
  html_document:
...
```{r eval = FALSE}
reg_by_plot %>% filter(plot_id == 808) %>% view()
```
...
```{r eval = FALSE}
reg_by_plot_height %>% filter(plot_id == 818) %>% view()
```
...
```{r eval = FALSE}
dataset2_0 %>% filter(plot_id == 808) %>% view()

dataset2_0 %>% filter(plot_id == 818) %>% view()
```
...
```{r eval = FALSE}
reg_by_plot_species %>% filter(plot_id == 808) %>% view()
```
...
```{r results = 'hide'}
dataset4_0 %>% filter(plot_id == 808) %>% view()
```
...
nrow(resultaat4); n
# in zelfde grootte-orde, OK, niet elke soort komt in de drie peridoes voor
```
...
```{r results = 'hide'}
reg_by_plot_height_species %>% filter(plot_id == 808) %>% view()
```
...
```{r results = 'hide'}
dataset3_0 %>% filter(plot_id == 808 & species == 87) %>% view()
dataset3_0 %>% filter(plot_id == 808 & height_class == 4000) %>% view()
```
