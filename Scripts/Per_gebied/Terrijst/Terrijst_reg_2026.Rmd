...
Plotinfo samenstellen, zijnde plottype, naam forest_reserve en info over survey 
en data al dan niet processed.
...
# twee keer aantal vergeten invullen subplot 47 en 65 van de kernvlakte
# gemaild naar Peter maar kan wellicht niet meer aangevuld worden
...
```
Voor `r forestreserve` gaat het om volgende plots:
...
range(check_rubbing_perc$rubbing_damage_perc, na.rm = T)

check_rubbing_perc %>% filter(is.na(rubbing_damage_perc)) %>% nrow() == 0
# 2 subplots waar aantal van reg niet ingevuld is (zie ook hoger) 
...
# allebei te maken met missing number, maar bij de ene is er maar één soort genoteerd
# en bij ander subplot drie soorten, en dan wordt verder gerekend met de soorten die
# wel een antal toegekend kregen (plot 65 en 47)
```
...
# 1, wegens missing number in subplot 65
...
check_rubbing_perc_NA %>% nrow() == 0
# door een NA bij hoogteklasse 1000 in de KV, ontstond er een extra record met 
...
# subplot 65 en 47 ander effect van missing number, want bij subplot 65 geen andere soorten
check_subplot65 <- reg_by_plot_height %>% 
  left_join(plotinfo %>% select(plot_id, period, game_impact_reg)) %>%
  filter(plot_id == 111000 & height_class == 1000 & period == 3 & subplot_id == 65) %>% 
  filter(game_impact_reg == TRUE) 

check_subplot65 <- reg_by_plot %>% 
  left_join(plotinfo %>% select(plot_id, period, game_impact_reg)) %>%
  filter(plot_id == 111000 & period == 3 & subplot_id == 65) %>% 
  filter(game_impact_reg == TRUE) 

check_subplot65 <- reg_by_core_area_height_species %>% 
  left_join(plotinfo %>% select(plot_id, period, game_impact_reg)) %>%
  filter(plot_id == 111000 & height_class == 4000 & period == 3) %>% 
  filter(game_impact_reg == TRUE) 
# enkel approx_nr_reg vertaald NA door naar plotniveau
# min, lci en uci rekenen gewoon alsof de NA er niet is
```
...
reg_by_plot_height %>% filter_at(col, any_vars(is.na(.))) %>% nrow() # FALSE
# mean, lci en uci soms wel en soms niet NA, afh. of er nog andere soorten waren in
# die hoogteklasse
# hier ook mean en lci, uci als NA , net als approx_nr_reg
# wellicht omdat er van die soort en hoogteklasse maar één (NA-)record was
...
# hier NIET mean en lci, uci als NA, maar gewoon verder gerekend zonder de NA
# blijkbaar approx_nr_established_ha en approx_nr_seedlings_ha wél berekend met weglating van de NA's
# " en als het effectief approx_nr_reg is NIET"
```
...
reg_by_plot_height_species %>% filter_at(col, any_vars(is.na(.))) %>% nrow()  # FALSE
# hier ook mean en lci, uci als NA , net als approx_nr_reg
# wellicht omdat er van die soort en hoogteklasse maar één (NA-)record was
```
...
reg_by_core_area_species %>% filter_at(col, any_vars(is.na(.))) %>% nrow() == 0 # TRUE
# approx_nr_established_ha en approx_nr_seedlings_ha wél berekend met weglating van de NA's
# terwijl approx_nr_regeneration_ha NIEt (zie chunck hierna)
...
# twee subplots zonder aantal => enkel approx_nr_regeneration_ha NA

```

> er wordt anders omgegaan met NA's bij approx_nr_established_ha en approx_nr_seedlings_ha
tegenover approx_nr_regeneration_ha
de eerste laten NA's gewoon vallen, de tweede wordt NA als er één waarde ontbreekt

Zie issue #178.

Idealiter vallen NA's enkel weg als er andere data zijn, dus niet als er geen andere 
data zijn, dan blijft resultaat best NA ipv "0"
Zo is het ook geprogrammeerd voor intervals mbv functie `sum_intervals()`:
```
  if (na_rm) {
...
source_file <- here::here(paste0("Scripts/Per_gebied/", forestreserve_short, "/", forestreserve_short, "_reg_2026.html"))
