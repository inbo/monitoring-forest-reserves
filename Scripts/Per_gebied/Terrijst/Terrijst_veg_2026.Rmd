...
totalcover_missing <- data_vegetation %>% 
  ...
  filter(!(is.na(total_soildisturbance_game_id) & period == 1))

nrow(totalcover_missing)
# eerst enkel 148 records van periode 1 met total_soildisturbance_game_id = NA
```
...
#229
```
**`r nrow(totalcover_20)` records met waterlayer cover of soildisturbance "Niet beschikbaar". **
**Allemaal periode 1 of 2.**
...
```{r results= 'hide'}
# NA
data_vegetation %>% filter(is.na(total_soildisturbance_game_id) & period != 1) %>% nrow() == 0
...
source_file <- here::here(paste0("Scripts/Per_gebied/", forestreserve_short, "/", forestreserve_short, "_veg_2026.html"))
