...
veg_by_plot <- read_excel(
  paste0(path_to_plotdata_teamdrive, "veg_by_plot.xlsx")
  ) %>% 
  select(-1) %>%   
  # select(-contains("eg"), -contains("min40cm")) %>%  # (veg en reg)
  left_join(plotinfo) %>% 
  filter(plottype == "CP") %>% 
  rename(year = year_main_survey)  
    # "main_survey": zie issue #179: beter overal vervangen door year

herblayer_by_plot <- read_excel(
  paste0(path_to_plotdata_teamdrive, "herblayer_by_plot.xlsx")
...
Nu enkel op soortniveau gegevens gegenereerd (zie verder)
...
write.xlsx(resultaat
           , paste0(path_to_plotdata_pc, "stat_veg.xlsx")
           , sheetName = "veg_by_plot"
           , showNA = FALSE
           , append = FALSE) 

write.xlsx(resultaat
           , paste0(path_to_stats_teamdrive, "stat_veg.xlsx")
           , sheetName = "veg_by_plot"
           , showNA = FALSE
           , append = FALSE) 
```
