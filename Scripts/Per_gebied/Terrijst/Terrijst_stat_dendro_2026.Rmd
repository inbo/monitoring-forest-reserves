...
subtitle: "Terrijst - dendrometrie"
author: "Anja Leyman"
date: "25/3/2026"
output:
  html_document:
...
plotinfo <- read_excel(paste0(path_to_plotdata_teamdrive, "plotinfo.xlsx")) %>% 
   select(forest_reserve, plot_id, period
          , contains("survey"), game_impact_veg
...
inlezen_functie <- function(tab, info = plotinfo) {
  ...
  if ("species" %in% colnames(tabel)) {
    ...
    if ("vol_lis_above30cm_m3_ha" %in% colnames(tabel)) {
    tabel <- tabel %>% 
      select(-vol_lis_above30cm_m3_ha)
    }
...
write.xlsx(resultaat
           , paste0(path_to_plotdata_pc, "stat_dendro.xlsx")
           , sheetName = "dendro_by_plot"
           , showNA = FALSE
           , append = FALSE) 

write.xlsx(resultaat
           , paste0(path_to_stats_teamdrive, "stat_dendro.xlsx")
           , sheetName = "dendro_by_plot"
           , showNA = FALSE
           , append = FALSE) 
```
...
Makkelijkste om dataset2_0 te corrigeren, want daar kunnen we nog link leggen 
met `lis_survey.`
...
resultaat2 <- create_statistics(
  ...
  include_year_range = TRUE,
  na_rm = TRUE,   # moet TRUE zijn voor stems_per_tree, is NA als soort niet voorkomt 
  # in plot en zodra er één NA is, wordt som/gemiddelde ook NA, terwijl er enkel 
  # moet geteld worden met records/plots waar er een waarde is
  # ook OK voor LIS, want daar in een bepaalde periode geen enkele meting en 
  # dan wordt som/gemiddelde van "niks" een NA en dat is OK
...
```{r add_zeros_all_decaystages, results = 'hide'}
names(dataset5)
...
```{r add_zeros_all_decaystages_all_species, results = 'hide'}
names(dataset6)
...
source_file <- here::here(paste0("Scripts/Per_gebied/", forestreserve_short, "/", forestreserve_short, "_stat_dendro_2026.html"))
