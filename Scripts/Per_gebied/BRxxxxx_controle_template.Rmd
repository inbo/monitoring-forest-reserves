...
Withoefse heide zit reeds in de moederdatabank (versie 20241119).

```{r Path_tmp}
path_to_fieldmap <- paste0(path_to_databases, "MDB_BR_X8_inbo2020_20241119/")
# nu al een meer recente versie, na datacontrole
```
...
Zie [overleg van 19/11/2024](https://docs.google.com/document/d/1tvvoDA0hTfdZEiPEzvKvHAuZnXbTZLCt?rtpof=true&usp=drive_fs)

(oa Growth: obv outliers: !! teveel die toch OK zijn
o	Voorstel Peter: Amerikaanse eik (net als Populier) niet vergelijken met de andere soorten  OK, in forrescalc opgenomen
o	Negatieve groei en nu dood: dat is OK, zou niet als fout mogen aangeduid staan
▪	© Anja: in forrescalc wordt enkel naar de boxplots gekeken, geen onderscheid neg. of pos. afwijking =>  zelf in R-script info over levend en dood, en boomsoort koppelen aan het overzicht + wegfilteren waar Peter niet hoeft naar te kijken
)

<br>

Peter op 26/11/24 manuele datacheck oldschool op recente data Terrijst,
na inmeten laatste cirkel daar, dan nog hoogtemetingen en check on the field,
als ik klaar bent met aanpassingen geautomatiseerde datacontrole dan zou deze set een
check kunnen zijn voor mijn scripts,
bij deze zouden er  toch veel minder fouten moeten uitrollen hoopt Peter ....

=> ik zou ook een deel van de "growth"-errors best al uitfilteren,
bv. enkel de "outlier_diameter/height" (outliers van én soort én tov alle bomen),
niet de _total en niet _species (! nog te bekijken)
...
Daarnaast ook **same place not coupled** zelf al bekijken: controlescript van Els
kijkt enkel naar old_id, niet naar coppice_id
Op termijn willen we af van die coppice-id, dus enerzijds moeten we ons er niet druk mer over maken, 
maar anderzijds hebben we dat wel toch nodig om de overstap te zetten naar stoven met levend en dood deel,
en dan zou 't fijn zijn als dat in orde staat.
Bij Heirnise laatste check (26/11/2024) waren er toch nog drie dode stoven waar volgens mij
de coppice-id niet OK was.
Maar Heirnisse is wel heel veel hakhout ...

Bij data-aanlevering zeg ik steeds dat er geen (goede) link is bij hakhout doorheen de tijd
enkel een link bij individuele bomen.
...
? nog nut om missing values weer te geven voor periode 1 en 2?
...
OPGEPAST: in Heirnisse - periode 1 - nog zeer veel voorkomend (1817 records).
...
<!-- 2 records: alive en code 40 (NVT - dood)  -->
...
In `r forestreserve` gaat het vooral om 2 walkers, 1 species shifters en 1 decaystage.
...
●	Growth: obv outliers: !! teveel die toch OK zijn
o	Voorstel Peter: Amerikaanse eik (net als Populier) niet vergelijken met de andere soorten  OK, in forrescalc opgenomen
o	Negatieve groei en nu dood: dat is OK, zou niet als fout mogen aangeduid staan
▪	© Anja: in forrescalc wordt enkel naar de boxplots gekeken, geen onderscheid neg. of pos. afwijking =>  zelf in R-script info over levend en dood, en boomsoort koppelen aan het overzicht + wegfilteren waar Peter niet hoeft naar te kijken
...
```{r eval = FALSE}
# Specify the path of the file you want to copy
source_file <- here::here(paste0("Scripts/Per_gebied/Withoefse_heide/", "Withoefse_heide_controle_nov2024.html"))
