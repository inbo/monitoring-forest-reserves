...
plot411 <- data_regeneration %>% filter(plot_id == 411)
```
...
table(reg_by_plot_height$period, reg_by_plot_height$height_class, reg_by_plot_height$plottype)
...
```{r eval = FALSE}
reg_by_plot_git <- read_forresdat(tablename = "reg_by_plot"
                                     , join_plotinfo = TRUE) %>% 
  filter(forest_reserve == forestreserve)

reg_by_plot_new <- reg_by_plot %>% filter(period != 3)
...
reg_by_plot_new %>% select(-year) %>% arrange(plot_id, period) == reg_by_plot_git %>% select(-year) %>% arrange(plot_id, period)
# OK denk ik andere ordening van variabelen
reg_by_plot_new == reg_by_plot_git
...
```{r eval = FALSE}
reg_by_core_area_species_git <- read_forresdat(tablename = "reg_by_core_area_species"
                                     , join_plotinfo = TRUE) %>% 
  filter(forest_reserve == forestreserve)

reg_by_core_area_species_new <- reg_by_core_area_species %>% filter(period != 3)

reg_by_core_area_species_git <- reg_by_core_area_species_git %>% 
  select(plot_id, year, everything()) 

nrow(reg_by_core_area_species_new) == nrow(reg_by_core_area_species_git)
summary(reg_by_core_area_species_new)

names(reg_by_core_area_species_new) == names(reg_by_core_area_species_git)
summary(reg_by_core_area_species_git)

diff <- reg_by_core_area_species_new %>% anti_join(reg_by_core_area_species_git, by = c("plot_id", "period"))
diff

names(reg_by_core_area_species_new); names(reg_by_core_area_species_git)
reg_by_core_area_species_new %>% select(-year) %>% arrange(plot_id, period) == reg_by_core_area_species_git %>% select(-year, -year_dendro) %>% arrange(plot_id, period)
# OK denk ik andere ordening van variabelen
reg_by_core_area_species_new == reg_by_core_area_species_git %>% select(-year_dendro)

head(reg_by_core_area_species_new %>% arrange(plot_id, period)); head(reg_by_core_area_species_git %>% arrange(plot_id, period))
head(reg_by_core_area_species_new %>% arrange(desc(plot_id), period)); head(reg_by_core_area_species_git %>% arrange(desc(plot_id), period))
