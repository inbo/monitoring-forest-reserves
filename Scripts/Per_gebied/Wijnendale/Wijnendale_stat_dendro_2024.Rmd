...
resultaat2 <- create_statistics(
  ...
  # en als NA's behouden blijven, dan is resultaat ook NA 
...
dataset5 <- deadw_by_decay_plot %>% 
  filter(decaystage_cde != 16 | is.na(decaystage_cde))
...
dataset5_0 <- add_zeros(dataset = dataset5 %>% 
                          select(forest_reserve, plot_id, period, decaystage_cde, contains("_ha")),
                        comb_vars = c("plot_id", "decaystage_cde", "period"),
                        grouping_vars = c("forest_reserve")
                        ) %>%
...
n_decay <- length(unique(dataset5$decaystage_cde))
...
dataset6_0 <- add_zeros(dataset = dataset6 %>% 
                          select(forest_reserve, plot_id, period, decaystage_cde, species, contains("_ha")),
                        comb_vars = c("plot_id", "decaystage_cde", "species", "period"),
                        grouping_vars = c("forest_reserve")
                        ) %>%
  left_join(qDecaystage, by = c("decaystage_cde" = "ID")) %>% 
  rename(afbraak = Value1, 
...
n_decay <- length(unique(dataset6$decaystage_cde))
...
source_file <- here::here("Scripts/Per_gebied/Wijnendale/Wijnendale_stat_dendro_2024.html")
