...
4 plots zijn "not processed": 400, 424, 426 en 482

Alle zijn ook opgenomen in "plots_excluded_from_monitoring_network.xlsx"
...
Deze niet verder meenemen in de berekeningen.
...
#plotinfo opnieuw inladen
plotinfo <- load_plotinfo(database = path_to_fieldmap_db) %>% 
  filter(forest_reserve == forestreserve)
names(plotinfo)
```

In het geval van `r forestreserve` gaat het om volgende plots die uitgesloten worden van de verdere analyse:     

```{r}
plotinfo %>% filter(survey_trees == FALSE) %>% distinct(plot_id)
```

We gaan verder met de processed plots
...
We zien dat er in de cirkelplots 3de decade effectief pas opgemeten wordt vanaf 30 cm.
...
# 26 vs 32, ca 25% wordt niet gedetecteerd
```
...
Globaal mist LIS ca 25% (32 vs 26 m3/ha)
...
diff
# CA is er nu bij
diff <- dendro_by_plot_git %>% anti_join(dendro_by_plot_new, by = c("plot_id", "period"))
diff
