...
table(dataset$forest_reserve, dataset$period)
  #                 1  2  3
  # Everzwijnbad 47 48 48
```

Er is één plot die niet opgeeten werd in 1ste decade:

```{r}
t <- dataset %>% filter(period == 3) %>% 
  anti_join(dataset %>% filter(period ==1),
           by = c("plot_id"))

plotinfo %>% filter(plot_id == t$plot_id)
# survey_reg en survey_veg = FALSE
```
plot `r t$plot_id`  (survey_reg en survey_veg = FALSE)



```{r results = 'hide'}
regeneration_by_plot %>% filter(plot_id == 126) %>% view()
regeneration_by_plot %>% filter(plot_id == 112) %>% view()
```
...
```{r results = 'hide'}
regeneration_by_plot_height %>% filter(plot_id == 126) %>% view()
```
...
```{r results = 'hide'}
dataset2_0 %>% filter(plot_id == 126) %>% view()
```
...
```{r results = 'hide'}
regeneration_by_plot_species %>% filter(plot_id == 126) %>% view()
```
...
```{r results = 'hide'}
dataset4_0 %>% filter(plot_id == 126) %>% view()
```
...
```{r results = 'hide'}
regeneration_by_plot_height_species %>% filter(plot_id == 126) %>% view()
```
...
```{r results = 'hide'}
dataset3_0 %>% filter(plot_id == 126 & species == 87) %>% view()
dataset3_0 %>% filter(plot_id == 126 & species == 16) %>% view()
```
