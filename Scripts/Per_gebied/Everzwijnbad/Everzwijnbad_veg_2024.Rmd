...
Wordt vanaf nu (grotendeels?) opgevangen door check-functies uit forrescalc.
Tijdelijk nog updaten in de databank met query, tot Peter probleem aangepakt heeft.
...
Geen waarnemingen met tree of shrub cover = "Niet beschikbaar"
<!-- Moet < 1% zijn => aanpassen met updatequery -->

```{r}
data_vegetation %>% 
  filter(shrub_cover_interval == "Niet beschikbaar" | tree_cover_interval == "Niet beschikbaar") %>% 
  nrow() == 0

total_TSHcover_missing2 <- data_vegetation %>% 
  filter(is.na(total_shrub_cover_id) | 
           is.na(total_tree_cover_id) |
           is.na(total_herb_cover_id)) 
# 1
```
...
Game disturbance: enkel 1e en 2de decade "20": dat is een NA, want NA as such komt niet voor. 

Dit mag behouden blijven, want wordt op zelfde manier behandeld als een NA.
Enkel aan te passen indien er "< 1%" mee bedoeld wordt. 
...
total_soildist_missing20 <- data_vegetation %>% 
  filter(total_soildisturbance_game_id == 20) # 291; periode 1 en 2
...
total_mosscover_missing <- data_vegetation %>% 
  filter(is.na(total_moss_cover_id)) # 4
...
# data_herblayer %>% filter(is.na(browse_index_id)) %>% nrow()  # 5085
missing_browsing <- data_herblayer %>% filter(is.na(browse_index_id))
table(missing_browsing$period)
#    1    2 
# 2632 2453
```
...
source_file <- here::here(paste0("Scripts/Per_gebied/", forestreserve_short, "/", forestreserve_short, "_veg_2024.html"))
