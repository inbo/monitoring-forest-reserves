---
title: "Statistieken obv cirkelplots"
subtitle: "Heirnisse - vegetatie"
author: "Anja Leyman"
date: "`r Sys.Date()`"
output:
  html_document:
    keep_md: false
    number_sections: true
    fig_caption: true
    code_folding: hide
    toc: true
    toc_float:
      collapsed: true
      smooth_scroll: true
---


```{r Rm, eval = FALSE}
rm(list=ls())
```

```{r Setup, include = FALSE}
knitr::opts_chunk$set(
  echo = TRUE,
  eval = TRUE,
  message = FALSE,
  warning = FALSE, 
  fig.width = 7,
  fig.height = 3,
  fig.align = TRUE)

# libraries & invoergegevens
source(here::here("scripts/Setup.R"))

```


```{r Setup2, include = FALSE}
forest_reserve <- "Heirnisse"
source(here::here("scripts/Per_gebied/", forest_reserve, "/setup_paths_gebied.R"))
path_to_datarequest_teamdrive <- paste0(path_to_teamdrive_gebied, "/04_statistieken_BR/")
```


# Vraagstelling

Statistieken op niveau van bosreservaat `r forestreserve`, obv de cirkelplots.

Alle basis variabelen van de monitoring:
- veg

Om gemiddeldes per BR te verkrijgen, beperken we ons tot de CP's. 
(KV's zijn beter afz. te verwerken)

Dit dient als insteek voor de ontwikkeling van functies die alles integreren:
zie `functions_overall_statistics.R`.

Eerst afzonderlijke functies per dendro_xxxx.csv
Dan al deze functies in één overkoepelend functie

Elke functie moet volgende stappen bevatten:   

- inlezen data     
- filteren op CP’s    
- filteren op afgewerkt      
- toevoegen nulwaardes waar nodig    
- ev. wegfilteren van bosreservaten zonder de vereiste data
- …    


# Inladen data

We werken verder met de data aangemaakt specifiek voor `r forestreserve`.

## Lookuplijsten

```{r }
con <- odbcConnectAccess2007(path_to_fieldmap_db)

  qHerbSpecies <- sqlFetch(con, "qHerbSpecies240810", stringsAsFactors = FALSE)
  qCoverHerbs <- sqlFetch(con, "qCoverHerbs", stringsAsFactors = FALSE)
  qBrowsIndex <- sqlFetch(con, "qBrowsIndex")
  qLayer <- sqlFetch(con, "qLayer")
  qtotalCover <- sqlFetch(con, "qtotalCover")

odbcClose(con)

qHerbSpecies <- qHerbSpecies %>% select(ID, name_nl = Value1)
qBrowsIndex <- qBrowsIndex %>% select(ID, browse_txt = Value1)
qCoverHerbs <- qCoverHerbs %>% select(ID, cover_herbs_txt = Value1)
qtotalCover <- qtotalCover %>% select(ID, cover_txt = Value1)

```


## Plotinfo

```{r }
plotinfo <- read_excel(paste0(path_to_datarequest, "plotinfo.xlsx")) %>% 
   select(-1)

```

## Vegetation

Enkel cirkelplots   

Zonder de plots waar exotenbeheer werd uitgevoerd na de eerste meting (in 2004 en 2008).

Deze plots (10 van de 68) worden mee **opgenomen** in de tabellen met **resultaten op plotniveau**.

In de strata-db staat immers voldoende info over deze plots:      
- in tabel "strata_remaining": veld "unmanaged since": 2008 (ipv 1991 bij de andere)    
- in tabel "cirkelplots_management_startbeheer_exotenbeheer": veld "management_indicator":
exotenbeheer tss periode 1 en 2     
- in tabel "plots_excluded_from_network": veld "info": exotenbeheer (2004 en 2008) na eerste meting     

Bij het berekenen van de **statistieken op reservaatniveau** is het echter logischer deze **NIET**
mee te nemen. 

Het gaat om volgende plots:

```{r }
plots_to_exclude <- read_xlsx(paste0(path_to_output_gdrive
                                     , "00_METADATA-ALL_PLOTS/plots_excluded_from_monitoring_network.xlsx")) %>% 
  filter(forest_reserve == forestreserve)

plots_to_exclude  %>% 
  select(plot_id, info)

```

```{r load_veg_herb_all}
veg_by_plot <- read_excel(
  paste0(path_to_datarequest, "veg_by_plot.xlsx")
  ) %>% 
  select(-1) %>%   
  # select(-contains("eg"), -contains("min40cm")) %>%  # (veg en reg)
  left_join(plotinfo) %>% 
  filter(plottype == "CP") %>% 
  anti_join(plots_to_exclude)

herblayer_by_plot <- read_excel(
  paste0(path_to_datarequest, "herblayer_by_plot.xlsx")
        # bij het inlezen van Excel-bestanden in R met read_excel() uit het readxl-pakket: 
        # als de eerste rij (header) of de eerste niet-lege rij van een kolom leeg is, 
        # probeert R het type te raden en kiest vaak logical. 
        # Daardoor worden alle waarden daarna geforceerd naar TRUE/FALSE/NA.
        # Standaard kijkt read_excel() maar naar de eerste 1000 rijen om het type te raden. 
        # Als de eerste rij leeg is, kan dat misgaan. Je kunt dit verhogen:
  , guess_max = 10000
  ) %>% 
  select(-1) %>%   
  # select(-contains("eg"), -contains("min40cm")) %>%  # (veg en reg)
  left_join(plotinfo) %>% 
  filter(plottype == "CP") %>% 
  anti_join(plots_to_exclude)


# read.xlsx is veel trager, sheetIndex of SheetName verplicht
# herblayer_by_plot <- read.xlsx(
#   paste0(path_to_datarequest, "herblayer_by_plot.xlsx")
#   # , sheetIndex = 1
#   , sheetName = "Sheet1"
#   ) %>% 
#   select(-1) %>%   
#   # select(-contains("eg"), -contains("min40cm")) %>%  # (veg en reg)
#   left_join(plotinfo) %>% 
#   filter(plottype == "CP")


```


# Statistieken

? Waarin geïnteresseerd op reservaatsniveau?    

Op plotniveau:    

- gemiddelde bedekking van vegetatielagen     


Per soort:   

- aantal plots waar soort voorkomt !! best procentueel, want exact aantal plots 
niet gekend door gebruiker    
- karakteristieke bedekking = som van bedekkingen over alle plots heen/aantal plots 
waar de soort voorkomt   
- vraatschade: per soort het percentage plots met minor (< 1/20) of explicit browsing (>= 1/20)    


Enkel bosplots

(10 plots uitgesloten wegens exotenbeheer, zie hoger)


## Gemiddelde bedekking vegetatielagen 

Obv `veg_by_plot`

###  Obv mid-waarde

Gemiddelde bedekking van   

- kruidlaag  
- struiklaag
- boomlaag
- moslaag
- soildisturbance !! indien opgemeten
- cumulated canopy_cover

Indien nodig zouden we ook vraatschade op reservaatsniveau kunnen berekenen, ongeacht de soort.
Maar ik ben niet zeker of dat nut heeft.
Nu enkel op soortniveau gegevens gegenereerd.


```{r data1}
dataset <- veg_by_plot %>%
  select(-contains(c("_min", "_max")))

table(dataset$forest_reserve, dataset$period)

```

Geen plots zonder enige bedekking.

```{r n_subplots_ok, eval = FALSE}
dataset %>% filter(number_of_species == 0) %>% nrow() == 0
summary(dataset)

# één subplot per plot - JA
dataset %>% group_by(plot_id, period) %>% summarize(n_subplots = n_distinct(subplot_id)) %>% 
  ungroup() %>% filter(n_subplots > 1) %>% nrow == 0
```

```{r, results='hide'}
dataset %>% filter(is.na(waterlayer_cover_mid)) %>% nrow()
dataset %>% filter(is.na(moss_cover_mid)) %>% nrow()
dataset %>% filter(is.na(tree_cover_mid)) %>% nrow()
dataset %>% filter(is.na(shrub_cover_mid)) %>% nrow()
dataset %>% filter(is.na(herb_cover_mid)) %>% nrow()

dataset %>% filter(is.na(cumm_herb_coverage_class_average_perc)) %>% nrow()
dataset %>% filter(is.na(cumulated_canopy_cover_mid)) %>% nrow()

# [1] 58  
# [1] 3
# [1] 2
# [1] 60
# [1] 0
# [1] 0
# [1] 60

```
```{r results='hide'}
names(dataset)
```

```{r variables}
variables_for_statistics <- dataset %>% 
  select(contains(c("mid", "perc", "number_of_species"))) %>% 
  names()

variables_for_statistics

```

```{r stat1, results='hide'}
resultaat <- create_statistics(
  dataset = dataset,
  level = c("period", "forest_reserve"),
  variables = variables_for_statistics,
  include_year_range = FALSE,
  na_rm = TRUE   # regelmatig een NA bij één of andere bedekking, vaak op reservaatsniveau
  # door na_rm = TRUE te zetten (wegfilteren van de NA's vooraleer er statistiek op toe te passen)
  # heb je beter zicht op hoeveel bedekkingen er effectief genoteerd werden
  #  --> kijken naar n_obs 
  # TRUE vs FALSE: verschil is dat bij "soildisturbance_game_cover_mid" nObs = 1 ipv 25
  # correcter dus om na_rm = TRUE te nemen (1 -of eigenlijk géén- waarnemingen, en daarom dus NA)
  ) %>% 
  forestmangr::round_df(., 2) %>% 
  # rename(strata = forest_reserve) %>% 
  mutate(strata = NA,
         stratum_name = NA,
         strata2 = NA,
         stratum_name2 = NA)

# library(waldo)
# waldo::compare(resultaat, resultaat_na)

resultaat %>% filter(is.na(mean)) %>% nrow()   
# 1: soildisturbance_game_cover_mid voor periode 1 - OK

# voor export naar excel ipv access-db
resultaat <- resultaat %>% 
  arrange(variable) %>% 
  select(-contains(c("strat"))) # lege velden

str(resultaat)
head(resultaat)
```

```{r eval = FALSE}
# check soildisturbance_game_cover_mid
soildist_BR <- plotinfo %>% 
  group_by(forest_reserve, period) %>% 
  summarize(n_game_impact_veg_T = sum(game_impact_veg == TRUE ),
            n_game_impact_veg_F = sum(game_impact_veg == FALSE )) %>% 
  ungroup()

resultaat2_ <- resultaat %>% left_join(soildist_BR)
# klopt

resultaat2_ %>% filter(variable == "soildisturbance_game_cover_mid" 
                      & is.na(mean)
                      & n_game_impact_veg_T > 0) %>% nrow() == 0
```



```{r table1}
resultaat %>% 
  DT::datatable(options = list(dom = 'tip')
                , rownames = FALSE
                , filter = "top")
```

We exporteren de resultaten naar een excel-file "veg_stat" met 
per tabblad de basisfile bv. veg_per_plot, ...

```{r}
path_to_datarequest
path_to_datarequest_teamdrive

```

```{r export1, results='hide'}
resultaat %>% distinct(variable)
# cumm_herb_coverage_class_average_perc				
# cumulated_canopy_cover_mid				
# herb_cover_mid				
# moss_cover_mid				
# number_of_species				
# shrub_cover_mid				
# soildisturbance_game_cover_mid				
# tree_cover_mid				
# waterlayer_cover_mid

write.xlsx(resultaat
           , paste0(path_to_datarequest, "stat_veg.xlsx")
           , sheetName = "veg_by_plot"
           , showNA = FALSE
           , append = FALSE) 

write.xlsx(resultaat
           , paste0(path_to_datarequest_teamdrive, "stat_veg.xlsx")
           , sheetName = "veg_by_plot"
           , showNA = FALSE
           , append = FALSE) 
```

### Obv intervallen - NIET

Statistics voor intervals cfr Els: variance bijna steeds Inf en mean = 0
=> geen bruikbare resultaten

Ev. later uit te klaren met Els ...


## Aantal/percentage plots waar soort voorkomt

Obv `herblayer_by_plot` in combinatie met `veg_by_plot`

```{r data2}
dataset2 <- herblayer_by_plot

table(dataset2$forest_reserve, dataset2$period)
```

```{r n_subplots_ok2, eval = FALSE}
names(dataset2)
range(dataset2$subplot_id)

# één subplot per plot - JA
dataset2 %>% group_by(plot_id, period) %>% summarize(n_subplots = n_distinct(subplot_id)) %>% 
  ungroup() %>% filter(n_subplots > 1) %>% nrow == 0

```

```{r n_plots_per_species, results='hide'}
names(dataset2)  # herblayer_by_plot, reeds gefilterd op CPs en open area

dataset2 %>% filter(is.na(coverage_class_average_perc)) %>% nrow() == 0

n_plots_per_species <- dataset2 %>% 
  filter(!is.na(coverage_class_average_perc)) %>% 
  group_by(forest_reserve, period, species) %>% 
  summarize(n_plots_species = n()) %>% 
  ungroup()

names(n_plots_per_species)

n_plots_per_BR <- dataset %>%   # veg_by_plot
  # filter(plottype == "CP") %>% 
  group_by(forest_reserve, period) %>%
  summarize(n_total_plots = n()) %>% 
  ungroup()

species_perc_plots <- n_plots_per_species %>% 
  left_join(n_plots_per_BR) %>% 
  mutate(perc_plots = 100*n_plots_species/n_total_plots)

# percentage steeds kleiner dan 100?
species_perc_plots %>% filter(perc_plots > 100) %>% nrow() == 0
```

```{r resultaat2, results='hide'}
names(resultaat)
names(species_perc_plots)

resultaat2 <- species_perc_plots %>% 
  left_join(qHerbSpecies, by = c("species" = "ID")) %>% 
  forestmangr::round_df(., 2) %>% 
  mutate(variable = "perc_plots_with_species"
         , n_obs = n_total_plots
         , mean = perc_plots
         , variance = NA, lci = NA, uci = NA
         , strata = "species"
         , stratum_name = name_nl
         , strata2 = NA
         , stratum_name2 = NA) 

```


```{r table2}
resultaat2 %>% select(forest_reserve, period, name_nl, n_plots_species, 
                      n_total_plots, perc_plots) %>% 
  DT::datatable(options = list(dom = 'tip')
                , rownames = FALSE
                , filter = "top")
```

We exporteren de resultaten naar een excel-file "veg_stat" met 
per tabblad de basisfile bv. veg_per_plot, ...

```{r}
path_to_datarequest
path_to_datarequest_teamdrive

```

```{r export2, eval = FALSE}
resultaat2 %>% distinct(variable)
# perc_plots_with_species

# NIET NODIG, want ook in resultaat 3
# write.xlsx(resultaat2
#            , paste0(path_to_datarequest, "stat_veg.xlsx")
#            , sheetName = "perc_plots_with_species"
#            , showNA = FALSE
#            , append = TRUE) 
# 
# write.xlsx(resultaat2
#            , paste0(path_to_datarequest_teamdrive, "stat_veg.xlsx")
#            , sheetName = "perc_plots_with_species"
#            , showNA = FALSE
#            , append = TRUE) 
```


## Karakteristieke bedekking

Obv aantal plots waar de soort voorkomt

Karakteristieke bedekking = som van bedekkingen in de plots waar soort voorkomt, 
gedeeld door het aantal plots waar de soort voorkomt


```{r results='hide'}
dataset2 %>% filter(is.na(coverage_class_average_perc)) %>% nrow() == 0
names(dataset2)
karakt_bedekking <- dataset2 %>% 
  group_by(forest_reserve, period, species) %>% 
  summarize(sum_cover = sum(coverage_class_average_perc)) %>% 
  ungroup() %>% 
  left_join(species_perc_plots) %>% 
  mutate(karakt_bedekking = sum_cover/n_plots_species)
  
```

```{r resultaat3, results='hide'}
names(karakt_bedekking)
resultaat3 <- karakt_bedekking %>% 
  left_join(qHerbSpecies, by = c("species" = "ID")) %>% 
  forestmangr::round_df(., 2) 
# %>% 
#   mutate(variable = "karakt_bedekking"
#          , n_obs = n_total_plots
#          , mean = karakt_bedekking
#          , variance = NA, lci = NA, uci = NA
#          , strata = "species"
#          , stratum_name = name_nl
#          , strata2 = NA
#          , stratum_name2 = NA) 
  
names(resultaat3)
```


```{r table3}
resultaat3 %>% select(forest_reserve, period, name_nl, sum_cover, n_plots_species, 
                      karakt_bedekking) %>% 
  DT::datatable(options = list(dom = 'tip')
                , rownames = FALSE
                , filter = "top")



```

We exporteren de resultaten naar een excel-file "veg_stat" met 
per tabblad de basisfile bv. veg_per_plot, ...

```{r}
path_to_datarequest
path_to_datarequest_teamdrive

```

```{r export3}
write.xlsx(resultaat3
           , paste0(path_to_datarequest, "stat_veg.xlsx")
           , sheetName = "karakt_bedekking"
           , showNA = FALSE
           , append = TRUE) 

write.xlsx(resultaat3
           , paste0(path_to_datarequest_teamdrive, "stat_veg.xlsx")
           , sheetName = "karakt_bedekking"
           , showNA = FALSE
           , append = TRUE) 
```



### Vraatschade

Aantal plots en percentage plots met meer of minder vraatschade: 

- Geen vraatschade	  
- Vraatschade waarneembaar (minder 1/20)	  
- Vraatschade duidelijk (meer dan 1/20)	  
- Perc_plots_met vraatschade	  
- Perc_plots_met vraatschade_>20%   


Obv aantal plots waar de soort voorkomt

Percentage plots met vraatschade = aantal plots met vraatschade, 
gedeeld door het aantal plots waar de soort voorkomt


```{r results='hide'}
dataset2 %>% filter(is.na(browse_index) & game_impact_veg == 10) %>% nrow() == 0
names(dataset2)
table(dataset2$browse_index, dataset2$browse_index_id)

vraatschade <- dataset2 %>% 
  filter(game_impact_veg == TRUE) %>%  # enkel waar vraat opgemeten is
  group_by(forest_reserve, period, species) %>% 
  summarize(n_plots_browsing = sum(browse_index != "No browsing")
            , n_plots_minor_browsing = sum(browse_index == "Browsing observed (less than 1/20)")
            , n_plots_explicit_browsing = sum(browse_index == "Browsing explicit (more than 1/20)")
            ) %>% 
  ungroup() %>% 
  left_join(species_perc_plots %>% select(forest_reserve, period, species, n_plots_species)
            , by = c("forest_reserve", "period", "species")) %>% 
  mutate(perc_plots_browsing = 100 * n_plots_browsing/n_plots_species
         , perc_plots_minor_browsing = 100 * n_plots_minor_browsing/n_plots_species
         , perc_plots_explicit_browsing = 100 * n_plots_explicit_browsing/n_plots_species)
  
```

```{r resultaat4, results='hide'}
names(vraatschade)
resultaat4 <- vraatschade %>% 
  left_join(qHerbSpecies, by = c("species" = "ID")) %>% 
  forestmangr::round_df(., 2) 
# %>% 
#   mutate(variable = "karakt_bedekking"
#          , n_obs = n_total_plots
#          , mean = karakt_bedekking
#          , variance = NA, lci = NA, uci = NA
#          , strata = "species"
#          , stratum_name = name_nl
#          , strata2 = NA
#          , stratum_name2 = NA) 
  
names(resultaat4)
```


```{r table4}
resultaat4 %>% select(name_nl, period
                      , contains("plots")
                      , -perc_plots_browsing
                      ) %>% 
  DT::datatable(options = list(dom = 'tip')
                , rownames = FALSE
                , filter = "top")



```

We exporteren de resultaten naar een excel-file "veg_stat" met 
per tabblad de basisfile bv. veg_per_plot, ...

```{r}
path_to_datarequest
path_to_datarequest_teamdrive

```

```{r export4}
write.xlsx(resultaat4
           , paste0(path_to_datarequest, "stat_veg.xlsx")
           , sheetName = "browsing"
           , showNA = FALSE
           , append = TRUE) 

write.xlsx(resultaat4
           , paste0(path_to_datarequest_teamdrive, "stat_veg.xlsx")
           , sheetName = "browsing"
           , showNA = FALSE
           , append = TRUE) 
```


# Copy html to teamdrive

Na knitting - manueel

```{r eval = FALSE}
# Specify the path of the file you want to copy
source_file <- here::here(paste0("Scripts/Per_gebied/", forestreserve_short, "/", forestreserve_short, "_stat_veg_2025.html"))

# Specify the destination directory where you want to copy the file
destination_dir <- path_to_datarequest_teamdrive

# Copy the file to the destination directory
file.copy(from = source_file, to = destination_dir, overwrite = TRUE)
# ? file.copy
```
