...
```{r eval = FALSE}
reg_by_plot %>% filter(plot_id == 708) %>% view()
```
...
```{r eval = FALSE}
reg_by_plot_height %>% filter(plot_id == 708) %>% view()
```
...
# bij periode 1 ook nog heightclass = NA, zie verder, geen probleem

# n_distinct(dataset2$plot_id, dataset2$period) == 99
```
...
```{r eval = FALSE}
dataset2_0 %>% filter(plot_id == 708) %>% view()
```
...
```{r eval = FALSE}
reg_by_plot_species %>% filter(plot_id == 708) %>% view()
```
...
dataset4_0 <- add_zeros(dataset = dataset4 %>% 
                        ...
                        grouping_vars = c("forest_reserve", "period")  
                        #period als grouping var zorgt ervoor dat er geen soorten 
...
```{r results = 'hide'}
dataset4_0 %>% filter(plot_id == 708) %>% view()
```
...
```{r results = 'hide'}
reg_by_plot_height_species %>% filter(plot_id == 708) %>% view()
```
...
```{r results = 'hide'}
dataset3_0 %>% filter(plot_id == 708 & species == 87) %>% view()
dataset3_0 %>% filter(plot_id == 708 & height_class == 4000) %>% view()
```
...
source_file <- here::here(paste0("Scripts/Per_gebied/", forestreserve_short, "/", forestreserve_short, "_stat_reg_2025.html"))
