...
subtitle: "Heirnisse - dendrometrie"
author: "Anja Leyman"
date: "`r Sys.Date()`"
output:
  html_document:
...
Zonder de plots waar exotenbeheer werd uitgevoerd na de eerste meting (in 2004 en 2008).

Deze plots (10 van de 68) worden mee **opgenomen** in de tabellen met **resultaten op plotniveau**.
...
# mean()
t1 <- dataset %>% filter(period == 3)
mean(t1$vol_LIS_m3_ha)
```
...
nrow(dataset2_0) #5916
```

> hierna zie je dat er iets niet klopt: mean(LIS per plot) = 39
maar som van mean per soort = 35



```{r}
# komt elke soort in elke plot voor?
t2 <- dataset2_0 %>% 
  filter(period == 3) %>% 
  group_by(species) %>% 
  summarize(n = n()
  ...
  ungroup()
range(t2$n) # 68 68
# JA, elke plot komt voor, 
range(t2$mean_LIS) #  0.00000 14.68063 => geen NA's , enkel nullen
...
```{r results='hide'}
# check of deadwood volumes die NA moeten zijn, niet 0 geworden zijn (LIS en log_above30cm)
dataset2_0 %>% filter(period %in% c(1,2) & !is.na(vol_LIS_m3_ha)) %>% nrow() 
...
Makkelijkste om dataset2_0 te corrigeren, want daar kunnen we nog link leggen met `lis_survey.`
...
dataset3_0_ <- dataset3_0 %>% 
  ...
  mutate(vol_log_m3_ha= ifelse(dbh_class_5cm %in% diam_klasses_under_10cm
                               , NA, vol_log_m3_ha)
           , vol_log_m3_ha= ifelse(survey_lis == TRUE & dbh_class_5cm %in% diam_klasses_under_30cm
                               , NA, vol_log_m3_ha)
         )
# t <- dataset2_0_ %>% filter(is.na(vol_LIS_m3_ha) & !is.na(vol_log_above30cm_m3_ha))
