...
# ok, is total_soildisturbance_game periode 1
```
...
total_soildist_missing20 <- data_vegetation %>% 
  filter(total_soildisturbance_game_id == 20) # 25 records periode 1

data_vegetation %>% filter(is.na(total_soildisturbance_game_id)) %>% nrow() == 25
```
...
Geen vegetatie-opnames in periode 2
...
data_herblayer %>% filter(is.na(browse_index_id)) %>% distinct(period) # OK enkel periode 1
```
