...
<!-- **OPGEPAST**: plot 212: geen verjonging, 0 soorten.  -->
<!-- Gevraagd aan Peter (mail 19/11/25), kan inderdaad 0 zijn, ligt aan rand van ven
(antw 20/11/2025) -->
<!-- Code al opnieuw gerund -->
...
```{r TEMP_plot212_NA, eval = FALSE}
# plot 212: even gedcaht dat reg niet genoteerd was in periode 2
# maar blijkt nu toch wellicht geen verjonging te zijn
...
reg_by_plot %>% filter(plot_id == 212) %>% view()
```
...
reg_by_plot_height %>% filter(plot_id == 212) %>% view()
```
...
dataset2_0 %>% filter(plot_id == 212) %>% view()
```
...
nrow(resultaat2); n
# OK, niet exact zelfde want rubbing maar in één periode opgemeten
...
```{r eval = FALSE}
reg_by_plot_species %>% filter(plot_id == 212) %>% view()
```
...
```{r results = 'hide'}
dataset4_0 %>% filter(plot_id == 212) %>% view()
```
...
```{r results = 'hide'}
reg_by_plot_height_species %>% filter(plot_id == 212) %>% view()
```
...
```{r results = 'hide'}
dataset3_0 %>% filter(plot_id == 212 & species == 87) %>% view()
dataset3_0 %>% filter(plot_id == 212 & height_class == 5000) %>% view()
```
