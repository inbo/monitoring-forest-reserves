...
Periode 3: beperkt (12 missings), nochtans had Peter alle "missings" opgelost??   

> snelle check: plot 816, tree-id 19: zit niet in vorige databank, maar wel in de nieuwe?
...
- 1 walker coppice-id: volgens Peter "verkeerd gekoppeld", maar blijkbaar nog niet opgelost    

=> we behouden enkel die ene walker coppice-id nog

```{r filter}
incorrect_diff_ <- incorrect_diff %>% 
  filter(anomaly == "walker coppice_id")

incorrect_diff <- incorrect_diff_
```
...
```  
Eén deviating date di evolgens Peter correct is.
...
- nog enkele "missings" in periode 3 ==> zie [missing_NA_controle.xlsx](https://docs.google.com/spreadsheets/d/1s6-K2N1zgOMwV2i_by034HEswKEDWe34?rtpof=true&usp=drive_fs)

Nog 12 "missings", nochtans had Peter alle “missings” opgelost!
**Snelle check: plot 816, tree-id 19: zit niet in vorige databank, maar wel in de nieuwe?**

- één walker die niet opgelost is ==> zie [incorrect_NA_controle.xlsx](https://docs.google.com/spreadsheets/d/1t_yPupSo5LhAdXcmDOABe6rMunGtc8Di?rtpof=true&usp=drive_fs)


- EVENTUEEL iufro_heights_0
