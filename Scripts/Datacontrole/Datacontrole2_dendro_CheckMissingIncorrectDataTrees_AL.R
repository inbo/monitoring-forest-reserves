# DOEL: datacontrole trees - focus op missing en impossible values ----

# ter aanvulling op functie uit forrescalc
...
missing_values <- check_data_trees2(path_to_fieldmap)
...
missing_values <- data_trees %>%
  # niet in A3 of A4
  mutate(
    missing =
      ifelse(
        .data$Plottype == 20 & sqrt(.data$X_m ^ 2 + .data$Y_m ^ 2) > r_A4,
        "tree not in A4",
        NA
      ),
    problem =
      ifelse(
        .data$Plottype == 20 & .data$AliveDead == 11 & .data$DBH_mm < dbh_min_a4 &
          sqrt(.data$X_m ^ 2 + .data$Y_m ^ 2) > r_A3,
        "tree not in A3",
        .data$problem
      ),
    problem =
      ifelse(
        .data$Plottype == 20 & .data$AliveDead == 12 & .data$DBH_mm < dbh_min_a4_dead &
          sqrt(.data$X_m ^ 2 + .data$Y_m ^ 2) > r_A3,
        "tree not in A3",
        .data$problem
      )
    ...
    problem =
      ifelse(
        ...
        ifelse(
          !is.na(.data$coppiceproblem),
          paste(.data$problem, .data$coppiceproblem, sep = " / "),
          # dus opsomming vna alle "problems" gescheiden door komma
          .data$problem
        ),
        .data$coppiceproblem
      ),
